...
class PlayerBuilder {
  public:
    ...
    PlayerBuilder &with_id(EntityId id) {
        id_ = id;
        return *this;
    }
    PlayerBuilder &named(std::string_view name) {
        name_ = name;
        return *this;
    }
    PlayerBuilder &at_level(int level) {
        level_ = level;
        return *this;
    }
    PlayerBuilder &of_class(std::string_view cls) {
        character_class_ = cls;
        return *this;
    }
    PlayerBuilder &of_race(std::string_view race) {
        race_ = race;
        return *this;
    }

    PlayerBuilder &with_strength(int str) {
        strength_ = str;
        return *this;
    }
    PlayerBuilder &with_dexterity(int dex) {
        dexterity_ = dex;
        return *this;
    }
    PlayerBuilder &with_intelligence(int intel) {
        intelligence_ = intel;
        return *this;
    }
    PlayerBuilder &with_constitution(int con) {
        constitution_ = con;
        return *this;
    }

    PlayerBuilder &with_hp(int hp) {
        hit_points_ = hp;
        return *this;
    }
    PlayerBuilder &with_max_hp(int max_hp) {
        max_hit_points_ = max_hp;
        return *this;
    }

    PlayerBuilder &as_warrior() { return of_class("warrior").with_strength(16).with_hp(80); }

    PlayerBuilder &as_sorcerer() { return of_class("sorcerer").with_intelligence(16).with_hp(50); }

    PlayerBuilder &as_rogue() { return of_class("rogue").with_dexterity(16).with_hp(60); }

    PlayerBuilder &as_cleric() { return of_class("cleric").with_constitution(15).with_hp(70); }
    ...
    std::unique_ptr<Player> build() {
        ...
        if (strength_)
            stats.strength = *strength_;
        if (dexterity_)
            stats.dexterity = *dexterity_;
        if (intelligence_)
            stats.intelligence = *intelligence_;
        if (constitution_)
            stats.constitution = *constitution_;

        if (hit_points_)
            stats.hit_points = *hit_points_;
        if (max_hit_points_)
            stats.max_hit_points = *max_hit_points_;
...
class ObjectBuilder {
  public:
    ...
    ObjectBuilder &with_id(EntityId id) {
        id_ = id;
        return *this;
    }
    ObjectBuilder &named(std::string_view name) {
        name_ = name;
        return *this;
    }
    ObjectBuilder &of_type(ObjectType type) {
        type_ = type;
        return *this;
    }
    ObjectBuilder &with_weight(int weight) {
        weight_ = weight;
        return *this;
    }
    ObjectBuilder &with_value(int value) {
        value_ = value;
        return *this;
    }
    ObjectBuilder &with_description(std::string_view desc) {
        description_ = desc;
        return *this;
    }
    ...
    ObjectBuilder &as_armor(int ac_bonus = 2) { return of_type(ObjectType::Armor).with_value(ac_bonus * 10); }
...
class RoomBuilder {
  public:
    ...
    RoomBuilder &with_id(EntityId id) {
        id_ = id;
        return *this;
    }
    RoomBuilder &named(std::string_view name) {
        name_ = name;
        return *this;
    }
    RoomBuilder &with_sector(SectorType sector) {
        sector_ = sector;
        return *this;
    }
    RoomBuilder &with_description(std::string_view desc) {
        description_ = desc;
        return *this;
    }

    RoomBuilder &with_exit(std::string_view direction, EntityId target_room, bool closed = false, bool locked = false) {
    ...
    RoomBuilder &as_outdoor() {
        return with_sector(SectorType::Field).with_description("This is an outdoor area with open sky above.");
    }

    RoomBuilder &as_dungeon_room() {
        return with_sector(SectorType::Inside).with_description("A dark, stone-walled chamber deep underground.");
    }

    RoomBuilder &as_safe_room() {
        return named("Safe Room").with_description("This room emanates a feeling of safety and peace.");
    }
...
class ScenarioBuilder {
  public:
    ...
    static CombatScenario create_basic_combat() {
        auto room = RoomBuilder().named("Combat Arena").as_dungeon_room().build();

        auto attacker = PlayerBuilder().named("Attacker").as_warrior().at_level(10).build();

        auto target = PlayerBuilder().named("Target").as_sorcerer().at_level(8).build();

        return CombatScenario{std::move(attacker), std::move(target), std::move(room)};
    }
    ...
    static TreasureScenario create_treasure_hunt() {
        ...
        auto player = PlayerBuilder().named("TreasureHunter").as_rogue().at_level(5).build();
        ...
        return TreasureScenario{std::move(player), std::move(room), std::move(treasures)};
    }
};
...
class RandomDataGenerator {
  public:
    explicit RandomDataGenerator(uint32_t seed = std::random_device{}()) : rng_(seed) {}
    ...
    std::unique_ptr<Object> random_object() {
        auto types = std::vector<ObjectType>{ObjectType::Weapon, ObjectType::Armor, ObjectType::Treasure,
  ...
  private:
    ...
    std::string generate_random_name() {
        ...
        return prefixes[prefix_dist(rng_)] + suffixes[suffix_dist(rng_)] + std::to_string(num_dist(rng_));
    }
};
