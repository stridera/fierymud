...
FieryMUD uses a modern, stable test framework built on **Catch2** with custom test harnesses designed for MUD-specific functionality. The framework provides:

- ✅ **100% reliable** stable tests (no segfaults)
- ⚡ **Fast execution** (synchronous, no threads)
- 🔧 **Rich test objects** for comprehensive integration testing
- 📊 **Multiple test categories** (unit, stable integration, legacy integration)
...
| Category | Executable | Description | Status |
|----------|------------|-------------|--------|
| **Unit Tests** | `unit_tests` | Fast, isolated component tests | ✅ Stable |
| **Stable Integration** | `stable_tests` | Reliable integration tests with LightweightTestHarness | ✅ Stable |
| **Legacy Integration** | `integration_tests` | Older tests (may have threading issues) | ⚠️ Unstable |
...
│   └── test_container_system_simple.cpp
├── integration/             # Integration tests
│   ├── test_*_stable.cpp   # Stable integration tests ← RECOMMENDED
│   └── test_*.cpp          # Legacy integration tests
├── common/                  # Shared test infrastructure
│   ├── lightweight_test_harness.hpp  # Modern test harness ← USE THIS
│   ├── test_harness.hpp    # Legacy harness (deprecated)
...
cmake --build build --target stable_tests    # Stable integration tests
cmake --build build --target tests          # Combined (unit + stable)

# Legacy targets (use with caution)
cmake --build build --target integration_tests  # Legacy integration
```
...
```bash
# Run the most reliable tests (unit + stable integration)
./run_tests.sh                    # Default: recommended tests
./run_tests.sh recommended        # Same as above

# Run specific test categories
./run_tests.sh unit               # Unit tests only
./run_tests.sh stable             # Stable integration tests only
...
```bash
# Run stable integration tests with specific filters
./build/stable_tests "[objects]"           # Object system tests
./build/stable_tests "[enhanced]"          # Enhanced harness features
...
The `LightweightTestHarness` is the **recommended** framework for integration testing.
...
harness.execute_command("get test_torch")
       .execute_command("light test_torch")    // Activate light source
       .execute_command("equipment");          // Should show lit torch

// Consumable testing
harness.execute_command("get test_bread")
       .execute_command("eat test_bread");     // Consume food item

// Scroll usage
harness.execute_command("get test_scroll")
       .execute_command("read test_scroll");   // Use single-use item

// Liquid container testing
harness.execute_command("get test_waterskin")
       .execute_command("drink from test_waterskin");
```
...
# Solution: Ensure filename ends with "_stable.cpp" for stable tests
...
harness.execute_command("north");
// Add delay or check command success first
REQUIRE(harness.current_room_id() == EntityId{101});
```
...
- Name stable test files with **"_stable.cpp"** suffix
...
- [ ] File named `*_stable.cpp`
- [ ] Includes `LightweightTestHarness`
- [ ] Test case has `[stable]` tag
- [ ] Tests both success and error cases
- [ ] Verifies command output
- [ ] Checks state changes
- [ ] Runs within performance budget
- [ ] Added to appropriate test category

## Contributing

When adding new test functionality:

1. **Follow existing patterns** from stable tests
2. **Document new test objects** in this guide  
3. **Update examples** if adding new harness features
4. **Run full test suite** before committing
5. **Update migration status** if fixing legacy tests

## Migration Status ✅ COMPLETED

The test framework migration from legacy (segfaulting) tests to stable LightweightTestHarness tests has been **completed successfully**:

- ✅ **100% stable test success rate** achieved
- ✅ **LightweightTestHarness** replaces problematic TestHarness
- ✅ **Enhanced test objects** provide comprehensive testing capabilities
- ✅ **Build system** properly separates stable vs legacy tests
- ✅ **Documentation** complete and up-to-date

### Recommended Usage

- **Primary**: Use stable integration tests (`stable_tests`) for all new development
- **Secondary**: Use unit tests (`unit_tests`) for isolated component testing
- **Legacy**: Avoid legacy integration tests unless absolutely necessary

---

*Last Updated: 2024-08-27*  
*Framework Version: Modern Stable (LightweightTestHarness-based)*