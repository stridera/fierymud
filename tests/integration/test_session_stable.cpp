...
TEST_CASE("Session Stable: State Consistency and Lifecycle", "[session][integration][stable]") {
    ...
    SECTION("State consistency through movement") {
        ...
        harness.execute_command("look").then_output_contains("North Room");
    ...
    SECTION("State recovery after errors") {
        ...
        harness.execute_command("invalidcommand").then_output_contains("Unknown command");
...
TEST_CASE("Session Stable: Regression Testing", "[session][integration][regression][stable]") {
    SECTION("No memory leaks during session creation/destruction") {
        ...
        for (int i = 0; i < 50; ++i) {
            ...
            session->execute_command("look").then_output_contains("Test Start Room");
        ...
        for (auto &session : sessions) {
            session->execute_command("say Session still active").then_output_contains("You say");
        }
    ...
    SECTION("Session state consistency under stress") {
        ...
        std::vector<std::string> commands = {"look",  "say test", "emote nods",       "north",
                                             "south", "look",     "say another test", "emote smiles"};
