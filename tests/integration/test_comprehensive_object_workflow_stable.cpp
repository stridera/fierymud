...
TEST_CASE("Comprehensive Object Workflow: Container Management", "[integration][stable][workflow][containers]") {
    ...
    SECTION("Complete chest unlock, open, get workflow") {
        // Step 1: Get the key
        harness.execute_command("get test_key").then_output_not_contains("error").then_player_stat_equals("level", 1);
...
TEST_CASE("Comprehensive Object Workflow: Equipment Systems", "[integration][stable][workflow][equipment]") {
    ...
    SECTION("Complete weapon and armor equipping workflow") {
        ...
        harness.execute_command("get test_sword").then_output_not_contains("error");
        harness.execute_command("get test_shield").then_output_not_contains("error");

        // Step 2: Equip weapon
        harness.execute_command("wield test_sword").then_output_not_contains("error");

        // Step 3: Equip armor
        harness.execute_command("wear test_shield").then_output_not_contains("error");
...
TEST_CASE("Comprehensive Object Workflow: Mixed Object Interactions", "[integration][stable][workflow][mixed]") {
    ...
    SECTION("Object state persistence through complex operations") {
        ...
        std::vector<std::string> all_objects = {"test_chest",   "test_sword",    "test_shield", "test_torch",
...
TEST_CASE("Comprehensive Object Workflow: Edge Cases and Error Recovery",
          "[integration][stable][workflow][edge_cases]") {
    LightweightTestHarness harness;
    ...
    SECTION("State consistency under stress") {
        ...
        std::vector<std::string> stress_commands = {"look",
                                                    "inventory",
                                                    "equipment",
                                                    "get test_key",
                                                    "drop test_key",
                                                    ...
                                                    "remove test_sword",
                                                    "get test_bag",
                                                    "put test_key in test_bag",
                                                    "get test_key from test_bag"};
...
TEST_CASE("Comprehensive Object Workflow: Performance Validation", "[integration][stable][workflow][performance]") {
    ...
    SECTION("Object system scalability") {
        ...
        std::vector<std::string> all_objects = {"test_sword",   "test_shield",    "test_torch",
                                                "test_lantern", "test_key",       "test_bread",
                                                "test_scroll",  "test_waterskin", "test_bag"};
