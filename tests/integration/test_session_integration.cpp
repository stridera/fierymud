/***************************************************************************
 *   File: tests/test_session_integration.cpp Part of FieryMUD *
 *  Usage: Integration tests for game sessions and player interaction      *
 ***************************************************************************/
...
TEST_CASE("Session Integration: Basic Character Creation", "[session][integration]") {
...
TEST_CASE("Session Integration: Character Creation with Custom Names", "[session][integration]") {
    UnifiedTestHarness::run_session_test([&](MockGameSession &session) {
        struct TestCase {
            std::string name;
            bool should_work;
        ...
        std::vector<TestCase> test_cases = {{"Aragorn", true, "Normal name"},
                                            {"Bob", true, "Short name"},
        ...
        for (const auto &test_case : test_cases) {
            ...
            if (test_case.should_work && !test_case.name.empty()) {
                REQUIRE(session.wait_for_output_containing("Welcome, " + test_case.name + "!",
...
TEST_CASE("Session Integration: Multiple Sessions", "[session][multi][integration]") {
...
TEST_CASE("Session Integration: Basic Commands", "[session][integration]") {
    UnifiedTestHarness::run_session_test([&](MockGameSession &session) {
...
TEST_CASE("Session Integration: Movement", "[session][integration]") {
    UnifiedTestHarness::run_session_test([&](MockGameSession &session) {
...
TEST_CASE("Session Integration: Error Handling", "[session][integration]") {
    UnifiedTestHarness::run_session_test([&](MockGameSession &session) {
...
TEST_CASE("Session Integration: Session Persistence", "[session][integration]") {
    UnifiedTestHarness::run_session_test([&](MockGameSession &session) {
...
TEST_CASE("Session Integration: Environment Interaction", "[session][integration]") {
    UnifiedTestHarness::run_session_test([&](MockGameSession &session) {
...
TEST_CASE("Session Integration: Multi-Player Interaction", "[session][integration]") {
    UnifiedTestHarness::run_session_test([&](MockGameSession &session1) {
        auto session2 = UnifiedTestHarness::create_session();
