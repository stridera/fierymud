...
TEST_CASE("Enhanced Features: Multi-Command Execution", "[integration][enhanced][multi]") {
    ...
    std::vector<std::string> commands = {"look", "north", "look", "south"};
...
TEST_CASE("Enhanced Features: State Validation", "[integration][enhanced][state]") {
    ...
    harness.then_player_stat_equals("level", 1).then_player_stat_equals("max_hp", 20);
...
TEST_CASE("Enhanced Features: Comprehensive Integration", "[integration][enhanced][comprehensive]") {
    ...
    harness.then_player_stat_equals("level", 1).then_output_size_is(0); // No initial output

    // Execute complex command sequence
    std::vector<std::string> sequence = {"say Starting comprehensive test",
                                         "look",
                                         "north",
                                         "say I moved north",
                                         "look",
    ...
    harness.then_executes_within_ms([&]() { harness.execute_commands_accumulate(sequence); }, 200);
    ...
    // Final validation
    harness
        .then_player_stat_equals("level", 1) // Should still be level 1
...
TEST_CASE("Enhanced Features: Error Handling", "[integration][enhanced][error]") {
    ...
    harness
        .execute_command("west") // No west exit
...
TEST_CASE("Enhanced Features: Object System Integration", "[integration][enhanced][objects]") {
    SECTION("Interactive objects are created and accessible") {
        ...
        harness.execute_command("inventory").then_output_contains("test_sword");

        harness.execute_command("drop test_sword");
        harness.execute_command("look").then_output_contains("test_sword");
    }
    ...
    SECTION("Diverse object types testing") {
        ...
        std::vector<std::string> test_objects = {"test_chest", "test_sword", "test_shield", "test_torch",
                                                 "test_key",   "test_bread", "test_bag"};
...
TEST_CASE("Enhanced Features: Performance Benchmarks", "[integration][enhanced][benchmark]") {
    ...
    SECTION("Navigation performance") {
        ...
        harness.then_executes_within_ms(
            [&]() {
                for (int i = 0; i < 20; ++i) {
                    harness.execute_command("north").execute_command("south");
                }
            },
    ...
    SECTION("Output processing performance") {
        ...
        harness.then_executes_within_ms([&]() { harness.execute_commands_accumulate(chatty_commands); },
