...
TEST_CASE("CommandSystem Stable: Integration", "[command][system][integration][stable]") {
    SECTION("Command system initialization and registration") {
        ...
        harness.execute_command("north").then_output_contains("You move north");

        REQUIRE(harness.current_room_id() == EntityId{101});

        harness.execute_command("south").then_output_contains("You move south");
        ...
        harness.execute_command("who").then_output_contains("No players are currently online");
    }
    ...
    SECTION("Command aliases and abbreviations") {
        ...
        harness.execute_command("l").then_output_contains("Test Start Room");

        harness.execute_command("n").then_output_contains("You move north");
        REQUIRE(harness.current_room_id() == EntityId{101});

        harness.execute_command("s").then_output_contains("You move south");
        REQUIRE(harness.current_room_id() == EntityId{100});

        // Test inventory alias
        harness.execute_command("i").then_output_contains("You are not carrying anything");
    }
    ...
    SECTION("Command error handling and invalid commands") {
        ...
        harness.execute_command("invalidcommandname").then_output_contains("Unknown command");
        ...
        harness.execute_command("west").then_output_contains("Cannot move West");

        harness.execute_command("up").then_output_contains("Cannot move Up");
...
TEST_CASE("CommandSystem Stable: Advanced Integration", "[command][system][advanced][stable]") {
    SECTION("Command execution sequence and state consistency") {
        ...
        for (const auto &[command, expected_output] : command_sequence) {
            harness.execute_command(command).then_output_contains(expected_output);
    ...
    SECTION("Command system performance and stress testing") {
        ...
        std::vector<std::string> stress_commands = {"look", "say test", "emote nods", "i", "who", "look"};
    ...
    SECTION("Command privilege system testing") {
        ...
        harness.execute_command("say I can use player commands").then_output_contains("You say");

        harness.execute_command("who").then_output_contains("No players are currently online");

        harness.execute_command("inventory").then_output_contains("You are not carrying anything");
...
TEST_CASE("CommandSystem Stable: Regression Testing", "[command][system][regression][stable]") {
    SECTION("No crashes on rapid command execution") {
        ...
        std::vector<std::string> rapid_commands = {"look", "say test1", "north", "south", "emote nods", "who",
    ...
    SECTION("Memory stability during extended command sessions") {
        ...
        std::vector<std::string> session_commands = {"look",  "say hello", "emote waves", "who",        "i",
