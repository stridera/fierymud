...
TEST_CASE("Multiplayer: Player Movement and Separation", "[multiplayer][stable]") {
    SECTION("Players can move independently") {
        ...
        // Alice moves north
        alice_harness.execute_command("north").then_output_contains("You move north");
        ...
        alice_harness.execute_command("look").then_output_contains("North Room");

        // Bob looks around in original room
        bob_harness.execute_command("look").then_output_contains("Test Start Room");

        // Test room-specific commands work correctly
        alice_harness.execute_command("say I'm in the north room").then_output_contains("You say");

        bob_harness.execute_command("say I'm in the start room").then_output_contains("You say");
    }
    ...
    SECTION("Players can reunite by moving") {
        ...
        charlie_harness.execute_command("north").then_output_contains("You move north");
        ...
        diana_harness.execute_command("north").then_output_contains("You move north");
        ...
        charlie_harness.execute_command("look").then_output_contains("North Room");

        diana_harness.execute_command("look").then_output_contains("North Room");
    }
}
...
TEST_CASE("Multiplayer: Stress Testing and Stability", "[multiplayer][stress]") {
    SECTION("Multiple harnesses handle rapid commands") {
        ...
        for (auto &harness : harnesses) {
            harness->execute_command("look").then_output_contains("Test Start Room");
        }

        INFO("Successfully executed " << (num_players * commands_per_player) << " commands across " << num_players
                                      << " players");
    }
...
TEST_CASE("Multiplayer: Error Handling and Recovery", "[multiplayer][error]") {
    SECTION("Invalid commands don't affect other players") {
        ...
        harry_harness.execute_command("invalidcommandxyz").then_output_contains("Unknown command");

        // Iris should be unaffected and able to execute valid commands
        iris_harness.execute_command("look").then_output_contains("Test Start Room");

        // Harry should recover and be able to execute valid commands
        harry_harness.execute_command("look").then_output_contains("Test Start Room");
    }
    ...
    SECTION("Network simulation errors don't cascade") {
        ...
        kelly_harness.execute_command("north").then_output_contains("You move north");

        REQUIRE(kelly_harness.current_room_id() == EntityId{101});

        // Jack should still be functional after errors
        jack_harness.execute_command("look").then_output_contains("Test Start Room");
...
TEST_CASE("Multiplayer: Regression Testing", "[multiplayer][regression]") {
    SECTION("No segfaults during rapid player creation") {
        ...
        for (auto &harness : rapid_harnesses) {
            harness->execute_command("say Still working").then_output_contains("You say");
        }
    ...
    SECTION("Session isolation prevents state bleeding") {
        ...
        clean_harness.execute_command("look").then_output_contains("Test Start Room");
...
TEST_CASE("Multiplayer: Performance Benchmarks", "[multiplayer][performance]") {
    ...
    SECTION("Parallel command execution performance") {
        ...
        INFO("Executed " << total_commands << " commands in " << duration.count() << "ms (" << commands_per_second
                         << " commands/second)");
    }
}
