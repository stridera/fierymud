...
TEST_CASE("World Parsing: Room objects and ID references", "[world][parsing][rooms]") {
    ...
    SECTION("Parse zone with room objects") {
        ...
        json zone_json = {
            {"zone",
             {{"id", "1000"}, {"name", "Room Object Zone"}, {"top", 1099}, {"lifespan", 30}, {"reset_mode", "Normal"}}},
            {"rooms",
             {{{"id", "1001"},
               {"name", "Test Room 1"},
    ...
    SECTION("Parse zone with room ID references") {
        ...
        json zone_json = {
            {"zone",
             {{"id", "2000"}, {"name", "Room ID Zone"}, {"top", 2099}, {"lifespan", 30}, {"reset_mode", "Normal"}}},
    ...
    SECTION("Parse zone with mixed room objects and ID references") {
        ...
        json zone_json = {
            {"zone",
             {{"id", "3000"}, {"name", "Mixed Room Zone"}, {"top", 3099}, {"lifespan", 30}, {"reset_mode", "Normal"}}},
            {"rooms",
             {{{"id", "3001"},
    ...
    SECTION("Parse zone with empty rooms array") {
        ...
        json zone_json = {
            {"zone",
             {{"id", "4000"}, {"name", "Empty Rooms Zone"}, {"top", 4099}, {"lifespan", 30}, {"reset_mode", "Normal"}}},
            {"rooms", {}} // Empty array
        };
    ...
    SECTION("Parse zone without rooms array") {
        ...
        json zone_json = {
            {"zone",
             {{"id", "5000"}, {"name", "No Rooms Zone"}, {"top", 5099}, {"lifespan", 30}, {"reset_mode", "Normal"}}}
...
TEST_CASE("World Parsing: Room parsing error handling", "[world][parsing][rooms][errors]") {
    ...
    SECTION("Handle room object parsing errors gracefully") {
        ...
        json zone_json = {
            {"zone",
             {{"id", "7000"}, {"name", "Room Error Zone"}, {"top", 7099}, {"lifespan", 30}, {"reset_mode", "Normal"}}},
            {"rooms",
             {{{"id", "7001"},
               {"name", "Valid Room"},
               {"description", "A valid room."},
