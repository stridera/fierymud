...
MockPlayerConnection::MockPlayerConnection(asio::io_context& io_context) : PlayerConnection(io_context) {
    // Initialize test-specific state
}

Result<void> MockPlayerConnection::initialize() {
    // Initialize parent
    auto result = PlayerConnection::initialize();
    if (!result) {
        return result;
    }
...
void MockPlayerConnection::close() {
    PlayerConnection::close();
}

Result<void> MockPlayerConnection::send(std::string_view message) {
    if (!is_connected()) {
...
Result<void> MockPlayerConnection::send_prompt(std::string_view prompt) {
    return send(prompt);
}
...
MockGameSession::MockGameSession() : io_context_(), work_guard_(asio::make_work_guard(io_context_)), io_thread_([this]() { io_context_.run(); }) {
    initialize_world_server();
    connection_ = std::make_shared<MockPlayerConnection>(io_context_);
    connection_->set_world_server(world_server_.get());
}
...
std::string MockGameSession::player_name() const {
    return player_name_;
}

ConnectionState MockGameSession::connection_state() const {
    return connection_ ? connection_->state() : ConnectionState::WaitingForName;
}
...
public:
    ~TestEnvironmentGuard() {
        UnifiedTestHarness::cleanup_test_environment();
    }
};
...
Result<void> UnifiedTestHarness::run_unit_test(std::function<void()> test) {
    ...
    try {
        ...
        return std::unexpected(Error{ErrorCode::TestFailure, 
            fmt::format("Unit test failed: {}", e.what())
        });
    }
}
...
Result<void> UnifiedTestHarness::run_integration_test(std::function<void()> test) {
    ...
    try {
        ...
        return std::unexpected(Error{ErrorCode::TestFailure, 
            fmt::format("Integration test failed: {}", e.what())
        });
    }
}
...
Result<void> UnifiedTestHarness::run_session_test(std::function<void(MockGameSession&)> test) {
    ...
    try {
        ...
        return std::unexpected(Error{ErrorCode::TestFailure, 
            fmt::format("Session test failed: {}", e.what())
        });
    }
}
