...
TEST_CASE("Mobile JSON parsing with money handling", "[core][actor][mobile][json]") {
    ...
    SECTION("Parse mobile with partial money fields") {
        json mobile_json = {
            ...
            {"money", {
                {"gold", "5"}
                // Missing copper, silver, platinum
            }}
        };
...
TEST_CASE("Mobile JSON parsing error handling", "[core][actor][mobile][errors]") {
    ...
    SECTION("Enhanced error reporting for mob_flags parsing") {
        ...
        json mobile_json = {
            ...
            {"level", 1},
            {"alignment", 0},
            {"mob_flags", 123}  // Invalid type - number instead of string
        };
        
        // Should throw an exception with enhanced error reporting
        REQUIRE_THROWS_WITH(Mobile::from_json(mobile_json), 
                           Catch::Matchers::ContainsSubstring("mob_flags"));
    }
    ...
    SECTION("Enhanced error reporting for effect_flags parsing") {
        json mobile_json = {
            ...
            {"level", 1},
            {"alignment", 0},
            {"effect_flags", 456}  // Invalid type - number instead of string
        };
        
        // Should throw an exception with enhanced error reporting
        REQUIRE_THROWS_WITH(Mobile::from_json(mobile_json), 
                           Catch::Matchers::ContainsSubstring("effect_flags"));
    }
}
