...
from .flags import DAMAGE_TYPES, LIQUIDS, SPELLS, WEAR_FLAGS
from .flags import EXTRA_OBJ_FLAGS, OBJECT_TYPES, EFFECTS, AFFECTS
...
class Object(MudObject):
    ...
    def parse(self, data):
        ...
        item_types = OBJECT_TYPES[int(type_flag)]
        ...
        self.stats['extra_flags'] = extra_flags_text
        ...
        while True:
            ...
            if line.startswith('E'):
                ...
                if "affects" not in self.stats:
                    self.stats["affects"] = []
                self.stats["affects"].append({'location': AFFECTS[int(location)], 'modifier': int(modifier)})
    ...
    def obj_val(itype: str, *args):
        ...
        match itype:
            ...
            case "CONTAINER":
                flags = Object.build_flags(args[1], ['Closeable', 'Pickproof', 'Closed', 'Locked'])
                results = {
                    'Capacity': args[0],
                    'flags': flags,
                    'Key': args[2],
            ...
            case _:
                print("Unknown type " + itype, "Flags:", args)
                results = {}
        return results
