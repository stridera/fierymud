        ...
        try:
            for line in mudfile:
                if not got_version:
                    got_version = True
                    if line != "1":
                        raise Exception(f"Unsupported version: {line}")
                    continue
    ...
    def parse(self, data):
        ...
        while data:
            ...
            if line.startswith("location:"):
                ...
                while line != "~":
                    k, v = line.split()
                    self.stats["variables"][k] = v.strip()
                    line = data.pop(0).strip()
            elif line.startswith("spells:"):
                self.stats["spells"] = []
                line = data.pop(0).strip()
                while line != "~":
                    spell_num, length = line.split()
                    spell = SPELLS[int(spell_num)]
                    self.stats["spells"].append({"spell": spell, "length": length})
                    line = data.pop(0).strip()
            else:
                if ":" in line:
                    k, v = line.split(":")
