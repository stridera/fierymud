...
int parse_race(char_data *ch, char_data *vict, char *arg);
int race_by_menu(char arg);
void send_race_menu(descriptor_data *d);
int interpret_race_selection(char arg);
void init_proto_race(char_data *ch);
void init_char_race(char_data *ch);
void update_char_race(char_data *ch);
int racial_skill_proficiency(int skill, int race, int level);
void convert_race(char_data *ch, int newrace);
void apply_racial_bonus_hit_and_dam(char_data *ch);
int natural_move(char_data *ch);
void scale_attribs(char_data *ch);
