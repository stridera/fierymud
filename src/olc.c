...
ACMD(do_olc)
{
   ...
   if (!isdigit(*buf1)) {
       ...
       *             in db.c */
         ...
         sprintf(buf, "OLC: %s reverses object extra descs for %d, %s.",
                  GET_NAME(ch), number, obj_proto[real_num].short_description);
         ...
         sprintf(buf, "OLC: %s reverses object extra descs for zone %d.",
                  GET_NAME(ch), zone_table[real_num].number);
         mudlog(buf, CMP, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE);
    ...
    */
   if (subcmd == SCMD_OLC_HEDIT && GET_LEVEL(ch) < LVL_GOD &&
                   !has_olc_access(ch, HEDIT_PERMISSION)) {
   ...
   if (subcmd != SCMD_OLC_HEDIT) OLC_NUM(d) = number;
   else {
   ...
   case SCMD_OLC_SDEDIT:
      ...
      if((real_num < 0) || (real_num > TOP_SKILL) || (!str_cmp("!UNUSED!",
                        skills[real_num].name)))
         {
...
void cleanup_olc(struct descriptor_data *d, byte cleanup_type)
{
   if (d->olc) {
      ...
      if (OLC_ROOM(d)) {
          ...
          */
         switch (cleanup_type) {
         case CLEANUP_ALL:            free_room(OLC_ROOM(d)); /* fall through */
 ...
 * Fixed sdedit to accept the name of the spell instead of a number as an argument.
 * gurlaek
 *
