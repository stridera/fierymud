...
enum class AbilityType { Spell, Skill, Chant, Song };
...
Result<std::vector<AbilityClassInfo>> load_ability_classes(pqxx::work &txn, int ability_id);
 ...
 */
std::vector<AbilityRequirement> parse_ability_requirements(std::string_view json_str, int ability_id = -1);
...
enum class UserRole { Player, Immortal, Builder, HeadBuilder, Coder, God };
...
struct PlayerMailData {
    ...
    bool has_unretrieved_wealth() const {
        return !wealth_retrieved_at.has_value() &&
               (attached_copper > 0 || attached_silver > 0 || attached_gold > 0 || attached_platinum > 0);
    }

    bool has_unretrieved_object() const { return !object_retrieved_at.has_value() && attached_object_id.has_value(); }

    bool has_any_unretrieved() const { return has_unretrieved_wealth() || has_unretrieved_object(); }
};
...
Result<int> send_player_mail(pqxx::work &txn, const std::string &sender_character_id,
                             const std::string &recipient_character_id, const std::string &body,
                             int attached_copper = 0, int attached_silver = 0, int attached_gold = 0,
                             int attached_platinum = 0,
