...
const ConfigEntry* ConfigLoader::get_entry(
    ...
    auto result = ConnectionPool::instance().execute([this](pqxx::work& txn) -> Result<void> {
        try {
            ...
            return std::unexpected(
                Errors::DatabaseError(fmt::format("Failed to load config: {}", e.what())));
        }
    });
...
Result<void> ConfigLoader::reload_category(std::string_view category) {
    ...
    auto result = ConnectionPool::instance().execute(
        [this, cat = std::string(category)](pqxx::work& txn) -> Result<void> {
        try {
            // Remove existing entries for this category
            std::erase_if(entries_, [&cat](const auto& pair) {
...
std::optional<std::string> ConfigLoader::get_string(
    std::string_view category, std::string_view key) const {
...
std::optional<int> ConfigLoader::get_int(
    std::string_view category, std::string_view key) const {
    if (const auto* entry = get_entry(category, key)) {
        try {
            return std::stoi(entry->value);
        } catch (...) {
            spdlog::warn("Failed to parse int config {}.{}: '{}'",
                category, key, entry->value);
        }
    }
    return std::nullopt;
}

std::optional<double> ConfigLoader::get_float(
    std::string_view category, std::string_view key) const {
    if (const auto* entry = get_entry(category, key)) {
        try {
            return std::stod(entry->value);
        } catch (...) {
            spdlog::warn("Failed to parse float config {}.{}: '{}'",
                category, key, entry->value);
        }
    }
    return std::nullopt;
...
std::optional<bool> ConfigLoader::get_bool(
    std::string_view category, std::string_view key) const {
    if (const auto* entry = get_entry(category, key)) {
        ...
        spdlog::warn("Failed to parse bool config {}.{}: '{}'",
            category, key, val);
    }
    return std::nullopt;
}

std::optional<std::chrono::seconds> ConfigLoader::get_seconds(
    std::string_view category, std::string_view key) const {
...
std::optional<std::chrono::minutes> ConfigLoader::get_minutes(
    std::string_view category, std::string_view key) const {
    if (auto val = get_int(category, key)) {
