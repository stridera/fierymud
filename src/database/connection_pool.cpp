...
Result<void> ConnectionPool::initialize(const DatabaseConfig &config, std::size_t pool_size) {
    ...
    if (initialized_) {
        return std::unexpected(Error{ErrorCode::InvalidState, "ConnectionPool already initialized"});
    }

    if (!config.is_valid()) {
        return std::unexpected(Error{ErrorCode::InvalidArgument, "Invalid database configuration"});
    }
    ...
    for (std::size_t i = 0; i < pool_size_; ++i) {
        try {
            ...
            if (!is_connection_healthy(*conn)) {
                ...
                return std::unexpected(Error{ErrorCode::NetworkError, fmt::format("Connection {} unhealthy", i)});
            }
            ...
            return std::unexpected(
                Error{ErrorCode::NetworkError, fmt::format("Database connection failed: {}", e.what())});
        } catch (const std::exception &e) {
            logger->error("Unexpected error creating connection: {}", e.what());
            all_connections_.clear();
            available_connections_ = {};
            return std::unexpected(
                Error{ErrorCode::InternalError, fmt::format("Connection creation failed: {}", e.what())});
        }
    }
    ...
    initialized_ = true;
    logger->info("Connection pool initialized successfully: {} connections ready", available_connections_.size());
...
std::unique_ptr<pqxx::connection> ConnectionPool::acquire_connection() {
    ...
    Log::database()->trace("Acquired connection: {} available, {} active", available_connections_.size(),
...
void ConnectionPool::release_connection(std::unique_ptr<pqxx::connection> conn) {
    ...
    Log::database()->trace("Released connection: {} available, {} active", available_connections_.size(),
                           pool_size_ - available_connections_.size());
...
ConnectionPool::PoolStats ConnectionPool::get_stats() const {
    ...
    return PoolStats{.total_connections = pool_size_,
