...
Result<void> log_script_error(pqxx::work &txn, int zone_id, int trigger_id, std::string_view error_type,
                              std::string_view error_message, std::optional<int> script_line = std::nullopt,
...
Result<std::vector<ScriptErrorEntry>> get_recent_script_errors(pqxx::work &txn, int limit = 25);
