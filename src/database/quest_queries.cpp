...
QuestObjectiveType parse_objective_type(std::string_view type_str) {
    if (type_str == "KILL_MOB")
        return QuestObjectiveType::KILL_MOB;
    if (type_str == "COLLECT_ITEM")
        return QuestObjectiveType::COLLECT_ITEM;
    if (type_str == "DELIVER_ITEM")
        return QuestObjectiveType::DELIVER_ITEM;
    if (type_str == "VISIT_ROOM")
        return QuestObjectiveType::VISIT_ROOM;
    if (type_str == "TALK_TO_NPC")
        return QuestObjectiveType::TALK_TO_NPC;
    if (type_str == "USE_SKILL")
        return QuestObjectiveType::USE_SKILL;
    if (type_str == "CUSTOM_LUA")
        return QuestObjectiveType::CUSTOM_LUA;
    return QuestObjectiveType::KILL_MOB; // Default fallback
}

QuestRewardType parse_reward_type(std::string_view type_str) {
    if (type_str == "EXPERIENCE")
    ...
    if (type_str == "ABILITY")
        return QuestRewardType::ABILITY;
...
std::string_view objective_type_to_string(QuestObjectiveType type) {
    switch (type) {
    case QuestObjectiveType::KILL_MOB:
        return "KILL_MOB";
    case QuestObjectiveType::COLLECT_ITEM:
        return "COLLECT_ITEM";
    case QuestObjectiveType::DELIVER_ITEM:
        return "DELIVER_ITEM";
    case QuestObjectiveType::VISIT_ROOM:
        return "VISIT_ROOM";
    case QuestObjectiveType::TALK_TO_NPC:
        return "TALK_TO_NPC";
    case QuestObjectiveType::USE_SKILL:
        return "USE_SKILL";
    case QuestObjectiveType::CUSTOM_LUA:
        return "CUSTOM_LUA";
    }
    return "KILL_MOB";
}
...
QuestTriggerType parse_trigger_type(std::string_view type_str) {
    if (type_str == "MOB")
        return QuestTriggerType::MOB;
    if (type_str == "LEVEL")
        return QuestTriggerType::LEVEL;
    if (type_str == "ITEM")
        return QuestTriggerType::ITEM;
    if (type_str == "ROOM")
        return QuestTriggerType::ROOM;
    if (type_str == "SKILL")
        return QuestTriggerType::SKILL;
    if (type_str == "EVENT")
        return QuestTriggerType::EVENT;
    if (type_str == "AUTO")
        return QuestTriggerType::AUTO;
    if (type_str == "MANUAL")
        return QuestTriggerType::MANUAL;
...
Result<QuestData> load_quest(pqxx::work &txn, int zone_id, int quest_id) {
    ...
    try {
        ...
        if (quest_result.empty()) {
            ...
            return std::unexpected(Errors::NotFound(fmt::format("Quest {}:{}", zone_id, quest_id)));
        }
        ...
        quest.trigger_type =
            parse_trigger_type(row["trigger_type"].is_null() ? "MOB" : row["trigger_type"].as<std::string>());
        ...
        if (!row["trigger_item_zone_id"].is_null() && !row["trigger_item_id"].is_null()) {
            quest.trigger_item = EntityId(row["trigger_item_zone_id"].as<int>(), row["trigger_item_id"].as<int>());
        }
        if (!row["trigger_room_zone_id"].is_null() && !row["trigger_room_id"].is_null()) {
            quest.trigger_room = EntityId(row["trigger_room_zone_id"].as<int>(), row["trigger_room_id"].as<int>());
        }
        ...
        for (const auto &phase_row : phases_result) {
            ...
            for (const auto &obj_row : objectives_result) {
                ...
                obj.internal_note =
                    obj_row["internal_note"].is_null() ? "" : obj_row["internal_note"].as<std::string>();
                ...
                if (!obj_row["target_mob_zone_id"].is_null() && !obj_row["target_mob_id"].is_null()) {
                    obj.target_mob =
                        EntityId(obj_row["target_mob_zone_id"].as<int>(), obj_row["target_mob_id"].as<int>());
                }
                if (!obj_row["target_object_zone_id"].is_null() && !obj_row["target_object_id"].is_null()) {
                    obj.target_object =
                        EntityId(obj_row["target_object_zone_id"].as<int>(), obj_row["target_object_id"].as<int>());
                }
                if (!obj_row["target_room_zone_id"].is_null() && !obj_row["target_room_id"].is_null()) {
                    obj.target_room =
                        EntityId(obj_row["target_room_zone_id"].as<int>(), obj_row["target_room_id"].as<int>());
                }
                ...
                if (!obj_row["deliver_to_mob_zone_id"].is_null() && !obj_row["deliver_to_mob_id"].is_null()) {
                    obj.deliver_to_mob =
                        EntityId(obj_row["deliver_to_mob_zone_id"].as<int>(), obj_row["deliver_to_mob_id"].as<int>());
                }
                obj.lua_expression =
                    obj_row["lua_expression"].is_null() ? "" : obj_row["lua_expression"].as<std::string>();
        ...
        for (const auto &reward_row : rewards_result) {
            ...
            if (!reward_row["object_zone_id"].is_null() && !reward_row["object_id"].is_null()) {
                reward.object = EntityId(reward_row["object_zone_id"].as<int>(), reward_row["object_id"].as<int>());
            }
        ...
        for (const auto &prereq_row : prereqs_result) {
            ...
            prereq.prerequisite_quest = EntityId(prereq_row["prerequisite_quest_zone_id"].as<int>(),
                                                 prereq_row["prerequisite_quest_id"].as<int>());
            quest.prerequisites.push_back(std::move(prereq));
        }

        logger->debug("Loaded quest {}:{} with {} phases, {} rewards, {} prerequisites", zone_id, quest_id,
                      quest.phases.size(), quest.rewards.size(), quest.prerequisites.size());
    ...
    try {
        ...
        if (result.empty()) {
            return std::unexpected(
                Errors::NotFound(fmt::format("Character quest progress for {}:{}", zone_id, quest_id)));
        }
    ...
    try {
        ...
        for (const auto &row : result) {
            auto progress_result =
                load_character_quest(txn, character_id, row["quest_zone_id"].as<int>(), row["quest_id"].as<int>());
            if (progress_result) {
...
Result<void> set_quest_variable(pqxx::work &txn, std::string_view character_id, int zone_id, int quest_id,
    ...
    auto logger = Log::database();
    logger->debug("Setting quest variable '{}' for character {} quest {}:{}", name, character_id, zone_id, quest_id);
    ...
    try {
        ...
        if (result.empty()) {
            return std::unexpected(Errors::NotFound(fmt::format("Character quest {}:{}", zone_id, quest_id)));
        }
...
Result<nlohmann::json> get_quest_variable(pqxx::work &txn, std::string_view character_id, int zone_id, int quest_id,
    ...
    auto logger = Log::database();
    logger->debug("Getting quest variable '{}' for character {} quest {}:{}", name, character_id, zone_id, quest_id);
    ...
    try {
        ...
        std::string vars_str = result[0]["variables"].as<std::string>();
        if (vars_str.empty() || vars_str == "{}") {
            return std::unexpected(Errors::NotFound(fmt::format("Quest variable '{}'", name)));
        }
