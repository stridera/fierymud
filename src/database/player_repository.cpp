...
Result<std::unique_ptr<Player>> PostgresPlayerRepository::load_player_by_id(std::string_view player_id) {
    return ConnectionPool::instance().execute(
        [player_id](pqxx::work &txn) { return PlayerQueries::load_player_by_id(txn, player_id); });
}

bool PostgresPlayerRepository::player_exists(std::string_view name) {
    auto result = ConnectionPool::instance().execute(
        [name](pqxx::work &txn) -> Result<bool> { return PlayerQueries::player_exists(txn, name); });
...
Result<void> PostgresPlayerRepository::save_character_items(std::string_view character_id,
...
Result<void>
PostgresPlayerRepository::save_character_effects(std::string_view character_id,
...
PostgresPlayerRepository::load_character_effects(std::string_view character_id) {
...
PlayerRepositoryProvider::PlayerRepositoryProvider()
    : repository_(std::make_unique<PostgresPlayerRepository>()), is_mock_(false) {}

IPlayerRepository &PlayerRepositoryProvider::get() { return *repository_; }
