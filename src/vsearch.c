...
bool parse_vlist_args(struct char_data *ch, char *argument, int *first, int *last) {
 ...
 * by parse_vlist_args.
 */
bool parse_vsearch_args(struct char_data *ch, char *argument, int subcmd, int *mode, const struct vsearch_type *modes,
    ...
    *mode = modes[type].type;
    ...
    case BOOLEAN:
        argument = any_one_arg(argument, arg);
        if (!str_cmp(arg, "1") || is_abbrev(arg, "true") || is_abbrev(arg, "yes") || is_abbrev(arg, "one") ||
            is_abbrev(arg, "on"))
            *value = 1;
    ...
    case INTEGER:
        ...
        *value = atoi(arg);
        if (*value || *arg == '0') {
            ...
            *compare = LE;
        else if (!str_cmp(arg, "ne") || is_abbrev(arg, "not") || !str_cmp(arg, "!=") || !str_cmp(arg, "<>"))
            *compare = NE;
...
ACMD(do_ssearch) {
    ...
    for (nr = 0; nr < top_shop && (SHOP_NUM(nr) <= last); ++nr) {
        ...
        case 3:
            match =
                (SHOP_KEEPER(nr) >= 0 && numeric_compare(mob_index[SHOP_KEEPER(nr)].virtual, value, bound, compare));
            break;
        case 4:
        ...
        case 10:
            for (temp = 0; SHOP_PRODUCT(nr, temp) != NOTHING; ++temp)
                if (numeric_compare(obj_index[SHOP_PRODUCT(nr, temp)].virtual, value, bound, compare))
                    match = TRUE;
            break;
        case 11:
    ...
    for (nr = 0; nr <= top_of_zone_table; ++nr) {
        ...
        for (cnr = 0; zone_table[nr].cmd[cnr].command != 'S'; ++cnr) {
            ...
            case 0:
                ...
                break;
            case 1:
                match = (com->command == 'M' && numeric_compare(mob_index[com->arg1].virtual, value, bound, compare));
                break;
            case 2:
                switch (com->command) {
                case 'O':
                ...
                case 'G':
                case 'E':
                    match = numeric_compare(obj_index[com->arg1].virtual, value, bound, compare);
                    break;
                }
                break;
            case 3:
                match = (com->command == 'O' && numeric_compare(obj_index[com->arg1].virtual, value, bound, compare));
                break;
            case 4:
                match = (com->command == 'P' && numeric_compare(obj_index[com->arg1].virtual, value, bound, compare));
                break;
            case 5:
                match = (com->command == 'P' && numeric_compare(obj_index[com->arg3].virtual, value, bound, compare));
                break;
            case 6:
                match = (com->command == 'G' && numeric_compare(obj_index[com->arg1].virtual, value, bound, compare));
                break;
            case 7:
                match = (com->command == 'G' && cmd_mob != NOWHERE && cmd_mob <= top_of_mobt &&
                         numeric_compare(mob_index[cmd_mob].virtual, value, bound, compare));
                break;
            case 8:
                match = (com->command == 'E' && numeric_compare(obj_index[com->arg1].virtual, value, bound, compare));
                break;
            case 9:
                ...
                break;
            case 10:
                match = (com->command == 'R' && numeric_compare(obj_index[com->arg2].virtual, value, bound, compare));
                break;
            case 11:
                if (com->command != 'D')
                    break;
            ...
            if (match) {
                ...
                case 'M':
                    sprintf(vbuf + vbuflen, "Load mob %s (%s%d%s), Max: %d\r\n",
                            mob_proto[com->arg1].player.short_descr, grn, mob_index[com->arg1].virtual, nrm, com->arg2);
                    break;
                case 'O':
                    sprintf(vbuf + vbuflen, "Load obj %s (%s%d%s), Max: %d\r\n", obj_proto[com->arg1].short_description,
                            grn, obj_index[com->arg1].virtual, nrm, com->arg2);
                    break;
                case 'G':
                    sprintf(vbuf + vbuflen, "Give %s (%s%d%s) to %s (%s%d%s), Max: %d\r\n",
                            obj_proto[com->arg1].short_description, grn, obj_index[com->arg1].virtual, nrm,
                            cmd_mob != NOWHERE && cmd_mob < top_of_mobt ? mob_proto[cmd_mob].player.short_descr
                                                                        : "a mob",
                            grn, cmd_mob != NOWHERE && cmd_mob < top_of_mobt ? mob_index[cmd_mob].virtual : -1, nrm,
                            com->arg2);
                    break;
                case 'E':
                ...
                case 'P':
                    sprintf(vbuf + vbuflen, "Put %s (%s%d%s) in %s (%s%d%s), Max: %d\r\n",
                            obj_proto[com->arg1].short_description, grn, obj_index[com->arg1].virtual, nrm,
                            obj_proto[com->arg3].short_description, grn, obj_index[com->arg3].virtual, nrm, com->arg2);
                    break;
                case 'R':
