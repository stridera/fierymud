/***************************************************************************
 * $Id: vsearch.c,v 1.70 2009/03/17 07:59:42 jps Exp $
 ***************************************************************************/
/**************************************************************************
...
void list_shops(struct char_data *ch, int first, int last);
...
#define UNRECOGNIZED_ARG(arg)	{ \
				  cprintf(ch, "Unrecognized vsearch mode: %s\r\n", (arg)); \
				  return; \
				}
...
static void page_char_to_char(struct char_data *mob, struct char_data *ch, int nfound)
{
   int count;
  ...
  pprintf(ch, "%4d. [%s%5d%s] " ELLIPSIS_FMT " %s %3d  %-*s  %s%c%-8s&0  %s%c%s&0\r\n",
      ...
      ELLIPSIS_STR(mob->player.short_descr, 39),
      CLASS_ABBR(mob), GET_LEVEL(mob), 9 + count, RACE_ABBR(mob),
...
bool numeric_compare(int value, int match, int bound, int mode) {
  switch (mode) {
    ...
    case EQ:   return (value == match);
    case GT:   return (value > match);
    case LT:   return (value < match);
    case GE:   return (value >= match);
    case LE:   return (value <= match);
    case NE:   return (value != match);
    case BT:   return (value >= match && value <= bound);
  }
  return FALSE;
}
 ...
 */
bool check_trigger_vnums(struct trig_proto_list *list, int trig_vnum, int bound, int mode) {
  while (list) {
 ...
 */
bool check_extra_descs(struct extra_descr_data *ed, char *string) {
  if (!ed)
    return FALSE;
...
bool parse_vlist_args(struct char_data *ch, char *argument,
                      int *first, int *last) {
 ...
 * by parse_vlist_args.
 */
bool parse_vsearch_args(struct char_data *ch, char *argument,
                        int subcmd, int *mode,
                        const struct vsearch_type *modes,
                        int *value, int *bound, char **string,
                        int *compare, flagvector *flags, int *first,
                        int *last) {
  int type, temp;

  if (!mode || !modes || !value || !bound || !string || !compare ||
      !flags || !first || !last) {
    if (ch)
  ...
  *mode = modes[type].type;
  ...
  switch (modes[type].data) {
    ...
    case BOOLEAN:
      argument = any_one_arg(argument, arg);
      if (!str_cmp(arg, "1") || is_abbrev(arg, "true") ||
          is_abbrev(arg, "yes") || is_abbrev(arg, "one") ||
          is_abbrev(arg, "on"))
        *value = 1;
      else if (is_abbrev(arg, "00000000") || is_abbrev(arg, "false") ||
          is_abbrev(arg, "no") || is_abbrev(arg, "zero") ||
          is_abbrev(arg, "off"))
        *value = 0;
      else {
    ...
    case TYPE:
      argument = delimited_arg(argument, arg, '\'');
      if (!*arg) {
        sprintf(buf, "You can search for the following %s types:",
                modes[type].name);
    ...
    case INTEGER:
      ...
      *value = atoi(arg);
      if (*value || *arg == '0') {
        ...
        *compare = EQ;
      else if (!str_cmp(arg, "gt") || !str_cmp(arg, ">"))
        *compare = GT;
      else if (!str_cmp(arg, "lt") || !str_cmp(arg, "<"))
        *compare = LT;
      else if (!str_cmp(arg, "ge") || !str_cmp(arg, ">="))
        *compare = GE;
      else if (!str_cmp(arg, "le") || !str_cmp(arg, "<="))
        *compare = LE;
      else if (!str_cmp(arg, "ne") || is_abbrev(arg, "not") || 
               !str_cmp(arg, "!=") || !str_cmp(arg, "<>"))
        *compare = NE;
      else if (!str_cmp(arg, "bt") || is_abbrev(arg, "between")) {
        *compare = BT;
        argument = any_one_arg(argument, arg);
    ...
    case FLAGS:
      ...
      if (!*arg) {
        sprintf(buf, "You can search for the following %s flags:",
                  modes[type].name);
        for (temp = 0; *modes[type].lookup[temp] != '\n'; ++temp) {
          if (!(temp % 4))
            strcat(buf, "\r\n");
    ...
    case TRGTYPE:
      ...
      for (temp = 0; *arg; any_one_arg(argument, arg)) {
        if (*value == MOB_TRIGGER &&
            (*bound = search_block(arg, trig_types, FALSE)) >= 0)
          *flags |= (1 << *bound);
        else if (*value == OBJ_TRIGGER &&
                 (*bound = search_block(arg, otrig_types, FALSE)) >= 0)
          *flags |= (1 << *bound);
        else if (*value == WLD_TRIGGER &&
                 (*bound = search_block(arg, wtrig_types, FALSE)) >= 0)
          *flags |= (1 << *bound);
        else if (is_abbrev(arg, "from")) {
          if (!temp) {
    ...
    case DAMTYPE:
      ...
      if (!*arg) {
        ...
        str_cat(buf, "You can search for the following damage types:");
        for (temp = 0; temp < NUM_DAMTYPES; ++temp) {
          if (!(temp % 4))
            str_cat(buf, "\r\n");
          str_catf(buf, "%-16s", damtypes[temp].name);
        }
        str_cat(buf, "\r\n");
        cprintf(ch, "%s", buf);
        return FALSE;
    ...
    case LIQUID:
      ...
      if (!*arg) {
        str_start(buf, sizeof(buf));
        str_cat(buf, "You can search for the following liquid types:");
        for (temp = 0; temp < NUM_LIQ_TYPES; ++temp) {
          if (!(temp % 4))
            str_cat(buf, "\r\n");
          str_catf(buf, "%-16s", LIQ_NAME(temp));
        }
        str_cat(buf, "\r\n");
        cprintf(ch, "%s", buf);
        return FALSE;
...
ACMD(do_vsearch)
{
  ...
  if (!*arg) {
    if (subcmd == SCMD_VSEARCH)
      ...
      send_to_char("Usage: vnum <type> <name> [[from] <start_vnum> [to] <end_vnum>]\r\n", ch);
    return;
  }
...
ACMD(do_msearch)
{
  ...
  if (subcmd == SCMD_VLIST) {
    ...
    if (subcmd == SCMD_VNUM && !*arg) {
      send_to_char("Usage: mnum <name> [[from] <start_vnum> [to] <end_vnum>]\r\n", ch);
      return;
    }
    if (!parse_vsearch_args(ch, argument, subcmd, &mode, vsearch_mobile_modes,
                            &value, &bound, &string, &compare, &flags[0],
                            &first, &last))
      return;
    ...
    if (mode == 18) {
      ...
      if (!found) {
        send_to_char("That is not a valid attack type.\r\n", ch);
        return;
      }
    }
  }
    ...
    switch (mode) {
      ...
      case 11:
        ...
        if (!mob->points.max_hit)
          match = numeric_compare(mob->points.hit * (mob->points.mana + 1) / 2 +
                                  GET_EX_MAIN_HP(mob) + mob->points.move,
                                  value, bound, compare);
        else
          match = numeric_compare((mob->points.hit + mob->points.mana) / 2,
                                  value, bound, compare);
        break;
      case 12:
      ...
      case 14:
        ...
        break;
      case 15:
        match = numeric_compare(mob->mob_specials.damnodice *
                                (mob->mob_specials.damsizedice + 1) / 2,
                                value, bound, compare);
        break;
      case 16:
...
ACMD(do_osearch)
{
  int mode, value, found, compare, bound, nr, first, last, temp,
      temp_found, type = -1;
  char *string;
  ...
  if (subcmd == SCMD_VLIST) {
    ...
    any_one_arg(argument, arg);
    if (subcmd == SCMD_VNUM && !*arg) {
      send_to_char("Usage: onum <name> [[from] <start_vnum> [to] <end_vnum>]\r\n", ch);
      return;
    }
    if (!parse_vsearch_args(ch, argument, subcmd, &mode, vsearch_object_modes,
                            &value, &bound, &string, &compare, &flags[0],
                            &first, &last))
      return;
        ...
        if (subcmd == SCMD_VITEM) {
          for (nr = 0; nr < MAX_SEARCH_ITEM_TYPES; ++nr)
            if (value_types[type].item_types[nr] == cmd)
              found = 1;
          if (!found) {
            send_to_char("Invalid search mode for the given object type.\r\n", ch);
            return;
          }
        }
        break;
  ...
  for (found = nr = 0; nr <= top_of_objt && (obj_index[nr].virtual <= last); ++nr) {
    ...
    if (subcmd == SCMD_VWEAR && !CAN_WEAR(obj, cmd))
      continue;
    if (type >= 0) {
      for (temp_found = temp = 0; temp < MAX_SEARCH_ITEM_TYPES &&
           value_types[type].item_types[temp]; ++temp)
...
/* EMPTY can be reused */
      ...
      case 14:
        for (temp = 0; temp < MAX_OBJ_APPLIES; ++temp) {
          if (obj->applies[temp].location != flags[0])
            continue;
          if (!numeric_compare(obj->applies[temp].modifier, value,
                               bound, compare))
            continue;
          match = TRUE;
          break;
      ...
      case 20:
        if (GET_OBJ_TYPE(obj) == ITEM_STAFF || GET_OBJ_TYPE(obj) == ITEM_WAND)
          ...
          match = (GET_OBJ_VAL(obj, VAL_SCROLL_SPELL_1) == value ||
                   GET_OBJ_VAL(obj, VAL_SCROLL_SPELL_2) == value ||
...
#define OBJ_TITLE_LENGTH 30
    ...
    if (match) {
      ...
      sprintf(vbuf, "%4d. [%s%5d%s] " ELLIPSIS_FMT " %s%3d&0 %s%5g&0 %*s " ANRM,
          ++found, grn, obj_index[nr].virtual, nrm,
          ELLIPSIS_STR(obj->short_description, OBJ_TITLE_LENGTH),
          ...
          GET_OBJ_LEVEL(obj),
          GET_OBJ_WEIGHT(obj) > 9999 ? "&5" : "",
          GET_OBJ_WEIGHT(obj) > 9999 ? 9999 : GET_OBJ_WEIGHT(obj),
          ...
          switch (GET_OBJ_TYPE(obj)) {
            ...
            case ITEM_POTION:
              if (GET_OBJ_VAL(obj, VAL_SCROLL_SPELL_1) == GET_OBJ_VAL(obj, VAL_SCROLL_SPELL_2) &&
                ...
                sprintf(vbuf, "%s%s%s%s x 3", vbuf,
                                   cyn,
                                   skill_name(GET_OBJ_VAL(obj, VAL_SCROLL_SPELL_1)),
                                   nrm);
              }
              else {
                for (temp = 1, temp_found = 0; temp <= 3; ++temp)
                  if (GET_OBJ_VAL(obj, temp) > 0)
                    sprintf(vbuf, "%s%s%s%s%s", vbuf,
                                       temp_found++ ? ", " : "", cyn,
                                       skill_name(GET_OBJ_VAL(obj, temp)),
                                       nrm);
              }
              break;
            case ITEM_WAND:
            case ITEM_STAFF:
              sprintf(vbuf, "%s%2d/%2d %s%s%s", vbuf,
                                 GET_OBJ_VAL(obj, VAL_WAND_CHARGES_LEFT),
                                 GET_OBJ_VAL(obj, VAL_WAND_MAX_CHARGES),
                                 cyn, skill_name(GET_OBJ_VAL(obj, VAL_WAND_SPELL)), nrm);
              break;
            case ITEM_WEAPON:
              sprintf(vbuf, "%s%3dD%-2d avg %.1f", vbuf,
                                 GET_OBJ_VAL(obj, VAL_WEAPON_DICE_NUM),
                                 GET_OBJ_VAL(obj, VAL_WEAPON_DICE_SIZE),
                                 WEAPON_AVERAGE(obj));
              break;
            case ITEM_ARMOR:
              sprintf(vbuf, "%s%2dac", vbuf,
                                 GET_OBJ_VAL(obj, VAL_ARMOR_AC));
              break;
            case ITEM_TRAP:
              sprintf(vbuf, "%s%dhp %s", vbuf,
                                 GET_OBJ_VAL(obj, VAL_TRAP_HITPOINTS),
                                 skill_name(GET_OBJ_VAL(obj, VAL_TRAP_SPELL)));
              break;
            case ITEM_CONTAINER:
              sprintf(vbuf, "%s%s%d", vbuf,
                                 subcmd == SCMD_VITEM ? "" : "size ",
                                 GET_OBJ_VAL(obj, VAL_CONTAINER_CAPACITY));
              break;
            case ITEM_DRINKCON:
            case ITEM_FOUNTAIN:
              sprintf(vbuf, "%s%2d/%2d %s%s", vbuf,
                                 GET_OBJ_VAL(obj, VAL_DRINKCON_REMAINING),
                                 GET_OBJ_VAL(obj, VAL_DRINKCON_CAPACITY),
                                 IS_POISONED(obj) ? "poison " : "",
                                 LIQ_NAME(GET_OBJ_VAL(obj, VAL_DRINKCON_LIQUID)));
              break;
            case ITEM_FOOD:
              sprintf(vbuf, "%s%d hrs%s", vbuf,
            ...
            case ITEM_MONEY:
              ...
              if (GET_OBJ_VAL(obj, VAL_MONEY_PLATINUM)) {
                sprintf(vbuf, "%s%d%sp%s", vbuf,
                                   GET_OBJ_VAL(obj, VAL_MONEY_PLATINUM), CLR(ch, HCYN), nrm);
                temp_found = TRUE;
              }
              if (GET_OBJ_VAL(obj, VAL_MONEY_GOLD))
                sprintf(vbuf, "%s%s%d%sg%s", vbuf,
                                   temp_found++ ? " " : "",
                                   GET_OBJ_VAL(obj, VAL_MONEY_GOLD), CLR(ch, HYEL), nrm);
              if (GET_OBJ_VAL(obj, VAL_MONEY_SILVER))
                sprintf(vbuf, "%s%s%ds", vbuf,
                                   temp_found++ ? " " : "",
                                   GET_OBJ_VAL(obj, VAL_MONEY_SILVER));
              if (GET_OBJ_VAL(obj, VAL_MONEY_COPPER))
                sprintf(vbuf, "%s%s%d%sc%s", vbuf,
                                   temp_found++ ? " " : "",
                                   GET_OBJ_VAL(obj, VAL_MONEY_COPPER), yel, nrm);
              break;
            case ITEM_PORTAL:
              temp = real_room(GET_OBJ_VAL(obj, VAL_PORTAL_DESTINATION));
              sprintf(vbuf, "%s%s[%s%5d%s] %.18s", vbuf,
                                 subcmd == SCMD_VITEM ? "" : "to room ",
                                 grn, GET_OBJ_VAL(obj, VAL_PORTAL_DESTINATION), nrm,
                                 temp == NOWHERE ? "NOWHERE" :
                                 world[temp].name);
              break;
          }
      }
...
ACMD(do_vitem)
{
  ...
  strcpy(buf, "Usage: vitem <type> [<field> <query>] [[from] <start_vnum> [to] [<end_vnum>]]\r\n"
              "Possible types are:");
...
ACMD(do_vwear)
{
  ...
  strcpy(buf, "Usage: vwear <position> [<field> <query>] [[from] <start_vnum> [to] [<end_vnum>]]\r\n"
...
ACMD(do_rsearch)
{
  int mode, value, found = 0, compare, bound, nr, first, last, count, length;
  char *string;
  flagvector flags[4];
  bool match;
  ...
  if (subcmd == SCMD_VLIST) {
    ...
    any_one_arg(argument, arg);
    if (subcmd == SCMD_VNUM && !*arg) {
      send_to_char("Usage: rnum <name> [[from] <start_vnum> [to] <end_vnum>]\r\n", ch);
      return;
    }
    if (!parse_vsearch_args(ch, argument, subcmd, &mode, vsearch_room_modes,
                            &value, &bound, &string, &compare, &flags[0],
                            &first, &last))
      return;
  }
  ...
  for (nr = 0; nr <= top_of_world && (world[nr].vnum <= last); ++nr) {
    ...
    if (match) {
      if (!found) {
        pprintf(ch, "Index VNum    Title                              Sector       Indoors Lit Exits\r\n");
        pprintf(ch, "----- ------- ---------------------------------- ------------ ------- --- -------\r\n");
      }
#define MARK_EXIT(r,d) ( \
      ...
      EXIT_IS_DOOR((r).exits[d]) ? "&0&3" : "&0&2")
#define ROOM_TITLE_LENGTH 34

      count = count_color_chars(world[nr].name);
      length = strlen(world[nr].name) - count;
      pprintf(ch, "%4d. [%s%5d%s] " ELLIPSIS_FMT " %s%-10.10s&0 %d %s%s %s%s%s%s%s%s%s%s%s%s%s%s&0\r\n",
            ++found, grn, world[nr].vnum, nrm,
            ELLIPSIS_STR(world[nr].name, ROOM_TITLE_LENGTH),
            sectors[world[nr].sector_type].color,
            sectors[world[nr].sector_type].name,
            sectors[world[nr].sector_type].mv,
            ROOM_FLAGGED(nr, ROOM_INDOORS) ? "&3indoors&0 " : "        ",
            ROOM_FLAGGED(nr, ROOM_ALWAYSLIT) ||
                  world[nr].sector_type == SECT_CITY ? "&3&blit&0" : "&9&bno &0",
            MARK_EXIT(world[nr], NORTH), capdirs[NORTH],
            MARK_EXIT(world[nr], SOUTH), capdirs[SOUTH],
            MARK_EXIT(world[nr], EAST), capdirs[EAST],
            MARK_EXIT(world[nr], WEST), capdirs[WEST],
            MARK_EXIT(world[nr], UP), capdirs[UP],
            MARK_EXIT(world[nr], DOWN), capdirs[DOWN]);
    }
  }
  if (found)
    start_paging(ch);
...
ACMD(do_esearch)
{
  ...
  if (subcmd == SCMD_VLIST) {
    ...
    any_one_arg(argument, arg);
    if (subcmd == SCMD_VNUM && !*arg) {
      send_to_char("Usage: enum <name> [[from] <start_vnum> [to] <end_vnum>]\r\n", ch);
      return;
    }
    if (!parse_vsearch_args(ch, argument, subcmd, &mode, vsearch_exit_modes,
                            &value, &bound, &string, &compare, &flags[0],
                            &first, &last))
      return;
  }
  ...
  for (nr = 0; nr <= top_of_world && (world[nr].vnum <= last); ++nr) {
    ...
    for (dir = 0; dir < NUM_OF_DIRS; ++dir) {
      ...
      switch (mode) {
        ...
        case 1:
          match = (exit->keyword &&
                   string_find(string, exit->keyword, compare));
          break;
        case 2:
          match = (exit->general_description &&
                   string_find(string, exit->general_description, compare));
          break;
        case 3:
        ...
        case 4:
          ...
          break;
        case 5:
          match = (exit->to_room != NOWHERE &&
                   numeric_compare(world[exit->to_room].vnum, value, bound, compare));
          break;
      }
      if (match) {
        if (!found) {
          pprintf(ch, "Index Dir      RoomNum Room Title           Exit Name/Key/Bits\r\n");
          pprintf(ch, "----- -----    ---------------------------- -----------------------\r\n");
        }
        if (!exit->keyword)
          *buf = '\0';
        ...
        pprintf(ch,
            "%4d. %s%c%-4s%s at [%s%5d%s] %-20.20s [%s]\r\n",
            ++found, yel, UPPER(*dirs[dir]),
            dirs[dir] + 1, nrm,
            grn, world[nr].vnum, nrm,
            world[nr].name, buf);
      }
    }
  }
...
const struct vsearch_type vsearch_shop_modes[] = {
  ...
  {2, "room", INTEGER},
  {3, "keeper", INTEGER},
  {4, "bits", FLAGS, shop_bits},
  {5, "tradeswith", FLAGS, trade_letters},
  {6, "open1", INTEGER},
  {7, "open2", INTEGER},
  {8, "close1", INTEGER},
  {9, "close2", INTEGER},
...
ACMD(do_ssearch)
{
  ...
  if (subcmd == SCMD_VLIST) {
    ...
    any_one_arg(argument, arg);
    if (subcmd == SCMD_VNUM && !*arg) {
      send_to_char("Usage: snum <name> [[from] <start_vnum> [to] <end_vnum>]\r\n", ch);
      return;
    }
    if (!parse_vsearch_args(ch, argument, subcmd, &mode, vsearch_shop_modes,
                            &value, &bound, &string, &compare, &flags[0],
                            &first, &last))
      return;
  }
  ...
  for (nr = 0; nr < top_shop && (SHOP_NUM(nr) <= last); ++nr) {
    ...
    switch (mode) {
      ...
      case 2:
        ...
        break;
      case 3:
        match = (SHOP_KEEPER(nr) >= 0 &&
                 numeric_compare(mob_index[SHOP_KEEPER(nr)].virtual,
                                 value, bound, compare));
        break;
      case 4:
      ...
      case 10:
        for (temp = 0; SHOP_PRODUCT(nr, temp) != NOTHING; ++temp)
          if (numeric_compare(obj_index[SHOP_PRODUCT(nr, temp)].virtual,
                              value, bound, compare))
            match = TRUE;
        break;
      case 11:
      ...
      case 12:
        for (temp = 0; SHOP_BUYTYPE(nr, temp) != NOTHING; ++temp)
          if (SHOP_BUYWORD(nr, temp) &&
              string_find(string, SHOP_BUYWORD(nr, temp), compare))
            match = TRUE;
        break;
    }
    if (match) {
      ...
      pprintf(ch, "%3d. [%5d] (%5d) %s (%s)\r\n",
              ...
              SHOP_KEEPER(nr) == NOBODY ? "no one" :
              mob_proto[SHOP_KEEPER(nr)].player.short_descr);
    }
  }
...
char *t_listdisplay(int nr, int index)
{
  ...
  snprintf(tbuf, MAX_INPUT_LENGTH, "%4d. [%s%5d%s] %-40.40s %s\r\n",
      index, grn, trig_index[nr]->virtual, nrm,
...
ACMD(do_tsearch)
{
  ...
  if (subcmd == SCMD_VLIST) {
    ...
    any_one_arg(argument, arg);
    if (subcmd == SCMD_VNUM && !*arg) {
      send_to_char("Usage: tnum <name> [[from] <start_vnum> [to] <end_vnum>]\r\n", ch);
      return;
    }
    if (!parse_vsearch_args(ch, argument, subcmd, &mode, vsearch_trigger_modes,
                            &value, &bound, &string, &compare, &flags[0],
                            &first, &last))
      return;
  ...
  for (nr = 0; nr < top_of_trigt && trig_index[nr]->virtual <= last; ++nr) {
    ...
    switch (mode) {
      ...
      case 2:
        match = (trig->attach_type == value &&
                 (IS_SET(GET_TRIG_TYPE(trig), flags[0]) == flags[0]));
        break;
      case 3:
    ...
    if (match) {
      if (!found) {
         pprintf(ch,
               "Index  VNum   Trigger Name                             Trigger Type \r\n");
         pprintf(ch,
               "----- ------- ---------------------------------------- ------------------\r\n");
      }
...
ACMD(do_zsearch)
{
  ...
  if (subcmd == SCMD_VLIST) {
    ...
    any_one_arg(argument, arg);
    if (subcmd == SCMD_VNUM && !*arg) {
      send_to_char("Usage: znum <name> [[from] <start_vnum> [to] <end_vnum>]\r\n", ch);
      return;
    }
    if (!parse_vsearch_args(ch, argument, subcmd, &mode, vsearch_zone_modes,
                            &value, &bound, &string, &compare, &flags[0],
                            &first, &last))
      return;
  }
  ...
  for (nr = 0; nr <= top_of_zone_table &&
               (zone_table[nr].number <= last); ++nr) {
    ...
    if (match) {
      if (!found) {
        pprintf(ch, "Num Name                           Age Reset  Freq Factor Top VNum\r\n");
        pprintf(ch,  "--- ------------------------------ --- ----------- ------ --------\r\n");
      }
      ++found;
      sprinttype(zone->reset_mode, zone_reset_modes, buf);
      pprintf(ch,
                    "%3d %-30.30s %3d %-6s  %3d %6d    %5d\r\n",
                    zone->number, zone->name,
                    zone->age, buf, zone->lifespan,
                    zone->zone_factor, zone->top);
    }
  }
  if (found)
    start_paging(ch);
...
const struct vsearch_type vsearch_zone_command_modes[] = {
  {1, "mobile", INTEGER},
  {2, "object", INTEGER},
...
{
  int mode, value, found = 0, compare, bound, nr, first, last,
      vbuflen, cnr, cmd_room = NOWHERE, cmd_mob = NOBODY;
  char *string;
  flagvector flags[4];
  bool match;
  ...
  else if (!parse_vsearch_args(ch, argument, subcmd, &mode,
                               vsearch_zone_command_modes, &value,
                               &bound, &string, &compare, &flags[0],
                               &first, &last))
    return;
  ...
  for (nr = 0; nr <= top_of_zone_table; ++nr) {
    ...
    for (cnr = 0; zone_table[nr].cmd[cnr].command != 'S'; ++cnr) {
      ...
      if (cmd_room == NOWHERE || cmd_room > top_of_world ||
          world[cmd_room].vnum < first || world[cmd_room].vnum > last)
        continue;
      match = FALSE;
      switch (mode) {
        case 0:
        ...
        case 1:
          match = (com->command == 'M' &&
                   numeric_compare(mob_index[com->arg1].virtual, value,
                                   bound, compare));
          break;
        case 2:
          switch (com->command) {
            ...
            case 'G':
            case 'E':
              match = numeric_compare(obj_index[com->arg1].virtual,
                                      value, bound, compare);
              break;
          }
          break;
        case 3:
          match = (com->command == 'O' &&
                   numeric_compare(obj_index[com->arg1].virtual, value,
                                   bound, compare));
          break;
        case 4:
          match = (com->command == 'P' &&
        ...
        case 5:
          match = (com->command == 'P' &&
                   numeric_compare(obj_index[com->arg3].virtual, value,
                                   bound, compare));
          break;
        case 6:
          match = (com->command == 'G' &&
          ...
          break;
        case 7:
          match = (com->command == 'G' && cmd_mob != NOWHERE &&
                   cmd_mob <= top_of_mobt &&
                   numeric_compare(mob_index[cmd_mob].virtual, value,
                                   bound, compare));
          break;
        case 8:
          match = (com->command == 'E' &&
          ...
          break;
        case 9:
          match = (com->command == 'E' && cmd_mob != NOWHERE &&
                   cmd_mob <= top_of_mobt &&
                   numeric_compare(mob_index[cmd_mob].virtual, value,
                                   bound, compare));
          break;
        case 10:
          match = (com->command == 'R' &&
                   numeric_compare(obj_index[com->arg2].virtual, value,
                                   bound, compare));
          break;
        case 11:
          if (com->command != 'D')
          ...
          for (value = 0; value < 4; ++value) {
            ...
            switch ((1 << value)) {
              ...
              case DOOR_RESET_CLOSED:
                if (com->arg3 != 1 && com->arg3 != 2 &&
                    com->arg3 != 4 && com->arg3 != 5)
                  match = FALSE;
                break;
          ...
          break;
        case 12:
          match = (com->command == 'D' && com->arg2 >= 0 &&
                   com->arg2 < NUM_OF_DIRS &&
                   world[cmd_room].exits[com->arg2] &&
                   world[cmd_room].exits[com->arg2]->to_room != NOWHERE &&
                   numeric_compare(world[world[cmd_room].exits[com->arg2]->to_room].vnum,
                   value, bound, compare));
          break;
        case 13:
          switch (com->command) {
        ...
        case 14:
          match = (com->command == 'M' &&
                   numeric_compare(com->arg2, value, bound, compare));
          break;
      }
      if (match) {
        if (!found) {
        ...
        vbuflen = sprintf(vbuf, "%4d. [%s%5d%s] ", ++found,
                grn, world[cmd_room].vnum, nrm);
        switch (com->command) {
          case 'M':
            sprintf(vbuf + vbuflen,
                        "Load mob %s (%s%d%s), Max: %d\r\n",
                        mob_proto[com->arg1].player.short_descr,
                        grn, mob_index[com->arg1].virtual, nrm,
                        com->arg2);
            break;
          case 'O':
            sprintf(vbuf + vbuflen,
                        "Load obj %s (%s%d%s), Max: %d\r\n",
                        obj_proto[com->arg1].short_description,
                        grn, obj_index[com->arg1].virtual, nrm,
                        com->arg2);
            break;
          case 'G':
            sprintf(vbuf + vbuflen,
                        "Give %s (%s%d%s) to %s (%s%d%s), Max: %d\r\n",
                        obj_proto[com->arg1].short_description,
                        grn, obj_index[com->arg1].virtual, nrm,
                        cmd_mob != NOWHERE && cmd_mob < top_of_mobt ?
                        mob_proto[cmd_mob].player.short_descr : "a mob",
                        grn, cmd_mob != NOWHERE && cmd_mob < top_of_mobt ?
                        mob_index[cmd_mob].virtual : -1, nrm, com->arg2);
            break;
          case 'E':
            sprintf(vbuf + vbuflen,
                        "Equip %s (%s%d%s) to %s (%s%d%s) %s, Max: %d\r\n",
                        obj_proto[com->arg1].short_description,
                        grn, obj_index[com->arg1].virtual, nrm,
                        cmd_mob != NOWHERE && cmd_mob < top_of_mobt ?
                        mob_proto[cmd_mob].player.short_descr : "a mob",
                        grn, cmd_mob != NOWHERE && cmd_mob < top_of_mobt ?
                        mob_index[cmd_mob].virtual : -1, nrm,
                        equipment_types[com->arg3], com->arg2);
            break;
          case 'P':
            sprintf(vbuf + vbuflen,
                        "Put %s (%s%d%s) in %s (%s%d%s), Max: %d\r\n",
                        obj_proto[com->arg1].short_description,
                        grn, obj_index[com->arg1].virtual, nrm,
                        obj_proto[com->arg3].short_description,
                        grn, obj_index[com->arg3].virtual, nrm,
                        com->arg2);
            break;
          case 'R':
            sprintf(vbuf + vbuflen,
                        "Remove %s (%s%d%s) from room\r\n",
                        obj_proto[com->arg2].short_description,
                        grn, obj_index[com->arg2].virtual, nrm);
            break;
          case 'D':
            sprintf(vbuf + vbuflen,
                        "Set door %s as %s\r\n",
                        dirs[com->arg2], com->arg3 ? (com->arg3 == 1 ?
                        "closed" : (com->arg3 == 2 ? "locked" :
                        (com->arg3 == 3 ? "hidden" : (com->arg3 == 4 ?
                        "hidden/closed/locked" : "hidden/closed")))) :
                        "open");
            break;
          }
        pprintf(ch, vbuf);
...
ACMD(do_ksearch)
{
  ...
  if (subcmd != SCMD_VSEARCH) {
    ...
    return;
                               ...
                               vsearch_skill_modes, &value, &bound, 
                               &string, &compare, flags, &first, &last))
    return;
  ...
  for (nr = 0; nr <= TOP_SKILL_DEFINE; ++nr) {
    ...
    if (match) {
      if (!found) {
        pprintf(ch, "Skill/Spell/Chant          Vio  Qst  H/O  Routines     Targets      Damage\r\n"
                    "-------------------------  ---  ---  ---  -----------  -----------  --------\r\n");
      }
      ++found;
      temp = talent_type(nr);
      switch (temp) {
        case SKILL: color = AFRED; break;
        case SPELL: color = AFCYN; break;
        case CHANT: color = AFYEL; break;
        default:    color = AFWHT; break;
      }
      if (skill->routines) {
      ...
      pprintf(ch, "%s%-26.26s@0 %3s  %3s  %3s  %-12.12s %-12.12s %s%s@0\r\n",
              color, skill->name,
              temp != SKILL ? YESNO(skill->violent) : "n/a", 
              temp != SKILL ? YESNO(skill->quest) : "n/a",
              temp == SKILL ? YESNO(skill->humanoid) : "n/a",
              temp != SKILL ? buf1 : "n/a", buf2,
              VALID_DAMTYPE(skill->damage_type) ? 
                damtypes[skill->damage_type].color : "",
              VALID_DAMTYPE(skill->damage_type) ? 
                damtypes[skill->damage_type].name : "n/a");
    }
  }
...
/***************************************************************************
 * $Log: vsearch.c,v $
 * Revision 1.70  2009/03/17 07:59:42  jps
 * Moved str_str to strings.c
 *
 * Revision 1.69  2009/03/17 07:55:37  jps
 * Moved ellipsis string formatting to strings.c.
 *
 * Revision 1.68  2009/03/09 20:36:00  myc
 * Renamed all *PLAT macros to *PLATINUM.
 *
 * Revision 1.67  2009/03/09 04:33:20  jps
 * Moved direction information from structs.h, constants.h, and constants.c
 * into directions.h and directions.c.
 *
 * Revision 1.66  2009/03/08 23:34:14  jps
 * Renamed spells.[ch] to casting.
 *
 * Revision 1.65  2009/03/08 21:43:27  jps
 * Split lifeforce, composition, charsize, and damage types from chars.c
 *
 * Revision 1.64  2009/02/28 19:43:47  myc
 * Adding attacktype to msearch.
 *
 * Revision 1.63  2009/02/18 19:26:11  myc
 ...
 *
 * Revision 1.62  2009/01/17 00:28:02  myc
 * Fix possible use of uninitialized variable.
 *
 * Revision 1.61  2008/09/22 02:09:17  jps
 * Changed weight into a floating-point value. Precision is preserved to
 * the 1/100 place.
 *
 * Revision 1.60  2008/09/18 16:37:46  myc
 * Vsearch now accepts <, <=, ==, =, >, >=, !=, and <> as numeric rules.
 *
 * Revision 1.59  2008/09/11 02:50:02  jps
 * Changed skills so you have a minimum position, and fighting_ok fields.
 *
 * Revision 1.58  2008/09/09 08:23:20  jps
 * Made rsearch sectors use an initial substring match.
 *
 * Revision 1.57  2008/09/07 18:45:39  jps
 * Add weight, level, and cost to the object list printout.
 *
 * Revision 1.56  2008/09/03 17:34:08  myc
 * Moved liquid information into a def struct array.
 *
 * Revision 1.55  2008/09/01 06:42:38  jps
 * Adjust room sector spacing
 *
 * Revision 1.54  2008/09/01 06:40:19  jps
 * Add mv cost to room list.
 *
 * Revision 1.53  2008/09/01 06:05:07  jps
 * Changed spacing of rlist to fit people's windows
 *
 * Revision 1.52  2008/09/01 05:51:50  jps
 * Added the room sector type's color.
 *
 * Revision 1.51  2008/09/01 05:32:33  jps
 * Add missing color format.
 *
 * Revision 1.50  2008/09/01 05:27:36  jps
 * More detail in room listing
 *
 * Revision 1.49  2008/08/29 05:43:10  myc
 * Osearch was using wrong method to check extra flags.
 *
 * Revision 1.48  2008/08/29 04:16:26  myc
 * Fixed parse_vsearch_args; was using wrong constant.
 *
 * Revision 1.47  2008/08/26 04:39:21  jps
 * Changed IN_ZONE to IN_ZONE_RNUM or IN_ZONE_VNUM and fixed zone_printf.
 *
 * Revision 1.46  2008/08/24 03:15:57  myc
 * Fix the way damtypes are printed on ksearch and properly
 * initialize some buffers.
 *
 * Revision 1.45  2008/08/24 02:41:42  myc
 * Fix minor bug with ksearching on level/circle.
 *
 * Revision 1.44  2008/08/24 02:34:34  myc
 * Adding ksearch command for searching sKills, spells, and chants.
 *
 * Revision 1.43  2008/08/15 03:59:08  jps
 * Added pprintf for paging, and changed page_string to take a character.
 *
 * Revision 1.42  2008/08/14 15:40:44  jps
 * Converted the remainder of the searches to use the new pager.
 *
 * Revision 1.41  2008/08/14 09:45:22  jps
 * Replaced the pager.
 *
 * Revision 1.40  2008/06/19 18:53:12  myc
 * Replaced the item_types array with a typedef struct array in
 * objects.c.  Added an OBJ_TYPE search mode to compensate.
 *
 * Revision 1.39  2008/06/07 19:06:46  myc
 * Moved object-related constants and routines to objects.h.
 *
 * Revision 1.38  2008/06/05 02:07:43  myc
 * Removed cost_per_day from objects.  Changed object flags to use
 * flagvectors.
 *
 * Revision 1.37  2008/05/18 20:16:11  jps
 * Created fight.h and set dependents.
 *
 * Revision 1.36  2008/05/18 05:18:06  jps
 * Renaming room_data struct's member "number" to "vnum", cos it's
 * a virtual number.
 *
 * Revision 1.35  2008/05/17 04:32:25  jps
 * Moved exits into exits.h/exits.c and changed the name to "exit".
 *
 * Revision 1.34  2008/04/20 17:48:05  jps
 * Removing unneeded externs. Adding vsearch.h.
 *
 * Revision 1.33  2008/04/20 03:55:17  jps
 * Split out the code for printing mobs in lists so that it can be called
 * from other places.
 *
 * Revision 1.32  2008/04/19 17:57:18  myc
 * Removed unused function proto.
 *
 * Revision 1.31  2008/04/13 10:55:28  jps
 * Use actual number for size searching now that the size numbers
 * have been fixed.
 *
 * Revision 1.30  2008/04/03 02:02:05  myc
 ...
 *
 * Revision 1.28  2008/03/27 00:46:06  jps
 * Add composition and life force to mobile vsearches.
 *
 * Revision 1.27  2008/03/24 03:57:01  jps
 * Add life force and composition to mob lists.
 *
 * Revision 1.26  2008/03/10 19:55:37  jps
 ...
 *
 * Revision 1.25  2008/03/09 06:38:37  jps
 ...
 * NPCs and players.
 *
 * Revision 1.24  2008/02/11 08:50:33  jps
 * Make trigger lists given from 'stat' more succinct.
 *
 * Revision 1.23  2008/02/10 20:30:03  myc
 * Object search was showing wrong spell name for potions and scrolls.
 *
 * Revision 1.22  2008/02/07 01:46:14  myc
 * Making msearch show class, level, and race.
 *
 * Revision 1.21  2008/02/06 21:53:53  myc
 * Adding exit search command.  Making the vbuf static and twice as big.
 * Putting headers on all the list outputs, and making the output for
 * osearch, rsearch, tsearch, and zsearch more pretty.
 *
 * Revision 1.20  2008/02/05 03:07:26  myc
 * Shortening all the vsearch command function names.  Adding
 * shop search (ssearch, snum, slist) and zone command search
 * (csearch, clist).  Adding a new feature to the NUMBER domain:
 * you can specify '*' for any positive number.  This is useful
 * with csearch.  Fixed a bug in parse_vsearch_args that was
 * dereferencing a pointer incorrectly.
 *
 * Revision 1.19  2008/02/04 01:46:12  myc
 * Implementing zsearch.
 *
 * Revision 1.18  2008/02/04 00:22:05  myc
 * The vsearch commands now use delimited_arg in many places, meaning you
 * can search with single-quoted parameters (that may contain spaces).
 * Also modified flag and type searches to display the possible flags
 * and types if you don't specify a query.
 *
 * Revision 1.17  2008/01/29 21:02:31  myc
 * Removing a lot of extern declarations from code files and moving
 * them to header files, mostly db.h and constants.h.
 *
 * Revision 1.16  2008/01/27 13:43:50  jps
 * Moved race and species-related data to races.h/races.c and merged species into races.
 *
 * Revision 1.15  2008/01/27 09:46:24  jps
 * Add a "CLASS" search case since the data source for class names is
 * no longer a simple array of strings.
 *
 * Revision 1.14  2008/01/26 14:26:31  jps
 * Moved a lot of skill-related code into skills.h and skills.c.
 *
 * Revision 1.13  2008/01/13 23:23:33  myc
 * Fixing a bug with searching on 'actflags'.
 *
 * Revision 1.12  2007/12/24 17:40:17  myc
 * Oops, I broke the number domain's default behavior.
 *
 * Revision 1.11  2007/12/22 23:51:39  myc
 * New search field 'key' for objects and rooms to find containers and
 * doors that are unlocked by a given vnum (or range of vnums; it uses
 * the number domain).
 *
 * Revision 1.10  2007/12/21 07:34:59  myc
 * Fixing 'ofind apply' to work with the value 0.
 *
 * Revision 1.9  2007/12/19 20:58:59  myc
 * Fix bug for 'msearch class' where array index was off by one.
 *
 * Revision 1.8  2007/11/01 21:03:40  myc
 * Fix bug with vlist * behavior.
 *
 * Revision 1.7  2007/11/01 20:46:06  myc
 * Removing debug.
 *
 * Revision 1.6  2007/11/01 20:35:34  myc
 * Tlist not working.
 *
 * Revision 1.5  2007/11/01 20:24:40  myc
 * Formatting fix.
 *
 * Revision 1.4  2007/10/17 17:18:04  myc
 * Renamed the search_block and search_block2 functions.
 * searchblock is now case sensitive, and search_block is not.
 *
 * Revision 1.3  2007/10/13 20:15:09  myc
 * Added functions to find spell/skill/chant nums to spells.h
 *
 * Revision 1.2  2007/10/11 20:14:48  myc
 * Had to rename a constant define to alleviate a conflict with spells.h.
 *
 * Revision 1.1  2007/09/28 20:49:35  myc
 * Initial revision
 *
 *
 ***************************************************************************/
