...
#define IS_CORPSE(obj)          (GET_OBJ_TYPE(obj) == ITEM_CONTAINER && \
                                 GET_OBJ_VAL((obj), VAL_CONTAINER_CORPSE) != NOT_CORPSE)
#define IS_PLR_CORPSE(obj)      (GET_OBJ_TYPE(obj) == ITEM_CONTAINER && \
...
#define VALID_OBJ_TYPE_NUM(type) \
    ((type) >= 0 && (type) < NUM_ITEM_TYPES)
#define VALID_OBJ_TYPE(obj)     \
    VALID_OBJ_TYPE_NUM(GET_OBJ_TYPE(obj))
#define OBJ_TYPE_NAME(obj)      \
    (VALID_OBJ_TYPE(obj) ? item_types[(int) GET_OBJ_TYPE(obj)].name : "--")
#define OBJ_TYPE_DESC(obj)      \
    (VALID_OBJ_TYPE(obj) ? item_types[(int) GET_OBJ_TYPE(obj)].desc : "--")
...
#define VALID_LIQ_TYPE(type) \
    ((type) >= 0 && (type) < NUM_LIQ_TYPES)
#define VALID_OBJ_LIQ(obj) \
    (VALID_LIQ_TYPE(GET_OBJ_VAL((obj), VAL_DRINKCON_LIQUID)))
#define LIQ_NAME(liq) \
    (liquid_types[VALID_LIQ_TYPE(liq) ? liq : LIQ_WATER ].name)
#define LIQ_ALIAS(liq) \
    (liquid_types[VALID_LIQ_TYPE(liq) ? liq : LIQ_WATER ].alias)
#define LIQ_COLOR(liq) \
    (liquid_types[VALID_LIQ_TYPE(liq) ? liq : LIQ_WATER ].color_desc)
#define LIQ_COND(liq, cond) \
    (liquid_types[VALID_LIQ_TYPE(liq) ? liq : LIQ_WATER ].condition_effects[(cond) == DRUNK || (cond) == FULL || (cond) == THIRST ? (cond) : THIRST])
...
#define WEAPON_AVERAGE(obj)     (((GET_OBJ_VAL(obj, VAL_WEAPON_DICE_SIZE) + 1) / 2.0) * \
                                  GET_OBJ_VAL(obj, VAL_WEAPON_DICE_NUM))
#define IS_WEAPON_PIERCING(obj) ((GET_OBJ_VAL(obj, VAL_WEAPON_DAM_TYPE) == TYPE_PIERCE - TYPE_HIT) || \
                                 (GET_OBJ_VAL(obj, VAL_WEAPON_DAM_TYPE) == TYPE_STING - TYPE_HIT) || \
                                 (GET_OBJ_VAL(obj, VAL_WEAPON_DAM_TYPE) == TYPE_STAB - TYPE_HIT))
#define IS_WEAPON_SLASHING(obj) ((GET_OBJ_VAL(obj, VAL_WEAPON_DAM_TYPE) == TYPE_SLASH - TYPE_HIT) || \
                                 (GET_OBJ_VAL(obj, VAL_WEAPON_DAM_TYPE) == TYPE_CLAW - TYPE_HIT) || \
                                 (GET_OBJ_VAL(obj, VAL_WEAPON_DAM_TYPE) == TYPE_WHIP - TYPE_HIT))
#define IS_WEAPON_CRUSHING(obj) ((GET_OBJ_VAL(obj, VAL_WEAPON_DAM_TYPE) == TYPE_BLUDGEON - TYPE_HIT) || \
...
#define OBJ_IS_OPENABLE(obj)    (GET_OBJ_TYPE(obj) == ITEM_CONTAINER && \
                                 IS_SET(GET_OBJ_VAL(obj, VAL_CONTAINER_BITS), CONT_CLOSEABLE))
...
#define IS_POISONED(obj)        ((GET_OBJ_TYPE(obj) == ITEM_FOOD || \
                                  GET_OBJ_TYPE(obj) == ITEM_DRINKCON || \
                                  GET_OBJ_TYPE(obj) == ITEM_FOUNTAIN) ? \
                                 GET_OBJ_VAL((obj), VAL_DRINKCON_POISONED) : FALSE)

#define MONEY_VALUE(obj)        (GET_OBJ_VAL(obj, VAL_MONEY_PLATINUM) * 1000 + \
                                 GET_OBJ_VAL(obj, VAL_MONEY_GOLD) * 100 + \
                                 GET_OBJ_VAL(obj, VAL_MONEY_SILVER) * 10 + \
                                 GET_OBJ_VAL(obj, VAL_MONEY_COPPER))

#define IS_VIEWABLE_GATE(obj)   (GET_OBJ_VNUM(obj) == OBJ_VNUM_HEAVENSGATE || \
                                 GET_OBJ_VNUM(obj) == OBJ_VNUM_MOONWELL || \
 ...
 */
#define OBJ_INVIS_TO_CHAR(obj, ch) \
  (OBJ_FLAGGED((obj), ITEM_INVISIBLE) && !EFF_FLAGGED((ch), EFF_DETECT_INVIS))
#define OBJ_HIDDEN_TO_CHAR(obj, ch) \
  (GET_OBJ_HIDDENNESS(obj) > GET_PERCEPTION(ch) && (obj)->last_to_hold != (ch))
#define MORT_CAN_SEE_OBJ(ch, obj) \
  (LIGHT_OK(ch) && !OBJ_INVIS_TO_CHAR(obj, ch) && !OBJ_HIDDEN_TO_CHAR(obj, ch))
#define CAN_SEE_OBJ(ch, obj) \
  (MORT_CAN_SEE_OBJ(ch, obj) || PRF_FLAGGED((ch), PRF_HOLYLIGHT))
...
extern void liquid_to_container(struct obj_data *container, int amount,
