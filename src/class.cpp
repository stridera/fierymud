 ...
 */
int parse_class(char_data *ch, char_data *vict, char *arg) {
...
void give_newbie_eq(char_data *ch) {
    struct obj_data *obj;
    int i;
...
int return_max_skill(char_data *ch, int skill) { return level_max_skill(ch, GET_LEVEL(ch), skill); }

void init_char_class(char_data *ch) { /* Nothing much to do here. */
}

void update_char_class(char_data *ch) {
    if (!VALID_CLASS(ch)) {
        char buf[500];
...
void advance_level(char_data *ch, enum level_action action) {
...
void convert_class(char_data *ch, int newclass) {
    int skill;
