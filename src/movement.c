...
void falling_check(struct char_data *ch)
{
   if (ch &&
      GET_LEVEL(ch) < 100 &&
      !EVENT_FLAGGED(ch, EVENT_GRAVITY) &&
      IN_ROOM(ch) != NOWHERE &&
      SECT(IN_ROOM(ch)) == SECT_AIR &&
...
void mount_pos_check(struct char_data *mount)
{
   ...
   if (!ch || GET_POS(mount) >= POS_KNEELING) return;
   ...
   if ((EFF_FLAGGED(ch, EFF_FLY) || GET_LEVEL(ch) >= LVL_IMMORT) &&
      ...
      /* What we really need is a generic "got wet" function, which would take care
       * of flames and ruin people's spellbooks (KIDDING!) */
      if (EFF_FLAGGED(ch, EFF_ON_FIRE)) {
         cprintf(ch, "You fall into the water, and your flames are put out with a hiss of steam.\r\n");
         act("$n falls into the water.  $U$s flames go out with a hissing sound.",
               FALSE, ch, 0, 0, TO_ROOM);
      } else {
