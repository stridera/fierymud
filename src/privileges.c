  ...
  for (priv = 0; priv < NUM_PRV_FLAGS; ++priv)
    if (PRV_FLAGGED(vict, priv)) {
      ...
      str_catf(buf, "  FLG %c%-19s (%d)\r\n",
               UPPER(*prv_flags[priv].desc),
               prv_flags[priv].desc + 1,
...
static void do_command_grant_revoke(struct char_data *ch, struct char_data *vict, char *argument, int subcmd, int type)
{
  ...
  if (!*arg)
    ...
    for (flag = 0; flag < NUM_PRV_FLAGS; ++flag)
      cprintf(ch, "%-15s %-20s %d\r\n",
              privilege_bits[flag], prv_flags[flag].desc, 
