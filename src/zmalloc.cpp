...
#define GET_ZBUCKET(addr) (((intptr_t)(addr) >> 3) & 0xFF)
    ...
    if (what) {
        for (prev_m = nullptr, m = memlist[GET_ZBUCKET(what)]; m; prev_m = m, m = m->next) {
            if (m->addr == what) {
                ...
                if (!ret) {
                    fprintf(zfd,
                            "zrealloc: FAILED for 0x%4.4x %d bytes mallocd at %s:%d,\n"
                            "          %d bytes reallocd at %s:%d.\n",
                            (intptr_t)m->addr, m->size, m->file, m->line, len, file, line);
                    if (zmalloclogging > 1)
                        zdump(m);
                    return nullptr;
                }
                ...
                if (zmalloclogging > 2)
                    fprintf(zfd,
                            "zrealloc: 0x%4.4x %d bytes mallocd at %s:%d, %d bytes "
                            "reallocd at %s:%d.\n",
                            (intptr_t)m->addr, m->size, m->file, m->line, len, file, line);
...
void zfree(unsigned char *what, char *file, int line) {
    ...
    for (m = memlist[GET_ZBUCKET(what)]; m; m = m->next) {
        if (m->addr == what) {
            ...
            if (m->frees > 1) {
                fprintf(zfd,
                        "zfree: ERR: multiple frees! 0x%4.4x %d bytes\n"
                        "       mallocd at %s:%d, freed at %s:%d.\n",
                        (intptr_t)m->addr, m->size, m->file, m->line, file, line);
                if (zmalloclogging > 1)
                    zdump(m);
            }
            gotit++;
