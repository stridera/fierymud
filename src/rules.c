...
void init_rules() {
  struct rule_vtable *vtable_list[] = {
    &RULE_VTABLE(and),
    &RULE_VTABLE(clan),
    /* &RULE_VTABLE(command), */
    &RULE_VTABLE(level),
    &RULE_VTABLE(namelist),
    &RULE_VTABLE(not),
    &RULE_VTABLE(or),
    NULL
  };
...
BEGIN_RULE_FUNC_MATCHER(clan, clan_rule) {
  return GET_CLAN(ch) && GET_CLAN(ch)->number == rule->number &&
          !OUTRANKS(rule->rank, GET_CLAN_RANK(ch));
} END_RULE_FUNC

BEGIN_RULE_FUNC_DESTRUCTOR(clan, clan_rule) {
  free(rule);
} END_RULE_FUNC

BEGIN_RULE_FUNC_PRINT(clan, clan_rule) {
  snprintf(buf, size, "%u %u", rule->number, rule->rank);
} END_RULE_FUNC

BEGIN_RULE_FUNC_ABBR(clan, clan_rule) {
  snprintf(buf, size, "%u" FCYN "%u" ANRM, rule->number, rule->rank);
} END_RULE_FUNC

BEGIN_RULE_FUNC_VERBOSE(clan, clan_rule) {
  struct clan *clan = find_clan_by_number(rule->number);
  if (clan)
    snprintf(buf, size, "clan %s, rank %u",
             clan->abbreviation, rule->rank);
  else
    snprintf(buf, size, "unknown clan %u, rank %u",
             rule->number, rule->rank);
} END_RULE_FUNC

BEGIN_RULE_FUNC_PARSE(clan, clan_rule) {
  struct clan *clan;
...
BEGIN_RULE_FUNC_MATCHER(level, range_rule) {
  return GET_LEVEL(ch) >= rule->min &&
         GET_LEVEL(ch) <= rule->max;
} END_RULE_FUNC

BEGIN_RULE_FUNC_DESTRUCTOR(level, range_rule) {
  free(rule);
} END_RULE_FUNC

BEGIN_RULE_FUNC_PRINT(level, range_rule) {
  snprintf(buf, size, "%d %d", rule->min, rule->max);
} END_RULE_FUNC

BEGIN_RULE_FUNC_ABBR(level, range_rule) {
  snprintf(buf, size, "%d", rule->min);
} END_RULE_FUNC

BEGIN_RULE_FUNC_VERBOSE(level, range_rule) {
  snprintf(buf, size, "level%s %d - %d",
           rule->min == rule->max ? "" : "s", rule->min, rule->max);
} END_RULE_FUNC

BEGIN_RULE_FUNC_PARSE(level, range_rule) {
  int min, max;
...
BEGIN_RULE_FUNC_MATCHER(namelist, namelist_rule) {
  return isname(GET_NAME(ch), rule->namelist);
} END_RULE_FUNC

BEGIN_RULE_FUNC_DESTRUCTOR(namelist, namelist_rule) {
  free(rule->namelist);
  free(rule);
} END_RULE_FUNC

BEGIN_RULE_FUNC_PRINT(namelist, namelist_rule) {
  snprintf(buf, size, "%s", rule->namelist);
} END_RULE_FUNC

BEGIN_RULE_FUNC_ABBR(namelist, namelist_rule) {
  snprintf(buf, size, "%s", rule->namelist);
} END_RULE_FUNC

BEGIN_RULE_FUNC_VERBOSE(namelist, namelist_rule) {
  snprintf(buf, size, "names: %s", rule->namelist);
} END_RULE_FUNC

BEGIN_RULE_FUNC_PARSE(namelist, namelist_rule) {
  return make_namelist_rule(buf);
} END_RULE_FUNC

PUBLISH_RULE_VTABLE(namelist);
...
RULE_CONSTRUCTOR(make_and_rule)(rule_t *left, rule_t *right) {
  return make_binary_rule(left, right, 0);
}

RULE_CONSTRUCTOR(make_or_rule)(rule_t *left, rule_t *right) {
  return make_binary_rule(left, right, 1);
}

BEGIN_RULE_FUNC_MATCHER(and, binary_rule) {
  return rule_matches(rule->left, ch) && rule_matches(rule->right, ch);
} END_RULE_FUNC

BEGIN_RULE_FUNC_MATCHER(or, binary_rule) {
  return rule_matches(rule->left, ch) || rule_matches(rule->right, ch);
} END_RULE_FUNC

BEGIN_RULE_FUNC_DESTRUCTOR(binary, binary_rule) {
...
void print_binary_rule(char *buf, size_t size, const char *insert,
                       const binary_rule *rule, bool verbose)
{
  char *p = buf;
  size_t diff;
...
BEGIN_RULE_FUNC_PRINT(binary, binary_rule) {
  print_binary_rule(buf, size, " ", rule, false);
} END_RULE_FUNC

BEGIN_RULE_FUNC_ABBR(and, binary_rule) {
  rule = rule; /* to avoid compiler warning */
  snprintf(buf, size, "AND");
} END_RULE_FUNC

BEGIN_RULE_FUNC_ABBR(or, binary_rule) {
  rule = rule; /* to avoid compiler warning */
  snprintf(buf, size, "OR");
} END_RULE_FUNC

BEGIN_RULE_FUNC_VERBOSE(and, binary_rule) {
  print_binary_rule(buf, size, " and ", rule, true);
} END_RULE_FUNC

BEGIN_RULE_FUNC_VERBOSE(or, binary_rule) {
  print_binary_rule(buf, size, " or ", rule, true);
} END_RULE_FUNC



bool process_binary_rule(const char *buf, rule_t **left, rule_t **right) {
...
BEGIN_RULE_FUNC_PARSE(or, binary_rule) {
  rule_t *left = NULL, *right = NULL;
  if (!process_binary_rule(buf, &left, &right))
    return NULL;
...
PUBLISH_RULE_VTABLE_MANUAL(and, and, binary, binary, and, and, and);
...
BEGIN_RULE_FUNC_MATCHER(not, unary_rule) {
  return !rule_matches(rule->child, ch);
} END_RULE_FUNC

BEGIN_RULE_FUNC_DESTRUCTOR(unary, unary_rule) {
  free_rule(rule->child);
  free(rule);
} END_RULE_FUNC

BEGIN_RULE_FUNC_PRINT(unary, unary_rule) {
  sprint_rule(buf, size, rule->child);
} END_RULE_FUNC

BEGIN_RULE_FUNC_ABBR(not, unary_rule) {
  rule = rule; /* avoid compiler warning */
  snprintf(buf, size, "NOT");
} END_RULE_FUNC

BEGIN_RULE_FUNC_VERBOSE(not, unary_rule) {
  size_t diff;
  snprintf(buf, size, "not (");
  SUD(size, 3); buf += 3;
  rule_verbose(buf, size, rule->child);
  diff = strlen(buf);
  SUD(size, diff); buf += diff;
  snprintf(buf, size, ")");
} END_RULE_FUNC

BEGIN_RULE_FUNC_PARSE(not, unary_rule) {
...
PUBLISH_RULE_VTABLE_MANUAL(not, not, unary, unary, not, not, not);
