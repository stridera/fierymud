...
ASPELL(spell_phosphoric_embers)
{
   ...
   act("@yYou hurl a handful of @Yburning phosphoric embers@y at $N.@0",
            FALSE, ch, 0, victim, TO_CHAR);
   act("@y$n@y hurls a handful of @Yflaring phosphoric embers@y at $N.@0",
            FALSE, ch, 0, victim, TO_NOTVICT);
   act("@y$n@y hurls a a handful of @Yburning phosphoric embers@y on you!@0",
            FALSE, ch, 0, victim, TO_VICT);

   event_obj = construct_delayed_cast(ch, victim, SPELL_PHOSPHORIC_EMBERS,
                                      MAG_DAMAGE, 4, 4 RL_SEC, skill,
                                      savetype, FALSE);
   event_create(EVENT_SPELL, delayed_cast_event, event_obj, TRUE,
                      &(ch->events), 4 RL_SEC);
...
ASPELL(spell_create_water)
{
   ...
   if (GET_OBJ_TYPE(obj) == ITEM_DRINKCON) {
      amount = MIN(GET_OBJ_VAL(obj, VAL_DRINKCON_CAPACITY) -
            GET_OBJ_VAL(obj, VAL_DRINKCON_REMAINING), 1 + 15 * skill / 2);
      if (amount <= 0) {
...
ASPELL(spell_resurrect)
{
   ...
   for (corpse = world[ch->in_room].contents; corpse; corpse = corpse->next_content) {
      if (CAN_SEE_OBJ(ch, corpse) && isname(GET_NAME(victim), corpse->name)
         ...
         exp = 0.60 * exp;
         gain_exp(victim, exp, GAIN_IGNORE_LEVEL_BOUNDARY | 
...
ASPELL(spell_soul_tap)
{
   ...
   if (GET_LIFEFORCE(victim) == LIFE_UNDEAD ||
       GET_LIFEFORCE(victim) == LIFE_MAGIC ||
       GET_LIFEFORCE(victim) == LIFE_ELEMENTAL) {
      act("$N has no soul to tap!", FALSE, ch, 0, victim, TO_CHAR);
      return CAST_RESULT_CHARGE;
   }

   act("@LYou reach a shadowy hand into the depths of $N@L's soul...@0",
            FALSE, ch, 0, victim, TO_CHAR);
   act("@L$n@L reaches a shadowy hand into the depths of $N@L's soul...@0",
            FALSE, ch, 0, victim, TO_NOTVICT);
   act("@L$n@L reaches a shadowy hand into the depths of your soul...@0",
            FALSE, ch, 0, victim, TO_VICT);

   event_obj = construct_delayed_cast(ch, victim, SPELL_SOUL_TAP,
                                      MAG_MANUAL, 5, 5 RL_SEC, skill,
                                      savetype, TRUE);
   event_create(EVENT_SPELL, delayed_cast_event, event_obj, TRUE,
                      &(ch->events), 5 RL_SEC);
...
#define CAN_BURN(ch)	(GET_COMPOSITION(ch) == COMP_FLESH || \
                         GET_COMPOSITION(ch) == COMP_PLANT || \
                         GET_COMPOSITION(ch) == COMP_BONE)
  ...
  if (victim) {
    ...
    for (fol = ch->followers; fol; fol = fol->next)
      if (CAN_BURN(fol->follower) &&
          EFF_FLAGGED(fol->follower, EFF_CHARM) && 
  ...
  if (ch == victim) {
    act("@yYou snap a @Rconflagration @yinto existence, lighting yourself up like a torch.&0",
        FALSE, ch, 0, 0, TO_CHAR);
    act("@y$n@y snaps a @Rconflagration @yinto existence, lighting $mself up like a torch!&0",
        FALSE, ch, 0, 0, TO_ROOM);
  }
  else {
    act("@yYou snap a @Rconflagration @yinto existence, lighting $N up like a torch.&0",
        FALSE, ch, 0, victim, TO_CHAR);
    act("@y$n@y snaps a @Rconflagration @yinto existence, lighting $N up like a torch!&0",
        FALSE, ch, 0, victim, TO_ROOM);
    act("@y$n@y snaps a @Rconflagration @yinto existence, enveloping you in flames!&0",
        FALSE, ch, 0, victim, TO_VICT);
  }

  event_obj = construct_delayed_cast(EFF_FLAGGED(victim, EFF_CHARM) &&
                                     victim->master == ch ? victim : ch, 
                                     victim, SPELL_PYRE, MAG_MANUAL,
                                     4, PYRE_DELAY, skill, savetype, FALSE);
  event_create(EVENT_SPELL, delayed_cast_event, event_obj, TRUE,
               &(victim->events), PYRE_DELAY);
...
ASPELL(spell_fracture)
{
  ...
  if (victim) {
    ...
    if (!RIGID(victim) || !(IS_HUMANOID(victim) ||
        GET_RACE(victim) == RACE_ANIMAL || GET_RACE(victim) == RACE_DRAGON)) {
    ...
    for (fol = ch->followers; fol; fol = fol->next)
      if (RIGID(fol->follower) && (IS_HUMANOID(fol->follower) ||
           GET_RACE(fol->follower) == RACE_ANIMAL ||
  ...
  ch = EFF_FLAGGED(victim, EFF_CHARM) && victim->master == ch ?
       victim : ch;
