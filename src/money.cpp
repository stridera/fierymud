...
int parse_coin_type(std::string_view name) {
...
std::string statemoney(const Money coins) {
...
std::string briefmoney(int spaces, int amt) {
    bool topset = false;
    bool lowset = false;
...
ObjData *create_money(const Money coins) {
    ...
    if (amount == 1) {
        if (coins[PLATINUM])
            ...
            which = COPPER;
        obj->name = fmt::format("{} coin", COIN_NAME(which));
        obj->short_description = fmt::format("a {}", obj->name);
        obj->description = fmt::format("A single {} is lying here.", obj->name);
        obj->ex_description->keyword = obj->name;
        obj->ex_description->description = fmt::format("A shiny {}!", obj->name);
    } else {
        auto guess = [](int amount, int scale) -> int {
            return ((amount / scale) + random_number(0, amount / scale)) * scale;
        };
        obj->name = money_desc(amount);
        obj->short_description = fmt::format("a {}", obj->name);
        obj->ex_description->keyword = obj->name;
        obj->description = fmt::format("{} is lying here.", capitalize_first(obj->short_description));
        if (amount < 10)
            obj->ex_description->description = fmt::format("There are {} coins.", amount);
        else if (amount < 100)
            obj->ex_description->description = fmt::format("There are about {} coins.", guess(amount, 10));
        else if (amount < 1000)
            obj->ex_description->description = fmt::format("It looks to be about {} coins.", guess(amount, 100));
        else if (amount < 100000)
            obj->ex_description->description = fmt::format("You guess there are maybe {} coins.", guess(amount, 1000));
        else
            obj->ex_description->description = "There are a LOT of coins.";
    }
