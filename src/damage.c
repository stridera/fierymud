...
int parse_damtype(struct char_data *ch, char *arg) {
    return parse_obj_name(ch, arg, "damage type", NUM_DAMTYPES, damtypes, sizeof(struct damdef));
}
...
bool damage_evasion(struct char_data *ch, struct char_data *attacker, struct obj_data *weapon, int dtype) {
    int s;
