...
double evaluate(evaluation_param *eval, double x) {
    ...
    while (eval[pos].type != DONE && x >= eval[pos].x)
        ++pos;
    if (pos > 0)
        --pos;

    if (eval[pos].type == LINEAR)
        return linear_evaluation(eval[pos].x, eval[pos].y, eval[pos + 1].x, eval[pos + 1].y, x);
    else if (eval[pos].type == SQUARED)
        return squared_evaluation(eval[pos].x, eval[pos].y, eval[pos + 1].x, eval[pos + 1].y, x);
    else if (eval[pos].type == LOG)
        return logarithmic_evaluation(eval[pos].x, eval[pos].y, eval[pos + 1].x, eval[pos + 1].y, x);
    else
        return 0;
}
...
double natural_logarithm(double value) { return log(value); }
