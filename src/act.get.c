...
static void queue_message(struct get_context *context, struct obj_data *obj, bool keep_count, const char *to_char,
                          const char *to_room) {
...
static bool can_get_obj(struct get_context *context, struct obj_data *obj) {
    ...
    if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch))
        ...
        queue_message(context, obj, TRUE, "You are not experienced enough to carry $p.", NULL);
    else
        return TRUE;
    return FALSE;
}
...
void perform_get_from_container(struct get_context *context, struct obj_data *obj, struct obj_data *cont) {
    struct char_data *ch = context->ch;
...
void get_from_container(struct char_data *ch, struct obj_data *cont, char *name, int *amount) {
    struct obj_data *obj;
