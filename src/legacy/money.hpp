...
class Money {
  public:
    ...
    Money(int *money)
        : platinum_(money[PLATINUM]), gold_(money[GOLD]), silver_(money[SILVER]), copper_(money[COPPER]) {}
    Money(const json &j) noexcept {
    ...
    int &operator[](int index) noexcept {
        ...
        static int dummy = 0;
        return dummy; // Return a dummy reference if index is invalid
    }
...
[[nodiscard]] bool is_coin_name(std::string_view name, int cointype);
