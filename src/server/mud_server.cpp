...
std::shared_ptr<Player> ModernMUDServer::find_player(std::string_view name) const {
    ...
    // Also check virtual sessions (admin API test players without network connections)
    if (auto virtual_player = fierymud::find_virtual_session(name)) {
...
ModernMUDServer::CommandExecutionResult ModernMUDServer::execute_command(std::string_view executor_name,
    ...
    asio::post(ws->get_strand(), [ws, actor, cmd_str, exec_name, promise]() {
        ...
        // Clear output queue before executing so we capture only this command's output
        ...
        if (player) {
            captured_output = player->get_output_queue();
        }
