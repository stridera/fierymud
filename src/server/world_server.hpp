...
struct GameLoopStats {
    ...
    GameLoopStats(const GameLoopStats &other)
        : start_time(other.start_time), commands_processed(other.commands_processed.load()),
          heartbeats(other.heartbeats.load()), average_tick_time(other.average_tick_time) {}

    // Move constructor
    GameLoopStats(GameLoopStats &&other) noexcept
        : start_time(std::move(other.start_time)), commands_processed(other.commands_processed.load()),
          heartbeats(other.heartbeats.load()), average_tick_time(std::move(other.average_tick_time)) {}
...
class WorldServer {
  public:
    ...
    void process_command(std::shared_ptr<PlayerConnection> connection, std::string_view command_text);
    void process_command(std::shared_ptr<Actor> actor, std::string_view command_text);
    ...
    void handle_player_reconnection(std::shared_ptr<PlayerConnection> old_connection,
                                    std::shared_ptr<PlayerConnection> new_connection, std::shared_ptr<Player> player);
  ...
  private:
    ...
    // Timer management
    void schedule_timer(std::chrono::milliseconds interval, std::function<void()> task,
