/***************************************************************************
 *   File: src/commands/rich_text.cpp                     Part of FieryMUD *
 *  Usage: Rich text formatting system implementation                      *
 *                                                                         *
...
std::string TextFormat::to_ansi() const {
...
bool TextFormat::has_formatting() const {
    return foreground != Color::Default || 
           background != BackgroundColor::Default || 
           !styles.empty();
}

TextFormat TextFormat::with_color(Color color) {
    TextFormat format;
    format.foreground = color;
    return format;
}

TextFormat TextFormat::with_background(BackgroundColor color) {
    TextFormat format;
    format.background = color;
    return format;
}

TextFormat TextFormat::with_style(TextStyle style) {
    TextFormat format;
    format.styles.push_back(style);
    return format;
}

TextFormat TextFormat::with_rgb(const ColorRGB& color) {
    // Note: RGB colors need special handling, not supported in basic TextFormat
    // This would require extending the system further
    return TextFormat{};
}

TextFormat TextFormat::with_256(const Color256& color) {
    // Note: 256 colors need special handling, not supported in basic TextFormat  
    // This would require extending the system further
    return TextFormat{};
}
...
RichText& RichText::colored(std::string_view str, Color color) {
    if (color != Color::Default) {
        insert_format(TextFormat::with_color(color));
    }
    content_ += str;
    if (color != Color::Default) {
        insert_reset();
...
RichText& RichText::bold(std::string_view str) {
    insert_format(TextFormat::with_style(TextStyle::Bold));
    content_ += str;
    insert_reset();
    return *this;
}
...
RichText& RichText::italic(std::string_view str) {
    insert_format(TextFormat::with_style(TextStyle::Italic));
    content_ += str;
    insert_reset();
    return *this;
}
...
RichText& RichText::underline(std::string_view str) {
    insert_format(TextFormat::with_style(TextStyle::Underline));
    content_ += str;
    insert_reset();
    return *this;
}
...
RichText& RichText::strikethrough(std::string_view str) {
    insert_format(TextFormat::with_style(TextStyle::Strikethrough));
    content_ += str;
    insert_reset();
    return *this;
}
...
RichText& RichText::formatted(std::string_view str, const TextFormat& format) {
...
void RichText::insert_format(const TextFormat& format) {
    if (format.has_formatting()) {
