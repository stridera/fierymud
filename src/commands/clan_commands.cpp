...
Result<CommandResult> cmd_clan(const CommandContext &ctx) {
    ...
    if (subcmd == "info" || subcmd == "information") {
        ...
        PlayerOutput::send_to_actor(ctx.actor(), "Usage: clan [info|who|motd|chat <message>|ranks]");
        return Ok(CommandResult::Success);
    }
}
...
Result<CommandResult> cmd_cset(const CommandContext &ctx) {
    ...
    if (!clan) {
        PlayerOutput::send_to_actor(ctx.actor(), fmt::format("Clan '{}' not found.", clan_name));
        return Ok(CommandResult::Success);
    }
    ...
    if (property == "motd") {
        ...
        PlayerOutput::send_to_actor(ctx.actor(), fmt::format("Set MOTD for clan '{}'.", (*clan)->name()));
        ...
        try {
            ...
            PlayerOutput::send_to_actor(
                ctx.actor(), fmt::format("Set application fee for clan '{}' to {} coins.", (*clan)->name(), fee));
        } catch (const std::exception &) {
        ...
        try {
            ...
            PlayerOutput::send_to_actor(
                ctx.actor(), fmt::format("Set minimum application level for clan '{}' to {}.", (*clan)->name(), level));
        } catch (const std::exception &) {
            PlayerOutput::send_to_actor(ctx.actor(), "Invalid application level.");
        }
    } else {
        PlayerOutput::send_to_actor(ctx.actor(), "Unknown property. Valid properties: motd, dues, appfee, applevel");
    }
