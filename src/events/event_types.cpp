...
GameEvent GameEvent::player_event(GameEventType type, std::string player_name, std::string message) {
    auto event = create(type, std::move(message));
    event.player_name = std::move(player_name);
    return event;
}

GameEvent GameEvent::chat_event(GameEventType type, std::string player_name, std::string message,
...
std::string_view GameEvent::channel() const noexcept { return get_channel(type); }
