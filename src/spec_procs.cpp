...
void apply_cost(int cost, char_data *ch);

int red_recall_room(char_data *ch);
int green_recall_room(char_data *ch);
int blue_recall_room(char_data *ch);
int gray_recall_room(char_data *ch);
...
SPECIAL(guild) {
    ...
    if (CMD_IS("level")) {
        ...
        if (getbaseclass(GET_CLASS((char_data *)(me))) != getbaseclass(GET_CLASS(ch))) {
...
void weapon_spell(char *to_ch, char *to_vict, char *to_room, char_data *ch, char_data *vict, struct obj_data *obj,
                  int spl) {
...
SPECIAL(holyw_weapon) {
    ...
    weapon_spell(
        ...
        ch, vict, (obj_data *)me, SPELL_HOLY_WORD);
    return 1;
}
...
SPECIAL(vampiric_weapon) {
    ...
    weapon_spell("A &9&bblack haze&0 forms around your $p as you strike $N!",
                 ...
                 "A &9&bblack haze&0 forms around $n's $p as $e strikes $N.", ch, vict, (obj_data *)me,
...
SPECIAL(fire_weapon) {
    ...
    weapon_spell("Your $o twinkles &3brightly&0 and sends waves of &6plasma&0 into $N!",
                 ...
                 "$n's $o twinkles &3brightly&0 and sends waves of &6plasma&0 into $N!", ch, vict, (obj_data *)me,
                 SPELL_FIREBALL);
...
SPECIAL(lightning_weapon) {
    ...
    weapon_spell("Your $o glows &4blue&0 and a bolt rushes through the air at $N!",
                 ...
                 "$n's $o glows &4blue&0 and a bolt rushes through the air at $N.", ch, vict, (obj_data *)me,
...
SPECIAL(frost_weapon) {
    ...
    weapon_spell("A burst of freezing air suddenly bursts from your $o!",
                 ...
                 "A burst of freezing air suddenly bursts from $n's $o!", ch, vict, (obj_data *)me, SPELL_CONE_OF_COLD);
...
int do_recall(char_data *ch, obj_data *obj, int cmd, char *argument, int color) {
    ...
    CREATE(recall, recall_event_obj, 1);
    recall->ch = targ;
...
SPECIAL(gray_recall) { return do_recall(ch, (obj_data *)me, cmd, argument, 3); }

SPECIAL(blue_recall) { return do_recall(ch, (obj_data *)me, cmd, argument, 2); }

SPECIAL(green_recall) { return do_recall(ch, (obj_data *)me, cmd, argument, 1); }

SPECIAL(red_recall) { return do_recall(ch, (obj_data *)me, cmd, argument, 0); }
...
SPECIAL(summon_dragon) {
    ...
    extern void summon_mount(char_data * ch, int mob_vnum, int base_hp, int base_mv);
    ...
    if ((item = (obj_data *)me) == NULL || item->worn_by != ch)
        return FALSE;
