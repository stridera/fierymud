...
class QuestManager {
  public:
    ...
    [[nodiscard]] std::vector<const QuestQueries::QuestData *> get_available_quests(std::string_view character_id,
                                                                                    int level) const;
    ...
    Result<void> start_quest(std::string_view character_id, int zone_id, int quest_id);
    ...
    /// @return Success or error message
    Result<void> update_objective(std::string_view character_id, int zone_id, int quest_id, int phase_id,
                                  int objective_id, int new_count);

    /// Mark an objective as completed
    Result<void> complete_objective(std::string_view character_id, int zone_id, int quest_id, int phase_id,
                                    int objective_id);

    /// Advance to next phase
    Result<void> advance_phase(std::string_view character_id, int zone_id, int quest_id, int next_phase_id);

    /// Complete a quest
    Result<void> complete_quest(std::string_view character_id, int zone_id, int quest_id);

    /// Abandon a quest
    Result<void> abandon_quest(std::string_view character_id, int zone_id, int quest_id);

    /// Get character's active quests
    [[nodiscard]] Result<std::vector<QuestQueries::CharacterQuestProgress>>
    get_active_quests(std::string_view character_id) const;

    /// Get character's completed quests
    [[nodiscard]] Result<std::vector<QuestQueries::CharacterQuestProgress>>
    get_completed_quests(std::string_view character_id) const;

    /// Check if character has quest in progress
    [[nodiscard]] bool has_quest_in_progress(std::string_view character_id, int zone_id, int quest_id) const;

    /// Check if character has completed quest
    [[nodiscard]] bool has_completed_quest(std::string_view character_id, int zone_id, int quest_id) const;
    ...
    Result<void> advance_objective(Actor &actor, int zone_id, int quest_id, int objective_id, int count = 1);

    /// Set a quest variable for an actor
    Result<void> set_quest_variable(Actor &actor, int zone_id, int quest_id, const std::string &name,
                                    const nlohmann::json &value);

    /// Get a quest variable for an actor
    [[nodiscard]] std::expected<nlohmann::json, Error> get_quest_variable(const Actor &actor, int zone_id, int quest_id,
    ...
    std::vector<const QuestQueries::QuestData *> check_level_triggers(std::shared_ptr<Actor> actor, int new_level);
    ...
    /// @return List of quests started
    std::vector<const QuestQueries::QuestData *> check_item_triggers(std::shared_ptr<Actor> actor,
    ...
    /// @return List of quests started
    std::vector<const QuestQueries::QuestData *> check_room_triggers(std::shared_ptr<Actor> actor,
    ...
    /// @return List of quests started
    std::vector<const QuestQueries::QuestData *> check_skill_triggers(std::shared_ptr<Actor> actor, int ability_id);
    ...
    void on_mob_killed(std::shared_ptr<Actor> killer, const EntityId &killed_mob_id);

    /// Called when an item is collected - updates COLLECT_ITEM objectives
    void on_item_collected(std::shared_ptr<Actor> collector, const EntityId &item_id);

    /// Called when an item is delivered to an NPC - updates DELIVER_ITEM objectives
    void on_item_delivered(std::shared_ptr<Actor> deliverer, const EntityId &recipient_mob_id, const EntityId &item_id);

    /// Called when an NPC is talked to - updates TALK_TO_NPC objectives
    void on_npc_talked(std::shared_ptr<Actor> talker, const EntityId &npc_id);

    /// Called when a room is visited - updates VISIT_ROOM objectives
    void on_room_visited(std::shared_ptr<Actor> visitor, const EntityId &room_id);

    /// Called when a skill is used - updates USE_SKILL objectives
    void on_skill_used(std::shared_ptr<Actor> user, int ability_id);
  ...
  private:
    ...
    struct QuestKey {
        ...
        bool operator==(const QuestKey &other) const { return zone_id == other.zone_id && quest_id == other.quest_id; }
    };

    struct QuestKeyHash {
        std::size_t operator()(const QuestKey &key) const {
            return std::hash<std::uint64_t>{}((static_cast<std::uint64_t>(key.zone_id) << 32) |
    ...
    bool can_accept_quest(std::string_view character_id, int zone_id, int quest_id, int level) const;
