...
void QuestManager::shutdown() {
    ...
    logger->info("QuestManager shutdown complete (started={}, completed={}, abandoned={})",
        stats_.quests_started, stats_.quests_completed, stats_.quests_abandoned);
}
...
std::expected<std::size_t, std::string> QuestManager::load_zone_quests(int zone_id) {
    ...
    auto result = pool.execute([zone_id](pqxx::work& txn)
        -> Result<std::vector<QuestQueries::QuestData>> {
        return QuestQueries::load_quests_in_zone(txn, zone_id);
    });

    if (!result) {
        last_error_ = fmt::format("Failed to load quests for zone {}: {}",
...
std::vector<const QuestQueries::QuestData*> QuestManager::get_available_quests(
...
Result<void> QuestManager::start_quest(
    std::string_view character_id,
...
Result<void> QuestManager::update_objective(
    std::string_view character_id,
    int zone_id, int quest_id,
    ...
    return pool.execute([&](pqxx::work& txn) -> Result<void> {
        return QuestQueries::update_objective_progress(
            txn, character_id, zone_id, quest_id, phase_id, objective_id, new_count);
    });
}

Result<void> QuestManager::complete_objective(
    std::string_view character_id,
    int zone_id, int quest_id,
    int phase_id, int objective_id) {
    ...
    auto result = pool.execute([&](pqxx::work& txn) -> Result<void> {
        return QuestQueries::complete_objective(
            txn, character_id, zone_id, quest_id, phase_id, objective_id);
    });
...
Result<void> QuestManager::advance_phase(
    std::string_view character_id,
    int zone_id, int quest_id,
    int next_phase_id) {
...
Result<void> QuestManager::complete_quest(
    std::string_view character_id,
    int zone_id, int quest_id) {
...
Result<void> QuestManager::abandon_quest(
    std::string_view character_id,
    int zone_id, int quest_id) {
...
Result<std::vector<QuestQueries::CharacterQuestProgress>> QuestManager::get_active_quests(
    ...
    return pool.execute([&](pqxx::work& txn)
        -> Result<std::vector<QuestQueries::CharacterQuestProgress>> {
        return QuestQueries::load_character_active_quests(txn, character_id);
    });
}

Result<std::vector<QuestQueries::CharacterQuestProgress>> QuestManager::get_completed_quests(
    std::string_view character_id) const {
...
bool QuestManager::has_quest_in_progress(
    std::string_view character_id,
    ...
    auto result = pool.execute([&](pqxx::work& txn)
        -> Result<QuestQueries::CharacterQuestProgress> {
...
bool QuestManager::has_completed_quest(
    std::string_view character_id,
    int zone_id, int quest_id) const {
...
Result<void> QuestManager::advance_objective(Actor& actor, int zone_id, int quest_id,
                                              int objective_id, int count) {
    const auto* player = dynamic_cast<const Player*>(&actor);
    if (!player) {
        return std::unexpected(Errors::InvalidState("Only players can have quests"));
    }
    ...
    auto progress_result = pool.execute([&](pqxx::work& txn)
        -> Result<QuestQueries::CharacterQuestProgress> {
        return QuestQueries::load_character_quest(txn, character_id, zone_id, quest_id);
    });
    ...
    for (const auto& obj_progress : progress_result->objective_progress) {
        if (obj_progress.objective_id == objective_id) {
            ...
            return update_objective(character_id, zone_id, quest_id,
                                     obj_progress.phase_id, objective_id, new_count);
        }
    }
...
Result<void> QuestManager::set_quest_variable(Actor& actor, int zone_id, int quest_id,
...
std::expected<nlohmann::json, Error> QuestManager::get_quest_variable(
...
std::vector<const QuestQueries::QuestData*> QuestManager::check_level_triggers(
...
std::vector<const QuestQueries::QuestData*> QuestManager::check_item_triggers(
    std::shared_ptr<Actor> actor,
...
std::vector<const QuestQueries::QuestData*> QuestManager::check_room_triggers(
    std::shared_ptr<Actor> actor,
...
std::vector<const QuestQueries::QuestData*> QuestManager::check_skill_triggers(
    std::shared_ptr<Actor> actor,
    int ability_id) {
...
void QuestManager::on_mob_killed(
...
std::size_t QuestManager::quest_count() const {
    return quest_cache_.size();
}
...
bool QuestManager::can_accept_quest(
    std::string_view character_id,
    int zone_id, int quest_id,
    int level) const {
    ...
    switch (quest.trigger_type) {
        ...
        case QuestQueries::QuestTriggerType::ITEM:
            if (quest.trigger_item) {
                QuestKey item_key{static_cast<int>(quest.trigger_item->zone_id()), static_cast<int>(quest.trigger_item->local_id())};
        ...
        case QuestQueries::QuestTriggerType::ROOM:
            if (quest.trigger_room) {
                QuestKey room_key{static_cast<int>(quest.trigger_room->zone_id()), static_cast<int>(quest.trigger_room->local_id())};
...
void QuestManager::unindex_quest_triggers(const QuestQueries::QuestData& quest) {
    ...
    auto remove_key = [&key](std::vector<QuestKey>& vec) {
        std::erase_if(vec, [&key](const QuestKey& k) {
            return k.zone_id == key.zone_id && k.quest_id == key.quest_id;
        });
    };
