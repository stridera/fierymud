...
void verify_obj_spell(struct obj_data *obj, int valnum, bool zero_ok) {
    if (!IS_SPELL(GET_OBJ_VAL(obj, valnum)) && !(zero_ok && GET_OBJ_VAL(obj, valnum) == 0)) {
        mprintf(L_WARN, LVL_IMMORT, "ERROR: Invalid spell in object prototype. vnum=%d spellnum=%d", GET_OBJ_VNUM(obj),
                GET_OBJ_VAL(obj, valnum));
 ...
 * Removing equipment restriction check from zone EQUIP commands.  So many
 *current zone commands violate the restrictions that it isn't worth it to
 *enforce restrictions at this time.
 *
 ...
 * Zone command E (equip char) will handle failures better, and destroy the
 *object rather than letting it sit in nowhere.
 *
 ...
 * Changed mob load so that when it is read from the mob file it reads the size
 *in instead of basing it on the race.
 *
 ...
 * fixed hidden exit with no door? for some reason setting EX_DESCRIPT seems to
 *override the other stuff about an exit...
 *
