...
/*class/species */
{
        /*mob_proto[i].player.class, mob_proto[i].player.race,
...
void interpret_espec(char *keyword, char *value, int i, int nr) {
        ...
        CASE("AFF2") { EFF_FLAGS(mob_proto + i)[1] = asciiflag_conv(value); }

        CASE("AFF3") { EFF_FLAGS(mob_proto + i)[2] = asciiflag_conv(value); }

        CASE("MOB2") { MOB_FLAGS(mob_proto + i)[1] = asciiflag_conv(value); }
...
char *parse_object(FILE *obj_f, int nr) {
        ...
        if (!get_line(obj_f, line) || (retval = sscanf(line, "%f %d %d %s %d %d %s %s", &obj_proto[i].obj_flags.weight,
                                                       t + 1, t + 2, f1, t + 4, t + 5, f2, f3)) != 8) {
        ...
        obj_proto[i].obj_flags.effect_flags[0] = asciiflag_conv(f1);
        /*    obj_proto[i].spell_component = t[4]; */
        /*    obj_proto[i].object_limitation = t[5]; */
        obj_proto[i].obj_flags.effect_flags[1] = asciiflag_conv(f2);
        obj_proto[i].obj_flags.effect_flags[2] = asciiflag_conv(f3);
        ...
        for (;;) {
            ...
            case 'X':
                get_line(obj_f, line);
                sscanf(line, "%s ", f1);
                obj_proto[i].obj_flags.extra_flags[1] = asciiflag_conv(f1);
                break;
