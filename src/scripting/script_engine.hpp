...
template <typename T> using ScriptResult = std::expected<T, ScriptError>;
...
class ScriptEngine {
  public:
    ...
    ScriptResult<void> compile_script(std::string_view script_code, std::string_view script_name);
    ...
    ScriptResult<sol::function> load_cached(sol::state_view thread_state, std::string_view script_code,
    ...
    ScriptResult<sol::protected_function_result> execute(std::string_view script_code,
                                                         std::string_view script_name = "script");

    /// Execute a pre-compiled function with error handling
    template <typename... Args>
    ScriptResult<sol::protected_function_result> safe_call(sol::protected_function &func, Args &&...args);
    ...
    void reset_instruction_counter() {
...
template <typename... Args>
ScriptResult<sol::protected_function_result> ScriptEngine::safe_call(sol::protected_function &func, Args &&...args) {
    if (!initialized_) {
