...
void register_actor_bindings(sol::state &lua) {
    ...
    lua.new_enum<Position>("Position", {{"Dead", Position::Dead},
    ...
    lua.new_usertype<Actor>(
        ...
        sol::property([](const Actor &a) { return a.stats().max_hit_points; }), "mana",
        sol::property([](const Actor &a) { return a.stats().mana; }), "max_mana",
        sol::property([](const Actor &a) { return a.stats().max_mana; }), "stamina",
        sol::property([](const Actor &a) { return a.stats().stamina; }), "max_stamina",
        sol::property([](const Actor &a) { return a.stats().max_stamina; }), "move",
        sol::property([](const Actor &a) { return a.stats().stamina; }),                 // Alias for backwards compat
        "max_move", sol::property([](const Actor &a) { return a.stats().max_stamina; }), // Alias for backwards compat
        "alignment", sol::property([](const Actor &a) { return a.stats().alignment; }), "wealth",
        sol::property(&Actor::wealth),

        // Primary stats
        "strength", sol::property([](const Actor &a) { return a.stats().strength; }), "dexterity",
        sol::property([](const Actor &a) { return a.stats().dexterity; }), "intelligence",
        sol::property([](const Actor &a) { return a.stats().intelligence; }), "wisdom",
        sol::property([](const Actor &a) { return a.stats().wisdom; }), "constitution",
        sol::property([](const Actor &a) { return a.stats().constitution; }), "charisma",
        sol::property([](const Actor &a) { return a.stats().charisma; }),

        // Position and state
        "position", sol::property([](const Actor &a) { return a.position(); }), "position_name",
        sol::property([](const Actor &a) { return std::string(ActorUtils::get_position_name(a.position())); }),

        // Room access (returns Room userdata or nil)
        "room", sol::property([](const Actor &a) -> std::shared_ptr<Room> { return a.current_room(); }),

        // Type checks
        "is_npc", sol::property([](const Actor &a) { return a.type_name() == "Mobile"; }), "is_player",
        sol::property([](const Actor &a) { return a.type_name() == "Player"; }),
        ...
        // State checks
        "is_fighting", sol::property(&Actor::is_fighting), "is_alive", sol::property(&Actor::is_alive), "can_act",
        sol::property(&Actor::can_act), "is_afk", sol::property(&Actor::is_afk),

        // Gender and race
        "gender", sol::property([](const Actor &a) { return std::string(a.gender()); }), "race",
        sol::property([](const Actor &a) { return std::string(a.race()); }), "size",
        sol::property([](const Actor &a) { return std::string(a.size()); }),

        // Methods - Communication
        "send", [](Actor &a, const std::string &msg) { a.send_message(msg); },

        // Methods - Actions (direct room broadcast, bypasses command system for script efficiency)
        "say",
        [](Actor &a, const std::string &msg) {
        ...
        [](Actor &a, int amount) {
            if (amount <= 0)
                return;
            a.stats().hit_points = std::min(a.stats().max_hit_points, a.stats().hit_points + amount);
        },

        // Methods - Inventory queries
        "has_item",
        [](const Actor &a, const std::string &keyword) -> bool {
            auto items = a.inventory().find_items_by_keyword(keyword);
            return !items.empty();
        },

        "item_count",
        [](const Actor &a, const std::string &keyword) -> int {
            auto items = a.inventory().find_items_by_keyword(keyword);
            return static_cast<int>(items.size());
        },

        // Methods - Economy (unified interface)
        "give_wealth", &Actor::give_wealth, "take_wealth", &Actor::take_wealth, "has_wealth", &Actor::can_afford,

        // Methods - Effects
        "has_effect", [](const Actor &a, const std::string &name) -> bool { return a.has_effect(name); },

        "has_flag", [](const Actor &a, ActorFlag flag) -> bool { return a.has_flag(flag); },
        ...
        "command",
        [](std::shared_ptr<Actor> self, const std::string &cmd) -> bool {
            if (!self || cmd.empty())
                return false;
        ...
        "engage",
        [](std::shared_ptr<Actor> self, std::shared_ptr<Actor> target) -> bool {
            if (!self || !target || self == target)
                return false;
            if (CombatManager::is_in_combat(*self))
                return false;
        ...
        "disengage",
        [](std::shared_ptr<Actor> self) -> bool {
            if (!self)
                return false;
            if (!CombatManager::is_in_combat(*self))
                return false;
        ...
        "cast",
        [](Actor &self, const std::string &spell_name, sol::optional<Actor *> target) -> bool {
            if (spell_name.empty())
                return false;
        ...
        // Methods - Communication
        "whisper",
        [](Actor &self, std::shared_ptr<Actor> target, const std::string &msg) {
            if (!target || msg.empty())
                return;
        ...
        "move",
        [](std::shared_ptr<Actor> self, const std::string &dir_str) -> bool {
            if (!self)
                return false;
        ...
        "follow",
        [](std::shared_ptr<Actor> self, sol::optional<std::shared_ptr<Actor>> target_opt) -> bool {
            if (!self)
                return false;
            if (!target_opt.has_value() || !target_opt.value())
                return false;
        ...
        "master", sol::property([](const Actor &self) -> std::shared_ptr<Actor> { return self.get_master(); }),

        // Check if following someone
        "has_master", sol::property([](const Actor &self) -> bool { return self.has_master(); }),

        // Methods - Inventory management
        // Destroy an item in actor's inventory by keyword
        // Usage: actor:destroy_item("keyword")
        "destroy_item",
        [](std::shared_ptr<Actor> self, const std::string &keyword) -> bool {
    ...
    lua.new_usertype<Mobile>(
        "Mobile", sol::no_constructor, sol::base_classes, sol::bases<Actor>(),

        // NPC-specific properties
        "is_aggressive", sol::property(&Mobile::is_aggressive), "is_shopkeeper", sol::property(&Mobile::is_shopkeeper),
        "is_banker", sol::property(&Mobile::is_banker), "is_teacher", sol::property(&Mobile::is_teacher),
        ...
        // Description
        "description", sol::property([](const Mobile &m) { return std::string(m.description()); }),

        // Combat dice
        "damage_dice_num", sol::property(&Mobile::bare_hand_damage_dice_num), "damage_dice_size",
        sol::property(&Mobile::bare_hand_damage_dice_size),

        // Mob trait/behavior/profession checks (new system)
        "has_trait", [](const Mobile &m, db::MobTrait trait) -> bool { return m.has_trait(trait); }, "has_behavior",
        [](const Mobile &m, db::MobBehavior behavior) -> bool { return m.has_behavior(behavior); }, "has_profession",
    ...
    lua.new_enum<db::MobBehavior>("MobBehavior", {{"Sentinel", db::MobBehavior::Sentinel},
    ...
    lua.new_enum<db::MobProfession>("MobProfession", {{"Banker", db::MobProfession::Banker},
    ...
    lua.new_enum<ActorFlag>("ActorFlag", {{"Blind", ActorFlag::Blind},
