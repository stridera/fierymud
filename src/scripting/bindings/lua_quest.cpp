...
void register_quest_bindings(sol::state& lua) {
    ...
    quest_table["advance_objective"] = [](Actor* actor, int zone_id, int quest_id,
                                          int objective_id, sol::optional<int> count) -> bool {
        if (!actor) {
    ...
    quest_table["set_variable"] = [](Actor* actor, int zone_id, int quest_id,
                                     const std::string& name, sol::object value) -> bool {
        if (!actor) {
