...
void register_room_bindings(sol::state &lua) {
    ...
    // SectorType enum
    lua.new_enum<SectorType>("SectorType", {{"Inside", SectorType::Inside},
    ...
    lua.new_usertype<Room>(
        ...
        // Basic properties
        "name", sol::property([](const Room &r) { return std::string(r.name()); }), "id",
        sol::property([](const Room &r) -> std::string {
            auto id = r.id();
        ...
        "base_light_level", sol::property(&Room::base_light_level), "effective_light",
        sol::property(&Room::calculate_effective_light), "capacity", sol::property(&Room::capacity),

        // State checks
        "is_dark", sol::property(&Room::is_dark), "is_peaceful", sol::property(&Room::is_peaceful), "allows_magic",
        sol::property(&Room::allows_magic), "allows_recall", sol::property(&Room::allows_recall), "allows_summon",
        sol::property(&Room::allows_summon), "allows_teleport", sol::property(&Room::allows_teleport), "is_death_trap",
        sol::property(&Room::is_death_trap), "is_full", sol::property(&Room::is_full),

        // Entry restriction (Lua script for access control)
        "entry_restriction", sol::property([](const Room &r) -> std::string { return r.entry_restriction(); }),
        ...
        "actor_count", sol::property([](const Room &r) -> int { return static_cast<int>(r.contents().actors.size()); }),
        ...
        "object_count",
        sol::property([](const Room &r) -> int { return static_cast<int>(r.contents().objects.size()); }),
        ...
        "has_exit", [](const Room &r, Direction dir) -> bool { return r.has_exit(dir); },

        "get_exit_room",
        [](const Room &r, Direction dir) -> std::shared_ptr<Room> {
        ...
        "exit_is_open",
        [](const Room &r, Direction dir) -> bool {
            const auto *exit = r.get_exit(dir);
            if (!exit)
                return false;
        ...
        "exit",
        [](std::shared_ptr<Room> room, const std::string &dir_str) -> std::shared_ptr<ExitWrapper> {
        ...
        "send_except",
        [](Room &r, std::shared_ptr<Actor> except, const std::string &msg) {
            for (auto &actor : r.contents().actors) {
                if (actor != except) {
                    actor->send_message(msg);
                }
        ...
        // Finding entities
        "find_actor",
        [](Room &r, const std::string &keyword) -> std::shared_ptr<Actor> {
            auto actors = r.find_actors_by_keyword(keyword);
            return actors.empty() ? nullptr : actors[0];
        },

        "find_actors",
        [](Room &r, const std::string &keyword) -> sol::as_table_t<std::vector<std::shared_ptr<Actor>>> {
            return sol::as_table(r.find_actors_by_keyword(keyword));
        },

        "find_object",
        [](Room &r, const std::string &keyword) -> std::shared_ptr<Object> {
            auto objects = r.find_objects_by_keyword(keyword);
            return objects.empty() ? nullptr : objects[0];
        },

        "find_objects",
        [](Room &r, const std::string &keyword) -> sol::as_table_t<std::vector<std::shared_ptr<Object>>> {
        ...
        "dir",
        [](const Room &r, const std::string &dir_str) -> std::shared_ptr<Room> {
        ...
        "at",
        [](std::shared_ptr<Room> room, sol::protected_function fn) -> sol::protected_function_result {
            if (!room || !fn.valid()) {
        ...
        [](std::shared_ptr<Room> room, int zone_id, int local_id) -> std::shared_ptr<Mobile> {
            ...
            spdlog::debug("room:spawn_mobile: Spawned mobile {}:{} in room {}", zone_id, local_id,
                          room->id().to_string());
            return mobile;
        },
        ...
        [](std::shared_ptr<Room> room, int zone_id, int local_id) -> std::shared_ptr<Object> {
            ...
            spdlog::debug("room:spawn_object: Spawned object {}:{} in room {}", zone_id, local_id,
                          room->id().to_string());
            return object;
        },
    ...
    lua.set_function("reverse_direction",
                     [](Direction dir) -> Direction { return RoomUtils::get_opposite_direction(dir); });

    lua.set_function("direction_name",
                     [](Direction dir) -> std::string { return std::string(RoomUtils::get_direction_name(dir)); });

    // Global get_room function - accepts (zone_id, local_id) composite format
    // This is the modern API - two separate integer parameters
    lua.set_function("get_room",
                     sol::overload(
