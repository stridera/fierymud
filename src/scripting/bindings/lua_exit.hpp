...
class ExitWrapper {
  public:
    ExitWrapper(std::shared_ptr<Room> room, Direction dir) : room_(std::move(room)), direction_(dir) {}

    // Check if this exit wrapper is valid
    bool is_valid() const { return room_ && room_->has_exit(direction_); }
    ...
    std::shared_ptr<Room> room() const { return room_; }
    ...
    bool lock() {
        ...
        if (!exit || !exit->has_door)
            return false;
        if (!exit->is_closed)
            return false; // Must be closed first
        exit->is_locked = true;
        return true;
    }
    ...
    bool set_state(sol::table state) {
        ...
        spdlog::debug("exit:set_state: updated exit in room {}", room_->id().to_string());
        return true;
    }
    ...
    bool set_destination(std::shared_ptr<Room> dest_room) {
        ...
        if (!dest_room) {
            ...
            spdlog::debug("exit:set_destination: cleared destination for exit in room {}", room_->id().to_string());
            return true;
        }
        exit->to_room = dest_room->id();
        spdlog::debug("exit:set_destination: set destination to {} for exit in room {}", dest_room->id().to_string(),
                      room_->id().to_string());
        return true;
    }
