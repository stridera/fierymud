...
struct compdef {
   ...
   char *color;
   int default_dtype;
   /* Helps determine whether physical attacks will "pass through" or "bounce off".
    * Also whether you leave a corpse behind. */
   int phase;
...
#define RIGID(ch) (VALID_COMPOSITION(ch) ? \
      compositions[GET_COMPOSITION(ch)].phase == PHASE_SOLID : TRUE)
#define PHASE(ch) (VALID_COMPOSITION(ch) ? \
      compositions[GET_COMPOSITION(ch)].phase : PHASE_SOLID)
