...
std::vector<std::shared_ptr<Player>> WorldServer::get_online_players() const { return online_players_; }
...
std::shared_ptr<CommandSystem> WorldServer::get_command_system() const {
    if (!command_system_) {
        ...
        command_system_ = std::make_shared<CommandSystem>(std::const_pointer_cast<WorldServer>(shared_from_this()));
