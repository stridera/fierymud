...
bool GameTime::is_season(Season season) const noexcept { return get_season() == season; }
...
SunlightState TimeSystem::calculate_sunlight(int hour, Hemisphere hemisphere) noexcept {
    ...
    if (!is_offset) {
        ...
        if (hour == 6)
            return SunlightState::Rise;
        if (hour >= 7 && hour < 19)
            return SunlightState::Light;
        if (hour == 19)
            return SunlightState::Set;
        return SunlightState::Dark;
    } else {
...
void TimeSystem::on_hour_changed(TimeChangeCallback callback) { hour_callbacks_.push_back(std::move(callback)); }

void TimeSystem::on_day_changed(TimeChangeCallback callback) { day_callbacks_.push_back(std::move(callback)); }

void TimeSystem::on_month_changed(TimeChangeCallback callback) { month_callbacks_.push_back(std::move(callback)); }

void TimeSystem::on_year_changed(TimeChangeCallback callback) { year_callbacks_.push_back(std::move(callback)); }
