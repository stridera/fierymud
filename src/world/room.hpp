...
struct ExitInfo {
    ...
    bool is_passable() const { return to_room.is_valid() && (!has_door || !is_closed); }

    /** Check if exit has functional door */
    bool has_functional_door() const { return has_door && !keyword.empty(); }
...
class Room : public Entity {
  public:
    ...
    static Result<std::unique_ptr<Room>> create(EntityId id, std::string_view name,
...
template <> struct fmt::formatter<Direction> {
    ...
    template <typename FormatContext> auto format(const Direction &dir, FormatContext &ctx) const {
        return fmt::format_to(ctx.out(), "{}", RoomUtils::get_direction_name(dir));
    }
};

/** Formatting support for SectorType */
template <> struct fmt::formatter<SectorType> {
    constexpr auto parse(format_parse_context &ctx) { return ctx.begin(); }

    template <typename FormatContext> auto format(const SectorType &sector, FormatContext &ctx) const {
