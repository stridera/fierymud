...
struct ZoneStats {
    ...
    std::chrono::seconds time_since_reset() const {
        return std::chrono::duration_cast<std::chrono::seconds>(
            std::chrono::steady_clock::now() - last_reset);
    }
};
...
public:
    ...
    static Result<std::unique_ptr<Zone>> create(EntityId id, std::string_view name, 
    ...
    void set_reset_time(std::chrono::steady_clock::time_point time) { 
        stats_.last_reset = time;
    }
    ...
    // Level restrictions
    bool allows_level(int level) const {
...
private:
    ...
    static Result<ZoneCommand> parse_args(ZoneCommandType type, 
...
template<>
struct fmt::formatter<ResetMode> {
    constexpr auto parse(format_parse_context& ctx) {
        return ctx.begin();
    }
    
    template<typename FormatContext>
    auto format(const ResetMode& mode, FormatContext& ctx) const {
...
template<>
struct fmt::formatter<ZoneFlag> {
    constexpr auto parse(format_parse_context& ctx) {
        return ctx.begin();
    }
    
    template<typename FormatContext>
    auto format(const ZoneFlag& flag, FormatContext& ctx) const {
...
template<>
struct fmt::formatter<ZoneCommandType> {
    constexpr auto parse(format_parse_context& ctx) {
        return ctx.begin();
    }
    
    template<typename FormatContext>
    auto format(const ZoneCommandType& type, FormatContext& ctx) const {
