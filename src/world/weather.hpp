...
template<>
struct fmt::formatter<WeatherIntensity> {
    constexpr auto parse(format_parse_context& ctx) {
        return ctx.begin();
    }
...
template<>
struct fmt::formatter<WeatherPattern> {
    constexpr auto parse(format_parse_context& ctx) {
        return ctx.begin();
    }
