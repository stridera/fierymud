...
struct GameTime {
    ...
    [[nodiscard]] constexpr int64_t total_hours() const noexcept {
        return static_cast<int64_t>(year) * TimeConstants::DAYS_PER_YEAR * TimeConstants::HOURS_PER_DAY +
               ...
               day * TimeConstants::HOURS_PER_DAY + hour;
    }
...
class TimeSystem {
  public:
    ...
    void on_sunlight_changed(
        std::function<void(SunlightState old_state, SunlightState new_state, Hemisphere hemisphere)> callback);
