...
void WorldManager::request_shutdown(int seconds_from_now, std::string_view reason) {
    ...
    shutdown_time_ = std::chrono::steady_clock::now() + std::chrono::seconds(seconds_from_now);
    shutdown_reason_ = std::string(reason);

    Log::warn("Shutdown requested: {} (in {} seconds)", reason.empty() ? "No reason given" : reason, seconds_from_now);
}
...
Result<void> WorldManager::load_world() {
    ...
    if (trigger_mgr.is_initialized()) {
        auto triggers_result =
            ConnectionPool::instance().execute([](pqxx::work &txn) { return TriggerQueries::load_all_triggers(txn); });
        if (triggers_result) {
    ...
    logger->info("World loading complete: {} zones, {} rooms loaded in {}ms", stats_.zones_loaded, stats_.rooms_loaded,
    ...
    if (zone_json.contains("objects") && zone_json["objects"].is_array()) {
        for (const auto &object_json : zone_json["objects"]) {
            ...
            if (object_result) {
                ...
                logger->trace("Loaded object prototype: {} ({}) in zone {}", object->name(), object->id(), zone_name);
            } else {
    ...
    if (zone_json.contains("mobs") && zone_json["mobs"].is_array()) {
        for (const auto &mob_json : zone_json["mobs"]) {
            ...
            if (mob_result) {
                ...
                logger->trace("Loaded mobile prototype: {} ({}) in zone {}", mob->name(), mob_id, zone_name);
            } else {
                logger->error("Failed to parse mobile {} in zone {}: {}", mob_id, zone_name,
                              mob_result.error().message);
            }
        }
    }
...
MovementResult WorldManager::move_actor(std::shared_ptr<Actor> actor, Direction direction) {
    ...
    logger->debug("Actor {} moved {} from {} to {}", actor->display_name(), direction, current_room->display_name(),
                  destination_room->display_name());
    ...
    for (const auto &follower_weak : actor->get_followers()) {
        ...
        if (follower && follower->current_room() == current_room) {
            ...
            logger->debug("Follower {} followed {} to {}", follower->display_name(), actor->display_name(),
                          destination_room->display_name());
        }
    }
...
MovementResult WorldManager::move_actor_to_room(std::shared_ptr<Actor> actor, EntityId room_id) {
    ...
    auto logger = Log::movement();
    logger->debug("Actor {} teleported to {}", actor->display_name(), destination_room->display_name());
    ...
    for (const auto &[id, room] : rooms_) {
        if (room) {
            ...
            if (!found_in_zone) {
                ...
                result.add_warning(fmt::format("Room {} ({}) is not in any zone", room->name(), room->id()));
            }
        }
    }
    ...
    if (start_room_.is_valid() && get_room(start_room_)) {
        ...
        for (const auto &[id, room] : rooms_) {
            if (room && !reachable.contains(id)) {
                ...
                result.add_warning(fmt::format("Room {} ({}) is not reachable from start room", room->name(), id));
            }
        }
    }
...
Result<void> WorldManager::repair_world_issues() {
    ...
    logger->info("Attempting to repair {} world issues ({} errors, {} warnings)",
                 validation.errors.size() + validation.warnings.size(), validation.errors.size(),
        ...
        for (const auto &[id, room] : rooms_) {
            ...
            for (Direction dir : exits) {
                ...
                if (!exit->to_room.is_valid() || !get_room(exit->to_room)) {
                    ...
                    logger->warn("Removed invalid exit {} from room {} (destination {} not found)", dir, room->id(),
                                 exit->to_room);
                    repairs_made++;
                }
            }
...
void WorldManager::update_statistics() {
    ...
    auto logger = Log::game();
    logger->debug(
        "Statistics updated: {} zones, {} rooms, {} object protos ({} spawned), {} mobile protos ({} spawned)",
        stats_.zones_loaded, stats_.rooms_loaded, stats_.objects_loaded, spawned_objects, stats_.mobiles_loaded,
        spawned_mobiles);
}
...
std::string WorldManager::get_world_summary() const {
    ...
    return fmt::format(
        ...
        "  Load Time: {}ms\n"
        "  Save Time: {}ms",
        zones_.size(), rooms_.size(), stats_.objects_loaded, stats_.mobiles_loaded, stats_.uptime().count(),
        stats_.time_since_save().count(), stats_.load_time.count(), stats_.save_time.count());
}
...
void WorldManager::dump_world_state(const std::string &filename) const {
    ...
    // Dump statistics
    dump["statistics"] = {{"zones_loaded", stats_.zones_loaded},       {"rooms_loaded", stats_.rooms_loaded},
                          {"objects_loaded", stats_.objects_loaded},   {"mobiles_loaded", stats_.mobiles_loaded},
                          {"uptime_seconds", stats_.uptime().count()}, {"load_time_ms", stats_.load_time.count()},
...
Result<void> WorldManager::load_zones_from_database() {
    ...
    auto load_result = ConnectionPool::instance().execute([&](pqxx::work &txn) -> Result<void> {
        ...
        for (auto &zone_ptr : *zones_result) {
            ...
            for (auto &mob_ptr : *mobs_result) {
                ...
                if (mob->is_aggressive()) {
                    ...
                    logger->trace("Loaded aggro mob: {} ({}) - condition: {}", mob->name(), mob_id,
                                  cond ? *cond : "none");
                }
        ...
        for (const auto &[room_id, room] : rooms_) {
            ...
            if (!exits_result) {
                logger->warn("Failed to load exits for room ({}, {}): {}", room_zone_id, room_local_id,
                             exits_result.error().message);
                continue;
            }
            ...
            for (const auto &exit_data : *exits_result) {
                ...
                if (!set_result) {
                    logger->warn("Failed to set exit {} for room ({}, {}): {}", exit_data.direction, room_zone_id,
                                 room_local_id, set_result.error().message);
                }
            }
        }
        ...
        for (auto &[zone_entity_id, zone] : zones_) {
            ...
            if (!mob_resets_result) {
                logger->warn("Failed to load mob resets for zone {}: {}", zone_id, mob_resets_result.error().message);
            } else {
                        ...
                        for (const auto &equip : equip_it->second) {
                            ...
                            if (equip.wear_location.empty()) {
                                ...
                                if (slot) {
                                    ...
                                    logger->warn(
                                        "Unknown wear_location '{}' for object {} on mob {}, defaulting to inventory",
            ...
            if (!obj_resets_result) {
                logger->warn("Failed to load object resets for zone {}: {}", zone_id,
                             obj_resets_result.error().message);
            } else {
                ...
                convert_content =
                    [&convert_content](const WorldQueries::ObjectResetContentData &data) -> ObjectContent {
        ...
        if (!shops_result) {
            ...
            for (const auto &shop_data : *shops_result) {
                ...
                if (mob_it == mobiles_.end()) {
                    logger->warn("Shop {} references non-existent mob {}", shop_data.id, shop_data.keeper_id);
                    continue;
                }
                ...
                auto items_result = WorldQueries::load_shop_items(txn, shop_data.zone_id, shop_data.id);
                if (items_result) {
                    for (const auto &item_data : *items_result) {
                        ...
                        if (obj_it == objects_.end()) {
                            logger->warn("Shop {} item references non-existent object {}", shop_data.id,
                                         item_data.object_id);
                            continue;
                        }
                ...
                auto mobs_result = WorldQueries::load_shop_mobs(txn, shop_data.zone_id, shop_data.id);
                if (mobs_result && !mobs_result->empty()) {
                    ...
                    for (const auto &mob_data : *mobs_result) {
                        ...
                        if (mob_proto_it == mobiles_.end()) {
                            logger->warn("Shop {} mob references non-existent mob {}", shop_data.id, mob_data.mob_id);
                            continue;
                        }
                        ...
                        shop_mob.cost = (mob_data.price > 0) ? mob_data.price : mob_proto->stats().level * 100;
                    ...
                    logger->debug("Loaded {} mobs for pet shop {}", mobs_result->size(), shop_data.keeper_id);
                }
        ...
        logger->info("Successfully loaded {} zones, {} rooms, {} mobs, {} objects from database", stats_.zones_loaded,
                     stats_.rooms_loaded, stats_.mobiles_loaded, stats_.objects_loaded);
        return Success();
    });
...
void WorldManager::validate_room_exits(std::shared_ptr<Room> room, ValidationResult &result) const {
    ...
    for (Direction dir : exits) {
        ...
        if (!exit->to_room.is_valid()) {
            ...
            result.add_error(fmt::format("Room {} has invalid exit {} destination", room->id(), dir));
            continue;
        }
        ...
        if (!get_room(exit->to_room)) {
            result.missing_exits++;
            result.add_error(
                fmt::format("Room {} exit {} points to non-existent room {}", room->id(), dir, exit->to_room));
        }
    }
}
...
void WorldManager::validate_zone_integrity(std::shared_ptr<Zone> zone, ValidationResult &result) const {
    ...
    if (!zone_result) {
        result.add_error(fmt::format("Zone {} validation failed: {}", zone->id(), zone_result.error().message));
    }
...
MovementResult WorldManager::check_movement_restrictions(std::shared_ptr<Actor> actor,
    ...
    if (to_room->has_entry_restriction() && !is_god) {
        ...
        if (engine.is_initialized()) {
            ...
            auto loaded = engine.load_cached(
                lua, script, fmt::format("room_restriction:{}:{}", to_room->id().zone_id(), to_room->id().local_id()));
        ...
        if (!zone->allows_level(actor_level)) {
            if (actor_level < zone->min_level()) {
                return MovementResult(
                    fmt::format("You must be at least level {} to enter this area", zone->min_level()));
            } else {
                return MovementResult(fmt::format("This area is for characters level {} and below", zone->max_level()));
            }
        }
    }
    ...
    if (RoomUtils::requires_swimming(sector)) {
        if (!actor->has_flag(ActorFlag::Waterwalk) && !actor->has_flag(ActorFlag::Flying) && !is_god) {
...
std::vector<Direction> WorldManager::reconstruct_path(const std::unordered_map<EntityId, EntityId> &came_from,
...
bool WorldManager::is_passable_for_actor(std::shared_ptr<Room> room, std::shared_ptr<Actor> actor) const {
    ...
    if (RoomUtils::requires_swimming(sector)) {
        ...
        if (!actor->has_flag(ActorFlag::Waterwalk) && !actor->has_flag(ActorFlag::Flying)) {
...
int WorldManager::get_movement_cost(std::shared_ptr<Room> /* from_room */, std::shared_ptr<Room> to_room,
    ...
    if (actor) {
        ...
        if (actor->max_carry_weight() > 0) {
            encumbrance_ratio =
                static_cast<float>(actor->current_carry_weight()) / static_cast<float>(actor->max_carry_weight());
        }
...
Result<void> WorldManager::spawn_mobile_in_zone(Mobile *prototype, EntityId zone_id) {
    ...
    new_mobile->set_bare_hand_damage(prototype->bare_hand_damage_dice_num(), prototype->bare_hand_damage_dice_size(),
    ...
    if (!move_result.success) {
        logger->error("Failed to place mobile {} in room {}: {}", prototype->name(), spawn_room->id(),
                      move_result.failure_reason);
        return std::unexpected(Errors::InvalidState(move_result.failure_reason));
    }

    logger->debug("Spawned mobile '{}' in room '{}' ({})", prototype->name(), spawn_room->name(), spawn_room->id());
...
void WorldManager::setup_zone_callbacks(std::shared_ptr<Zone> zone) {
    ...
    zone->set_get_room_callback([this](EntityId room_id) -> std::shared_ptr<Room> { return get_room(room_id); });
    ...
    zone->set_cleanup_zone_mobiles_callback([this](EntityId zone_id) { cleanup_zone_mobiles(zone_id); });
}
...
std::shared_ptr<Mobile> WorldManager::spawn_mobile_for_zone(EntityId mobile_id, EntityId room_id) {
    ...
    if (!move_result.success) {
        logger->error("Failed to place spawned mobile in room {}: {}", room_id, move_result.failure_reason);
        return nullptr;
    }
    ...
    if (prototype->is_aggressive() || mobile_ptr->is_aggressive()) {
        ...
        logger->trace("Spawned aggro mob '{}': condition={}", mobile_ptr->name(), cond ? *cond : "none");
    }
...
std::shared_ptr<Object> WorldManager::spawn_object_for_zone(EntityId object_id, EntityId room_id) {
    ...
    if (!is_equipment_request) {
        ...
        bool suitable_for_mobile =
            (new_object->weight() <= MOBILE_INVENTORY_MAX_WEIGHT && new_object->type() != ObjectType::Fountain);
            ...
            if (target_mobile) {
                ...
                if (!give_result) {
                    logger->warn("Failed to give object '{}' ({}) to mobile '{}' ({}): {}",
                                 prototype->short_description(), object_id, target_mobile->display_name(), room_id,
                                 give_result.error().message);
                    return nullptr;
                }

                logger->trace("Gave '{}' ({}) to mobile '{}' inventory", prototype->short_description(), object_id,
                              target_mobile->display_name());
                return shared_object;
            }
        }
    ...
    if (is_equipment_request) {
        ...
        if (!target_mobile) {
            logger->warn("Cannot equip object '{}' ({}) - mobile {} not found", prototype->short_description(),
                         object_id, mobile_id);
            return nullptr;
        }
        ...
        if (!equip_result) {
            logger->debug("Failed to equip '{}' ({}) on '{}' in slot {}: {} - adding to inventory",
                          ...
                          magic_enum::enum_name(equip_slot), equip_result.error().message);
            // If equip fails, add to inventory instead
            auto give_result = target_mobile->give_item(shared_object);
            if (!give_result) {
                logger->warn("Failed to give object '{}' ({}) to mobile '{}': {}", prototype->short_description(),
                             object_id, target_mobile->display_name(), give_result.error().message);
                return nullptr;
            }
        }

        logger->trace("Equipped '{}' ({}) on mobile '{}' in slot {}", prototype->short_description(), object_id,
    ...
    if (room) {
        ...
        logger->trace("Spawned '{}' ({}) in room '{}' ({})", prototype->short_description(), object_id, room->name(),
                      room_id);
        return shared_object;
    }

    // Room not found - this is actually an error since we expected a valid room
    logger->warn("Cannot place object '{}' ({}) - room {} not found", prototype->short_description(), object_id,
                 room_id);
    return nullptr;
}
...
std::shared_ptr<Mobile> WorldManager::spawn_mobile_to_room(EntityId prototype_id, EntityId room_id) {
    ...
    if (!result) {
        logger->error("Failed to spawn mobile {} to room {}: {}", prototype_id, room_id, result.error().message);
        return nullptr;
    }
...
void WorldManager::despawn_mobile(EntityId mobile_id) { unregister_spawned_mobile(mobile_id); }
...
void WorldManager::initialize_weather_callbacks() {
    ...
    Weather().set_weather_change_callback(
        [logger](EntityId zone_id, const WeatherState &old_state, const WeatherState &new_state) {
            if (zone_id == INVALID_ENTITY_ID) {
                logger->info("Global weather changed from {} to {}", old_state.get_summary(), new_state.get_summary());
            } else {
                logger->info("Zone {} weather changed from {} to {}", zone_id, old_state.get_summary(),
                             new_state.get_summary());
            }
        });
        ...
        for (auto &actor : room->contents().actors) {
            if (actor && actor->is_alive()) {
                ...
                if (tick_result.hp_gained > 0 || tick_result.stamina_gained > 0) {
                    Log::game()->trace("Regen tick: {} +{} HP, +{} stamina", actor->name(), tick_result.hp_gained,
                                       tick_result.stamina_gained);
                }
...
Result<void> backup_world_data(const std::string &world_path, const std::string &backup_path) {
    try {
        ...
        std::filesystem::copy(world_path, backup_path, std::filesystem::copy_options::recursive);
...
Result<void> restore_world_data(const std::string &backup_path, const std::string &world_path) {
    try {
        ...
        std::filesystem::copy(backup_path, world_path, std::filesystem::copy_options::recursive);
...
Result<void> export_world(const WorldManager &world, const std::string &filename, std::string_view format) {
    if (format == "json") {
...
Result<void> import_world(WorldManager &world, const std::string &filename, std::string_view format) {
    if (format == "json") {
