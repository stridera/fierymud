...
ObjectTemplate::ObjectTemplate(const Object *obj)
    : id_(obj ? obj->id() : INVALID_ENTITY_ID), name_(obj ? std::string(obj->name()) : ""),
      keywords_(obj ? join_keywords(obj->keywords()) : ""), description_(obj ? std::string(obj->description()) : ""),
      weight_(obj ? obj->weight() : 0), value_(obj ? obj->value() : 0), level_(obj ? obj->level() : 0),
      type_(obj ? std::string(magic_enum::enum_name(obj->type())) : "") {}

MobileTemplate::MobileTemplate(const Mobile *mob)
    : id_(mob ? mob->id() : INVALID_ENTITY_ID), name_(mob ? std::string(mob->name()) : ""),
