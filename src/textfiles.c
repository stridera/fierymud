...
ACMD(do_reload)
{
  ...
  if (!str_cmp(arg, "all"))
    ...
    for (argument = any_one_arg(argument, arg); *arg;
  ...
  if (!reload_help && !HAS_FLAGS(files, NUM_TEXT_FILES)) {
    ...
    for (i = 0; i < NUM_TEXT_FILES; ++i)
      cprintf(ch, "%-11.11s%s", text_files[i].name,
              !((i + COLS) % 7) ? "\r\n" : "");
...
static EDITOR_FUNC(text_save) {
  ...
  if (edit->command == ED_EXIT_SAVE) {
    if ((fl = fopen(text->path, "w"))) {
      ...
      mprintf(L_STAT, LVL_GAMEMASTER,
              "(GC): %s edits file '%s' in tedit",
              GET_NAME(d->character), text->path);
      fclose(fl);
    }
    else
      mprintf(L_ERROR, LVL_GAMEMASTER,
              "SYSERR: Can't write file '%s' for tedit",
              text->path);
  }
