...
Result<std::shared_ptr<Player>> GameTestHarness::create_test_player(std::string_view name, int player_class,
...
Result<std::string> GameTestHarness::execute_command(std::shared_ptr<Player> player, std::string_view command_line) {
    ...
    if (result != CommandResult::Success) {
        return std::unexpected(
            Error{ErrorCode::InvalidCommand, fmt::format("Command failed with result: {}", static_cast<int>(result))});
    }

    return output;
}

std::shared_ptr<Room> GameTestHarness::get_room(EntityId room_id) const { return world_server_->get_room(room_id); }
