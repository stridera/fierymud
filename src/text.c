 ...
 */
static void megabuf_done()
{
  megabuf_tail += strlen(megabuf_tail) + 1;
}
...
void format_text(char **ptr_string, int mode, struct descriptor_data *d, int maxlen) {
  ...
  if ((int)(strlen(formatted)) > maxlen) formatted[maxlen] = '\0';
...
{
  screen_buf sb;
  CREATE(sb, struct screen_buf_t, 1);
  sb->buf = NULL;
  sb->lines = NULL;
...
static void sb_compile_lines(screen_buf sb, size_t start_line)
{
  ...
  bool cap_next = TRUE, cap_next_next = FALSE,
       hyphenated = FALSE, was_hyphenated = FALSE;
...
const char *sb_get_buffer(screen_buf sb)
{
  return sb->buf;
}

size_t sb_get_capacity(screen_buf sb)
{
  return sb->capacity;
}

size_t sb_get_length(screen_buf sb)
{
  return sb->length;
}

size_t sb_get_lines(screen_buf sb)
{
  return sb->line_count;
}

size_t sb_get_width(screen_buf sb)
{
  return sb->line_width;
}

size_t sb_get_first_indentation(screen_buf sb)
{
  return sb->first_indent;
}

size_t sb_get_other_indentation(screen_buf sb)
{
  return sb->other_indent;
}

const char *sb_get_line(screen_buf sb, size_t line)
{
  return sb->lines[line];
}
...
void sb_append(screen_buf sb, const char *msg, ...)
{
  ...
  *buffer = ' ';
  ...
  for (len = strlen(buffer) + 1;
       len + sb->length >= sb->capacity;
       sb_increase_buffer(sb, sb->capacity));
