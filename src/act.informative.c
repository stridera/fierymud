    ...
    *buf = '\0';
    ...
    if (!*arg) {
        ...
        */
        ...
        if (is_abbrev(arg, "nourishment")) {
            if (GET_RACE(ch) == RACE_ARBOREAN) {
                if (!GET_COOLDOWN(ch, CD_INNATE_NOURISHMENT)) {
                    call_magic(ch, ch, 0, SPELL_NOURISHMENT, GET_LEVEL(ch), CAST_SPELL);
                    if (!ROOM_FLAGGED(IN_ROOM(ch), ROOM_NOMAGIC))
                        SET_COOLDOWN(ch, CD_INNATE_NOURISHMENT, 24 MUD_HR);
                } else
                    send_to_char("You're too tired to replenish yourself right now.\r\n", ch);
                return;
            }
