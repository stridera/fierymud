...
bool senses_living(struct char_data *ch, struct char_data *vict, int basepct)
{
    ...
    * and life is what this ability detects. Here we adjust the sensitivity accordingly. */
...
static void print_note_to_char(struct obj_data *obj, struct char_data *ch)
{
   ...
   if (obj->action_description)
      cprintf(ch, "%s%s",
                  obj->ex_description ? "There is something written upon it:\r\n\r\n" : "",
...
void print_obj_to_char(struct obj_data *obj, struct char_data *ch, int mode, char *additional_args)
{
   ...
   case SHOW_FULL_DESC:
      if (obj->ex_description &&
          !(GET_OBJ_TYPE(obj) == ITEM_BOARD && is_number(additional_args)))
         cprintf(ch, "%s", obj->ex_description->description);
...
void list_obj_to_char(struct obj_data *list, struct char_data *ch, int mode)
{
   ...
   #define STRINGS_MATCH(x, y, member) \
      ((x)->member == (y)->member || \
       !strcmp((x)->member, (y)->member))
   #define OBJECTS_MATCH(x, y) \
      ((x)->item_number == (y)->item_number && \
       (mode == SHOW_LONG_DESC ? \
         STRINGS_MATCH((x), (y), description) : \
         STRINGS_MATCH((x), (y), short_description)))
...
{
   if (GET_POS(targ) < 0 || GET_POS(targ) >= NUM_POSITIONS ||
         GET_STANCE(targ) < 0 || GET_STANCE(targ) >= NUM_STANCES) {
   ...
   if (IS_WATER(IN_ROOM(targ))) {
      switch (GET_POS(targ)) {
      case POS_SITTING:   cprintf(ch, " floating here.");                         break;
      case POS_STANDING: cprintf(ch, " floating here in the water.");      break;
      case POS_FLYING:    cprintf(ch, " hovering here above the water."); break;
      default:                cprintf(ch, " floating here, %s.",
   ...
   cprintf(ch, " is %s here%s%s.",
               ...
               GET_STANCE(targ) == STANCE_ALERT ? "" :
               GET_STANCE(targ) == STANCE_RESTING &&
                  GET_POS(targ) > POS_KNEELING ? "at ease" :
               stance_types[(int) GET_STANCE(targ)]);
}
...
static void print_char_long_desc_to_char(struct char_data *targ, struct char_data *ch)
{
   ...
   if (IS_NPC(targ) && !MOB_FLAGGED(targ, MOB_PLAYER_PHANTASM) &&
         GET_LDESC(targ) && GET_POS(targ) == GET_DEFAULT_POS(targ) &&
         GET_STANCE(targ) != STANCE_FIGHTING &&
         !EFF_FLAGGED(targ, EFF_MINOR_PARALYSIS) &&
         !EFF_FLAGGED(targ, EFF_MAJOR_PARALYSIS)) {
   ...
   if (IS_NPC(targ) && !MOB_FLAGGED(targ, MOB_PLAYER_PHANTASM))
      ...
      snprintf(buf, sizeof(buf), "@0%s%s%s @W(@0%s@W)@0 @L(@0%s@L)@0",
                   GET_NAME(targ),
                   GET_TITLE(targ) ? " " : "",
                   GET_TITLE(targ) ? GET_TITLE(targ) : "",
                   RACE_ABBR(targ), SIZE_DESC(targ));

   cprintf(ch, "%s", CAP(buf));

   if (RIDDEN_BY(targ) && RIDDEN_BY(targ)->in_room == targ->in_room && !IS_NPC(targ))
      cprintf(ch, " is here, ridden by %s.",
                  RIDING(targ) == ch ? "you" : PERS(RIDDEN_BY(targ), ch));

   else if (RIDING(targ) && RIDING(targ)->in_room == targ->in_room)
      cprintf(ch, " is here, mounted upon %s.",
                  RIDING(targ) == ch ? "you" : PERS(RIDING(targ), ch));
      ...
      if (affected_by_spell(targ, SPELL_ENTANGLE))
         cprintf(ch, " is here, %s.",
                     EFF_FLAGGED(targ, EFF_MAJOR_PARALYSIS) ?
                     "held still by thick vines" : "entangled in a mass of vines");
      else
         cprintf(ch, " is standing here, completely motionless.");
   }

   else if (EFF_FLAGGED(targ, EFF_MESMERIZED))
      cprintf(ch, " is here, gazing carefully at a point in front of %s nose nose.",
                  HSHR(targ));
...
static void print_char_flags_to_char(struct char_data *targ, struct char_data *ch)
{
   ...
   if (!IS_NPC(targ) && !targ->desc && PRF_FLAGGED(ch, PRF_HOLYLIGHT))
      str_catf(buf, " (@r%s%s@0)",
                   targ->forward ? "switched: @0" : "linkless",
...
const char *status_string(int cur, int max, int mode)
{
   ...
   case STATUS_PHRASE:
   default:
      sprintf(retbuf[lastbuf], "%s %s%s%s%s" ANRM ".",
              css.pre, css.color, css.cond, *css.post ? " " : "", css.post);
      break;
   }
...
static void print_char_equipment_to_char(struct char_data *targ, struct char_data *ch)
{
   ...
   if (found) {
      ...
      for (j = 0; j < NUM_WEARS; j++)
         if (GET_EQ(targ, wear_order_index[j])
               && CAN_SEE_OBJ(ch, GET_EQ(targ, wear_order_index[j]))) {
...
static void print_char_spells_to_char(struct char_data *targ, struct char_data *ch)
{
   ...
   if (EFF_FLAGGED(ch, EFF_DETECT_MAGIC) || PRF_FLAGGED(ch, PRF_HOLYLIGHT)) {
      ...
      if (affected_by_spell(targ, SPELL_BLESS))
         act("The &8shimmering telltales&0 of a &3&bmagical blessing&0 flutter about $S head.&0",
      ...
      else if (affected_by_spell(targ, SPELL_LESSER_ENDURANCE) ||
                   affected_by_spell(targ, SPELL_ENDURANCE) ||
                   affected_by_spell(targ, SPELL_GREATER_ENDURANCE) ||
                   affected_by_spell(targ, SPELL_VITALITY) ||
   ...
   if (affected_by_spell(targ, SPELL_MIRAGE))
      act("&7$S image &1wavers&7 and &9&bshimmers&0&7 and is somewhat indistinct.&0", TRUE, ch, 0, targ, TO_CHAR);
   ...
   if (EFF_FLAGGED(targ, EFF_CONFUSION)) {
      if (GET_POS(targ) >= POS_KNEELING) {
         if (GET_STANCE(targ) >= STANCE_RESTING)
            act("$S eyes &5drift&0 and &3dart&0 comically about, and $E has trouble balancing.",
                  TRUE, ch, 0, targ, TO_CHAR);
         else
            act("$E seems to have trouble balancing.",
                  TRUE, ch, 0, targ, TO_CHAR);
      }
      else if (GET_STANCE(targ) >= STANCE_RESTING)
         act("$S eyes &5drift&0 and &3dart&0 comically about.",
               TRUE, ch, 0, targ, TO_CHAR);
   }
   ...
   if (EFF_FLAGGED(targ, EFF_MINOR_PARALYSIS) || EFF_FLAGGED(targ, EFF_MAJOR_PARALYSIS)) {
      ...
      if (affected_by_spell(targ, SPELL_ENTANGLE)) {
         if (EFF_FLAGGED(targ, EFF_MAJOR_PARALYSIS))
            act("&2$E is held fast by &bthick twining vines&0&2.&0",
                  TRUE, ch, 0, targ, TO_CHAR);
         else
            act("&2$E is entwined by a tangled mass of vines.&0",
                  TRUE, ch, 0, targ, TO_CHAR);
      }
      else
         act("&6$E is completely still, and shows no awareness of $S surroundings.&0",
               TRUE, ch, 0, targ, TO_CHAR);
   }
   else if (EFF_FLAGGED(targ, EFF_MESMERIZED))
...
void print_char_to_char(struct char_data *targ, struct char_data *ch, int mode)
{
   ...
   case SHOW_BASIC_DESC:
      ...
      cprintf(ch, "%s %s\r\n", CAP(buf),
              status_string(GET_HIT(targ), GET_MAX_HIT(targ), STATUS_PHRASE));
...
static void print_char_infra_to_char(struct char_data *targ, struct char_data *ch, int mode)
{
   ...
   if (RIDING(targ) && RIDING(targ)->in_room == targ->in_room)
      cprintf(ch, "@rThe red shape of a %s being is here, mounted upon %s%s%s.@0\r\n",
                  SIZE_DESC(targ),
                  RIDING(targ) == ch ? "you.@0" : "a ",
                  RIDING(targ) == ch ? "" : SIZE_DESC(RIDING(targ)),
                  RIDING(targ) == ch ? "" : "-sized creature");
   else
      cprintf(ch, "@rThe red shape of a %c%s&0&1 living being%s is here.&0\r\n",
                  LOWER(*SIZE_DESC(targ)), SIZE_DESC(targ) + 1,
                  EFF_FLAGGED(targ, EFF_INFRAVISION) ? " &bwith glowing red eyes&0&1" : "");
}
...
void list_char_to_char(struct char_data *list, struct char_data *ch, int mode)
{
   ...
   #define STRINGS_MATCH(x, y, member) \
      ((x)->player.member == (y)->player.member || \
       !strcmp((x)->player.member, (y)->player.member))
   #define CHARS_MATCH(x, y) \
      (GET_MOB_VNUM(x) == GET_MOB_VNUM(y) && \
       FIGHTING(x) == FIGHTING(y) && \
       CASTING(x) == CASTING(y) && \
       (mode == SHOW_SHORT_DESC ? \
         STRINGS_MATCH((x), (y), short_descr) : \
         STRINGS_MATCH((x), (y), long_descr)) && \
       ((CAN_SEE(ch, (x)) && CAN_SEE(ch, (y))) || \
         (CAN_SEE_BY_INFRA(ch, (x)) && CAN_SEE_BY_INFRA(ch, (y)))))
   ...
   for (i = list; i; i = i->next_in_room) {
      ...
      if (RIDDEN_BY(i) && RIDDEN_BY(i)->in_room == i->in_room && IS_NPC(i) &&
...
ACMD(do_viewdam)
{
   ...
   sprintf(buf2, "%s%sSPELL                         ID %sNPC%s%s- DI FA RF   %sPC%s%s- DI FA %sB%s%s-LM   MAX   ON/OFF %sINT%s%s- ON/OFF%s\r\n",
                     ...
                     if (str_cmp(skills[i].name, "!UNUSED!"))
                        {
                           cprintf(ch, "%s%s%-22s%-8d%-3d%-3d%-8d%-3d%-5d%-4d%-5d%-12s%s%s \r\n",buf2,
                                       grn, (skills[i].name), SD_SPELL(i), SD_NPC_NO_DICE(i), SD_NPC_NO_FACE(i),
                                       SD_NPC_REDUCE_FACTOR(i), SD_PC_NO_DICE(i), SD_PC_NO_FACE(i), SD_LVL_MULT(i), SD_BONUS(i),
         ...
         for (i = 1; i <= MAX_SPELLS; i++)
            {
               if (str_cmp(skills[i].name, "!UNUSED!")) {
                  cprintf(ch, "%s%-22s%-8d%-3d%-3d%-8d%-3d%-5d%-4d%-3d%s%s\r\n",
                              grn, skills[i].name, SD_SPELL(i),   SD_NPC_NO_DICE(i), SD_NPC_NO_FACE(i)
                              , SD_NPC_REDUCE_FACTOR(i), SD_PC_NO_DICE(i), SD_PC_NO_FACE(i)
                              , SD_LVL_MULT(i), SD_BONUS(i), (SD_USE_BONUS(i) ? "TRUE" : "FALSE")
                              , nrm
                              );
               }
            }
         ...
         for (i=1;i<= MAX_SPELLS;i++)
            {
               ...
               sprintf(buf2, "%s%s%-22s%-8d%-3d%-3d%-8d%-3d%-5d%-4d%-3d%s%s\r\n",buf2,
                           grn, (skills[i].name), SD_SPELL(i), SD_NPC_NO_DICE(i),
                           SD_NPC_NO_FACE(i), SD_NPC_REDUCE_FACTOR(i),
...
static int search_for_doors(struct char_data *ch, char *arg)
{
   ...
   for (door = 0; door < NUM_OF_DIRS; ++door)
      if (CH_EXIT(ch, door) && CH_EXIT(ch, door)->to_room != NOWHERE &&
         ...
         if (GET_LEVEL(ch) >= LVL_IMMORT ||
               (CH_EXIT(ch, door)->keyword && arg &&
               isname(arg, CH_EXIT(ch, door)->keyword)) ||
               GET_INT(ch) > number(0, 200)) {
            sprintf(buf, "&8You have found%s hidden %s %s.&0",
                  CH_EXIT(ch, door)->keyword &&
                    isplural(CH_EXIT(ch, door)->keyword) ? "" : " a",
                  CH_EXIT(ch, door)->keyword ? "$F" : "door",
                  dirpreposition[door]);
      ...
      if (!found_something)
         for (j = world[ch->in_room].people;
            ...
            if (IS_HIDDEN(j) && j != ch) {
               ...
               /* The searcher COULD see this character if it weren't hidden. Will the searcher
                * discover it? */
...
void print_room_to_char(room_num room_nr, struct char_data *ch, bool ignore_brief)
{
   if (IS_DARK(room_nr) && !CAN_SEE_IN_DARK(ch)) {
      ...
      if (EFF_FLAGGED(ch, EFF_INFRAVISION))
         list_char_to_char(world[room_nr].people, ch,
               SHOW_LONG_DESC | SHOW_FLAGS | SHOW_SKIP_SELF | SHOW_STACK);
      return;
   }
   ...
   if (!PRF_FLAGGED(ch, PRF_BRIEF) || ignore_brief ||
         ROOM_FLAGGED(room_nr, ROOM_DEATH))
   ...
   list_obj_to_char(world[room_nr].contents, ch, SHOW_LONG_DESC |
         SHOW_FLAGS | SHOW_STACK | SHOW_NO_FAIL_MSG);
   list_char_to_char(world[room_nr].people, ch, SHOW_LONG_DESC |
         SHOW_FLAGS | SHOW_SKIP_SELF | SHOW_STACK);
}
...
void look_in_direction(struct char_data *ch, int dir)
{
   ...
   if (ROOM_EFF_FLAGGED(CH_NROOM(ch), ROOM_EFF_ISOLATION) &&
      ...
      cprintf(ch, "%s", exit && EXIT_IS_DESCRIPTION(exit) ?
      ...
      if (EXIT_IS_HIDDEN(exit) && GET_LEVEL(ch) < LVL_IMMORT) {
         ...
         if (EFF_FLAGGED(ch, EFF_FARSEE) || GET_CLASS(ch) == CLASS_RANGER)
            do_farsee(ch, dir);
                  ...
                  ROOM_EFF_FLAGGED(exit->to_room, ROOM_EFF_CIRCLE_FIRE))
            cprintf(ch, "&1&8The edge of a circle of fire burns wildly in this direction.&0\r\n");
...
static const char *describe_fullness(int remaining, int capacity)
{
   ...
   switch ((remaining * 3) / capacity) {
      case 0:   fullness = "less than half "; break;
      case 1:   fullness = "about half ";       break;
      case 2:   fullness = "more than half "; break;
      default: fullness = "";
   }
   return fullness;
}
   ...
   if (!*arg)
      ...
      if (GET_OBJ_VAL(obj, VAL_DRINKCON_REMAINING) <= 0)
         ...
         cprintf(ch, "It's %sfull of a %s liquid.\r\n",
                     describe_fullness(GET_OBJ_VAL(obj, VAL_DRINKCON_REMAINING),
                                                GET_OBJ_VAL(obj, VAL_DRINKCON_CAPACITY)),
...
static void do_farsee(struct char_data *ch, int dir)
{
   ...
   while (CAN_GO(ch, dir)) {
      ...
      cprintf(ch, "\r\n&0&6You extend your vision %s%s%s%s.&0\r\n",
                  ...
                  (dir == UP || dir == DOWN) ? "" : "to the ",
                  dirs[dir],
                  (dir == UP || dir == DOWN) ? "wards" : "");
...
void identify_obj(struct obj_data *obj, struct char_data *ch, int location)
{
  ...
  if (location)
    cprintf(ch, "You closely examine %s %s:\r\n"
                "Item type: %s\r\n",
            obj->short_description, relative_location_str(location),
            OBJ_TYPE_NAME(obj));
  else
    ...
    if ((i = obj->obj_flags.wear_flags &
             (~ITEM_WEAR_WIELD & ~ITEM_WEAR_2HWIELD & ~ITEM_WEAR_TAKE))) {
    ...
    case ITEM_POTION:
       if ((GET_OBJ_VAL(obj, VAL_SCROLL_SPELL_1) < 1) &&
            (GET_OBJ_VAL(obj, VAL_SCROLL_SPELL_2) < 1) &&
    ...
    case ITEM_WAND:
    case ITEM_STAFF:
      cprintf(ch, "This %s casts: %s\r\n"
                  "It has %d maximum charge%s and %d remaining.\r\n",
              OBJ_TYPE_NAME(obj),
              skills[GET_OBJ_VAL(obj, VAL_WAND_SPELL)].name,
              GET_OBJ_VAL(obj, VAL_WAND_MAX_CHARGES),
              GET_OBJ_VAL(obj, VAL_WAND_MAX_CHARGES) == 1 ? "" : "s",
    ...
    for (i = 0; i < MAX_OBJ_APPLIES; i++)
      if (obj->applies[i].location != APPLY_NONE)
        cprintf(ch, "   Apply: %s\r\n", format_apply(
                obj->applies[i].location,
                obj->applies[i].modifier));
}
...
ACMD(do_identify)
{
   ...
   if (GET_LEVEL(ch) >= LVL_GOD ||
        (GET_CLASS(ch) == CLASS_THIEF && GET_OBJ_LEVEL(obj) <= GET_LEVEL(ch) + 11)) {
   ...
   if (GET_CLASS(ch) == CLASS_THIEF || GET_OBJ_LEVEL(obj) < GET_LEVEL(ch) + 11) {
      ...
      cprintf(ch, "%s%s%s look%s like %s.\r\n", buf,
              bits == FIND_OBJ_INV ? "" : " ",
              bits == FIND_OBJ_INV ? "" : relative_location_str(bits),
              isplural(obj->name) ? "" : "s", OBJ_TYPE_DESC(obj));
      return;
   }
...
ACMD(do_weather)
{
   *buf2 = '\0';
   ...
   strcat(buf2, wind_message(zone_table[IN_ZONE_RNUM(ch)].wind_speed,
                                          zone_table[IN_ZONE_RNUM(ch)].wind_speed));
   ...
   strcat(buf2, precipitation_message(&zone_table[IN_ZONE_RNUM(ch)],
                                                       zone_table[IN_ZONE_RNUM(ch)].precipitation));

   cprintf(ch, "%sIt is %s.\r\n", buf2,
               seasons[hemispheres[IN_HEMISPHERE(ch)].season]);
}
...
static void cat_mortal_wholine(
      char *mbuf,
      const char *title,
      const struct char_data *ch,
      const bool show_as_anon,
      const bool show_area_in,
      const bool show_full_class,
      const bool show_color_nameflags)
{
   if (show_as_anon)
      ...
      sprintf(mbuf, "%s&0[%s%c%s] %s%s&0 %s%s&0&9&b(&0%s&0&9&b)&0", mbuf,
            show_full_class ? CLASS_WIDE(ch) : CLASS_ABBR(ch),
            PRF_FLAGGED(ch, PRF_ANON) ? '*' : ' ',
...
char *WHO_USAGE =
   "Usage: who [minlev-maxlev] [-qrzw] [-n name] [-c classes]\r\n";
...
ACMD(do_who) {
   ...
   buf[0] = buf1[0] = buf2[0] = arg[0] = Imm_buf_title[0] = Imm_buf[0] =
       Mort_buf_title[0] = Mort_buf[0] = 0;
      ...
      if (isdigit(*arg)) {
         ...
         switch (mode) {
            ...
            case 'r':
               ...
               strcpy(buf, buf1);
               break;
            /* 'w' - show where people are, by putting their area name after their name */
            case 'w':
               who_where = TRUE;
               strcpy(buf, buf1);
               break;
         ...
         if (GET_LEVEL(wch) >= LVL_IMMORT) {
            sprintf(buf, "%s%s[%s] %s %s",buf, CLRLV(ch, ADAR, C_SPR),
                  GET_WIZ_TITLE(wch) ? GET_WIZ_TITLE(wch) : WizLevels[GET_LEVEL(wch)-LVL_IMMORT],
                  GET_NAME(wch), GET_TITLE(wch) ? GET_TITLE(wch) : "");
         } else {
            cat_mortal_wholine(buf, buf2, wch,
                  PRF_FLAGGED(wch, PRF_ANON) && GET_LEVEL(ch) < LVL_IMMORT && wch != ch,  /* show_as_anon */
                  ((!pk_allowed && !PRF_FLAGGED(wch, PRF_ANON)) ||
                        wch == ch || GET_LEVEL(ch) >= LVL_IMMORT) && who_where, /* show_area_in */
  ...
  strcpy(Mort_buf_title,"*** Active TEST players on FieryTESTMud:\r\n\r\n");
   ...
   for (d = descriptor_list; d; d = d->next) {
      ...
      /* The title can take up a maximum of 45 spaces.  That's spaces, not characters,
       * so the following loops examine &X color codes as they try to put the
       * longest title possible that doesn't exceed 45. */
      c = 0;
      for (b = 0; a[b] != '\0'; b++) {
      ...
      if (GET_LEVEL(wch) >= LVL_IMMORT) {
         ...
         if (GET_LEVEL(ch) >= LVL_IMMORT || ch == wch ||
             (!PRF_FLAGGED(wch, PRF_ANON) && !pk_allowed)) {
            show_area_in = who_where;
         } else if (PRF_FLAGGED(wch, PRF_ANON)) {
            show_as_anon = TRUE;
         }

         cat_mortal_wholine(Mort_buf, buf2, wch, show_as_anon,
               show_area_in, showclass != CLASS_UNDEFINED, GET_LEVEL(ch) >= LVL_IMMORT);
      ...
      if (GET_LEVEL(ch) >= LVL_IMMORT) {
         switch (STATE(d)) {
            case CON_OEDIT: strcat(buf, " (Object Edit)"); break;
            case CON_MEDIT: strcat(buf, " (Mobile Edit)"); break;
            case CON_ZEDIT: strcat(buf, " (Zone Edit)"); break;
            case CON_SEDIT: strcat(buf, " (Shop Edit)"); break;
            case CON_REDIT: strcat(buf, " (Room Edit)"); break;
            case CON_TRIGEDIT: strcat(buf, " (Trigger Edit)"); break;
            case CON_HEDIT: strcat(buf, " (Help Edit)"); break;
            case CON_SDEDIT: strcat(buf, " (Skill Edit)"); break;
            case CON_GEDIT: strcat(buf, " (Grant Edit)"); break;
         }
      }
   ...
   if (Wizards)
      sprintf(buf, "\r\nThere %s %d visible deit%s%s", (Wizards == 1 ? "is" : "are"),
         Wizards, (Wizards == 1 ? "y" : "ies"), (Mortals ? " and there" : "."));
...
#define USERS_FORMAT \
"format: users [-l minlevel[-maxlevel]] [-n name] [-h host] [-c classlist]\r\n              [-o] [-p] [-i]\r\n"
...
ACMD(do_users)
{
   ...
   *userbuf = '\0';
   ...
   strcpy(line,
             "Soc  Username    User's Host   Idl  Login        RoomNo/RoomName      Position\r\n");
   strcat(line,
             "--- ---------- --------------- --- -------- ------------------------- ---------\r\n");
   strcpy(userbuf, line);
   ...
   for (d = descriptor_list; d; d = d->next) {
      ...
      *(timeptr + 8) = '\0';
      ...
      if (d->original && !d->connected)
         sprintf(idletime, "%3d", d->original->char_specials.timer *
      ...
      if (d->character && GET_NAME(d->character)) {
         if (d->original) {
            ...
            sprintf(line, format, d->desc_num,
                        nametrun, hostnum, idletime, timeptr,
                        room, position);
         } else {
            if ((d->character->in_room != NOWHERE) && (d->connected == 0)) {
                sprintf(roomstuff, "%d/%s",
                            world[d->character->in_room].vnum,
         ...
         sprintf(line, format, d->desc_num, "  ---   ",
                     hostnum, idletime, timeptr, " At menu screen or Other ",
                     "   -None   ");
   ...
   if (!*arg) {
      ...
      for (i = character_list; i; i = i->next)
         if (world[i->in_room].zone == world[ch->in_room].zone && CAN_SEE(ch, i) &&
               (i->in_room != NOWHERE) && isname(arg, GET_NAMELIST(i))) {
            sprintf(buf, "%-25s - %s\r\n", GET_NAME(i), world[i->in_room].name);
            send_to_char(buf, ch);
            return;
         }
            ...
            if (i && CAN_SEE(ch, i) && (i->in_room != NOWHERE)) {
               if (d->original)
                  ...
                  sprintf(buf, "%-20s - [%5d] %s\r\n", GET_NAME(i),
                              world[i->in_room].vnum, world[i->in_room].name);
               send_to_char(buf, ch);
            }
         }
   } else {
...
ACMD(do_consider)
{
   ...
   if (GET_LEVEL(victim) < LVL_IMMORT && IS_NPC(victim) &&
         MOB_FLAGGED(victim, MOB_MOUNTABLE)) {
      mountmsg = 1;
   ...
   for (i = 0; i < COMMANDS_LIST_COLUMNS; i++) {
      ...
      for (j = 0; j < column_height; j++) {
         ...
         for (cont = 1;cmd_num < num_of_cmds && cont; cmd_num++) {
            ...
            if (cmd_info[v].minimum_level >= 0 &&
                  can_use_command(vict, v) &&
...
const char *exp_bar(struct char_data *ch, int length, int gradations, int sub_gradations, bool color) {
   ...
   for (i = 0; i < length; ++i) {
      if (GET_LEVEL(ch) >= LVL_IMMORT ||
            (GET_LEVEL(ch) == LVL_MAX_MORT && current > total))
         strcat(bar, "-");
      else if (i == distance) {
         if (color)
            strcat(bar, FCYN);
...
static void show_abilities(struct char_data *ch, struct char_data *tch, bool verbose) {
   if (verbose)
      sprintf(buf, "Str: &3%s&0    Int: &3%s&0     Wis: &3%s&0\r\n"
                 "Dex: &3%s&0    Con: &3%s&0     Cha: &3%s&0\r\n",
            ability_message(GET_VIEWED_STR(tch)),
            ability_message(GET_VIEWED_INT(tch)),
            ability_message(GET_VIEWED_WIS(tch)),
            ability_message(GET_VIEWED_DEX(tch)),
            ability_message(GET_VIEWED_CON(tch)),
            ability_message(GET_VIEWED_CHA(tch)));
   else
      sprintf(buf, "Str: &3&b%d&0(&3%d&0)     %s%s"
                 "Int: &3&b%d&0(&3%d&0)     %s%s"
                 "Wis: &3&b%d&0(&3%d&0)\r\n"
                 "Dex: &3&b%d&0(&3%d&0)     %s%s"
                 "Con: &3&b%d&0(&3%d&0)     %s%s"
                 "Cha: &3&b%d&0(&3%d&0)\r\n",
            GET_VIEWED_STR(tch), GET_NATURAL_STR(tch),
              GET_VIEWED_STR(tch) < 100 ? " " : "",
              GET_NATURAL_STR(tch) < 100 ? " " : "",
            GET_VIEWED_INT(tch), GET_NATURAL_INT(tch),
              GET_VIEWED_INT(tch) < 100 ? " " : "",
              GET_NATURAL_INT(tch) < 100 ? " " : "",
            GET_VIEWED_WIS(tch), GET_NATURAL_WIS(tch),
            GET_VIEWED_DEX(tch), GET_NATURAL_DEX(tch),
              GET_VIEWED_DEX(tch) < 100 ? " " : "",
              GET_NATURAL_DEX(tch) < 100 ? " " : "",
            GET_VIEWED_CON(tch), GET_NATURAL_CON(tch),
              GET_VIEWED_CON(tch) < 100 ? " " : "",
              GET_NATURAL_CON(tch) < 100 ? " " : "",
            GET_VIEWED_CHA(tch), GET_NATURAL_CHA(tch));
  send_to_char(buf, ch);
}

static void show_points(struct char_data *ch, struct char_data *tch, bool verbose) {
  sprintf(buf, "Hit points: &1&b%d&0[&1%d&0] (&3%d&0)   Moves: &2&b%d&0[&2%d&0] (&3%d&0)\r\n",
          GET_HIT(tch), GET_MAX_HIT(tch), GET_BASE_HIT(tch),
          GET_MOVE(tch), GET_MAX_MOVE(tch), natural_move(tch));

  if (verbose)
    sprintf(buf, "%sArmor class: &3&8%s&0 (&3&8%d&0)  ",
            buf, armor_message(GET_AC(tch) + 5 * monk_weight_penalty(tch)),
            GET_AC(tch) + 5 * monk_weight_penalty(tch));
  else
     sprintf(buf, "%sArmor class: &3&8%d&0  ",
             buf, GET_AC(tch) + 5 * monk_weight_penalty(tch));

  if (verbose)
     sprintf(buf, "%sHitroll: &3&8%s&0 (&3&8%d&0)  "
     ...
     sprintf(buf, "%sHitroll: &3&b%d&0  Damroll: &3&b%d&0",
          buf, GET_HITROLL(tch) - monk_weight_penalty(tch),
          GET_DAMROLL(tch) - monk_weight_penalty(tch));
...
static void show_alignment(struct char_data *ch, struct char_data *tch, bool verbose) {
  if (verbose)
    sprintf(buf, "Alignment: %s%s&0 (%s%d&0)  ",
            IS_EVIL(tch) ? "&1&b" : (IS_GOOD(tch) ? "&3&b" : "&2&b"),
            align_message(GET_ALIGNMENT(tch)),
            IS_EVIL(tch) ? "&1&b" : (IS_GOOD(tch) ? "&3&b" : "&2&b"),
            GET_ALIGNMENT(tch));
  else
    sprintf(buf, "Alignment: %s%d&0  ",
            IS_EVIL(tch) ? "&1&b" : (IS_GOOD(tch) ? "&3&b" : "&2&b"),
            GET_ALIGNMENT(tch));
  send_to_char(buf, ch);
}
...
static void show_load(struct char_data *ch, struct char_data *tch, bool verbose) {
  if (verbose)
    sprintf(buf, "Encumbrance: &3&8%s&0 (&3&8%.2f&0/&3%d&0 lb)  ",
            (CURRENT_LOAD(tch) >= 0 && CURRENT_LOAD(tch) <= 10) ?
              carry_desc[CURRENT_LOAD(tch)] : "Your load is ridiculous!",
            IS_CARRYING_W(tch), CAN_CARRY_W(tch));
  else
    sprintf(buf, "Encumbrance: &3&8%.2f&0/&3%d&0 lb  ",
            IS_CARRYING_W(tch), CAN_CARRY_W(tch));
  send_to_char(buf, ch);
}
...
static void show_saves(struct char_data *ch, struct char_data *tch, bool verbose) {
  if (verbose)
    sprintf(buf, "Saving throws: PAR[&3&8%s&0/&3&8%d&0] "
                 "ROD[&3&8%s&0/&3&8%d&0] PET[&3&8%s&0/&3&8%d&0] "
                 "BRE[&3&b%s&0/&3&8%d&0] SPE[&3&b%s&0/&3&8%d&0]\r\n",
            save_message(GET_SAVE(tch, 0)), GET_SAVE(tch, 0),
            save_message(GET_SAVE(tch, 1)), GET_SAVE(tch, 1),
            save_message(GET_SAVE(tch, 2)), GET_SAVE(tch, 2),
            save_message(GET_SAVE(tch, 3)), GET_SAVE(tch, 3),
            save_message(GET_SAVE(tch, 4)), GET_SAVE(tch, 4));
  else
    sprintf(buf, "Saving throws: PAR[&3&b%d&0]  ROD[&3&b%d&0]  "
                 "PET[&3&b%d&0]  BRE[&3&b%d&0]  SPE[&3&b%d&0]\r\n",
          GET_SAVE(tch, 0), GET_SAVE(tch, 1), GET_SAVE(tch, 2),
          GET_SAVE(tch, 3), GET_SAVE(tch, 4));
  send_to_char(buf, ch);
}

#define COIN_FMT "%s%d " ANRM "%s"
#define PURSE_ELEMS(ch, coin) \
   COIN_COLOR(coin), GET_PURSE_COINS(ch, coin), capitalize(COIN_NAME(coin))
#define BANK_ELEMS(ch, coin) \
   COIN_COLOR(coin), GET_ACCOUNT_COINS(ch, coin), capitalize(COIN_NAME(coin))

static void show_coins(struct char_data *ch, struct char_data *tch) {
   cprintf(ch,
      "Coins carried: " COIN_FMT ", " COIN_FMT ", "
                        COIN_FMT ", and " COIN_FMT EOL
      "Coins in bank: " COIN_FMT ", " COIN_FMT ", "
                        COIN_FMT ", and " COIN_FMT EOL,
      PURSE_ELEMS(tch, PLATINUM), PURSE_ELEMS(tch, GOLD),
      PURSE_ELEMS(tch, SILVER), PURSE_ELEMS(tch, COPPER),
      BANK_ELEMS(tch, PLATINUM), BANK_ELEMS(tch, GOLD),
      BANK_ELEMS(tch, SILVER), BANK_ELEMS(tch, COPPER));
}
...
static void show_conditions(struct char_data *ch, struct char_data *tch, bool verbose) {
  ...
  if (verbose) {
    ...
    sprintf(buf, "Hunger: %d%s  Thirst: %d%s  Drunkenness: %d%s\r\n",
            GET_COND(tch, FULL) < 0 ? 0 : 24 - GET_COND(tch, FULL),
            GET_COND(tch, FULL) < 0 ? "ff" : "",
            GET_COND(tch, THIRST) < 0 ? 0 : 24 - GET_COND(tch, THIRST),
            GET_COND(tch, THIRST) < 0 ? "ff" : "",
            GET_COND(tch, DRUNK) < 0 ? 0 : GET_COND(tch, DRUNK),
            GET_COND(tch, DRUNK) < 0 ? "ff" : "");
  send_to_char(buf, ch);
}
...
ACMD(do_score)
{
   ...
   str_catf(buf, "Level: @Y%d@0%s  Class: %s",
        GET_LEVEL(tch),
        IS_STARSTAR(tch) ? buf1 : "",
        CLASS_FULL(tch));

  sprinttype(GET_SEX(tch), genders, buf1);
  if (GET_COMPOSITION(tch) != COMP_FLESH ||
      BASE_COMPOSITION(tch) != COMP_FLESH ||
      GET_LIFEFORCE(tch) != LIFE_LIFE) {
    if (GET_COMPOSITION(tch) == BASE_COMPOSITION(tch))
      *buf2 = '\0';
      ...
      sprintf(buf2, "%s%c%s",
              compositions[BASE_COMPOSITION(tch)].color,
              UPPER(*(compositions[BASE_COMPOSITION(tch)].name)),
              compositions[BASE_COMPOSITION(tch)].name + 1);
    str_catf(buf, "  Size: &3&8%c%s&0  Sex: &3&8%s&0\r\n"
                  "Race: %s  Life force: %s%c%s&0  "
                  "Composition: %s%c%s&0%s%s&0%s\r\n",
             UPPER(*SIZE_DESC(tch)), SIZE_DESC(tch) + 1, buf1,
             RACE_ABBR(tch),
             LIFEFORCE_COLOR(tch), UPPER(*LIFEFORCE_NAME(tch)), LIFEFORCE_NAME(tch) + 1,
             COMPOSITION_COLOR(tch), UPPER(*COMPOSITION_NAME(tch)), COMPOSITION_NAME(tch) + 1,
             *buf2 ? "(" : "", buf2, *buf2 ? ")" : "");
  }
  else
    str_catf(buf, "  Race: %s  Size: &3&8%c%s&0  Sex: &3&8%s&0\r\n",
             RACE_ABBR(tch), UPPER(*SIZE_DESC(tch)), SIZE_DESC(tch) + 1,
             buf1);

  str_catf(buf, "Age: &3&b%d&0&3 year%s&0, &3&b%d&0&3 month%s&0  "
                "Height: &3&b%s&0  Weight: &3&b%s&0\r\n",
           age(tch).year, age(tch).year == 1 ? "" : "s",
           age(tch).month, age(tch).month == 1 ? "" : "s",
  ...
  switch (GET_STANCE(tch)) {
    case STANCE_DEAD:            str_cat(buf, "&9&bDead&0\r\n"); break;
    case STANCE_MORT:            str_cat(buf, "&1Mortally wounded&0\r\n"); break;
    case STANCE_INCAP:           str_cat(buf, "&1&bIncapacitated&0\r\n"); break;
    case STANCE_STUNNED:         str_cat(buf, "&6&bStunned&0\r\n"); break;
    case STANCE_SLEEPING:
      if (IN_ROOM(tch) != NOWHERE && IS_WATER(IN_ROOM(tch)))
        str_cat(buf, "&4Sleeping&0\r\n");
      else switch (GET_POS(tch)) {
        case POS_STANDING: str_cat(buf, "&3Standing here sleeping&0\r\n"); break;
        case POS_KNEELING: str_cat(buf, "&3Kneeling here sleeping&0\r\n"); break;
        case POS_SITTING:  str_cat(buf, "&3Sitting here sleeping&0\r\n"); break;
        case POS_PRONE:    str_cat(buf, "&3Sleeping&0\r\n"); break;
        default:           str_cat(buf, "&1&8Invalid&0\r\n"); break;
      }
      break;
    case STANCE_RESTING:
      if (IN_ROOM(tch) != NOWHERE && IS_WATER(IN_ROOM(tch)))
        ...
        case POS_STANDING: str_cat(buf, "&3Standing here relaxed&0\r\n"); break;
        default:           str_cat(buf, "&3&bResting&0\r\n"); break;
      }
      break;
    case STANCE_ALERT:
      if (IN_ROOM(tch) != NOWHERE && IS_WATER(IN_ROOM(tch)))
        ...
        case POS_STANDING: str_cat(buf, "&3Standing&0\r\n"); break;
        case POS_KNEELING: str_cat(buf, "&3Kneeling&0\r\n"); break;
        case POS_SITTING:  str_cat(buf, "&3&bSitting&0\r\n"); break;
        case POS_PRONE:    str_cat(buf, "&3Lying alert&0\r\n"); break;
        default:           str_cat(buf, "&1&8Invalid&0\r\n"); break;
      }
      break;
    case STANCE_FIGHTING:     str_cat(buf, "&1&bFighting&0\r\n"); break;
    default:                  str_cat(buf, "&1&8Invalid&0\r\n"); break;
  }
  ...
  playing_time = real_time_passed((time(0) - REAL_CHAR(tch)->player.time.logon) +
                                  REAL_CHAR(tch)->player.time.played, 0);
...
ACMD(do_spells)
{
   ...
   int i, j, k, circle, skillnum, sphere, max_vis_circle,
         xcircle = 0, numspellsknown = 0;
   ...
   for (k = 0; k <= TOP_SKILL; ++k) {
      ...
      if (!IS_SPELL(i))
         continue;
      if (skills[i].min_level[GET_CLASS(tch)] < LVL_IMMORT &&
            GET_SKILL(tch, i) > 0) {
         circle = (skills[i].min_level[GET_CLASS(tch)] - 1 ) / 8;
         for (j = 0;
                  circle_spells[circle][j] &&
                  j < MAX_SPELLS_PER_CIRCLE &&
                  circle < max_vis_circle; ++j);
   ...
   if (xcircle) {
       if (tch == ch)
         ...
         pprintf(ch, "%s knows of the following %s in &4&bcircle %d&0:" EOL EOL,
                  CAP(GET_NAME(tch)),
                  numspellsknown == 1 ? "spell" : "spells", xcircle);
   } else {
      if (tch == ch)
         pprintf(ch, "You know of the following %s:" EOL EOL,
                  numspellsknown == 1 ? "spell" : "spells");
      else
         pprintf(ch, "%s knows of the following %s:" EOL EOL,
                  GET_NAME(tch),
                  numspellsknown == 1 ? "spell" : "spells");
   }
   ...
   for (i = 0; i < max_vis_circle; ++i) {
      if (circle_spells[i][0]) {
         if (xcircle && xcircle != i + 1) continue;
         for (j = 0; circle_spells[i][j]; ++j) {
            ...
            pprintf(ch, "%s  %s " ELLIPSIS_FMT " %s%-15s&0" EOL,
                  buf,
                  skills[skillnum].quest ? "&6*&0" : " ",
                  ELLIPSIS_STR(skills[skillnum].name, 25),
                  spheres[sphere].color,
                  spheres[sphere].name);
         }
      }
   }
         ...
         for (k = 0, j = 0; k <= TOP_SKILL; ++k) {
            ...
            if (!(page_length > 1 ? (j % (page_length - 1)) : j)) {
               ...
               for (class = 0; class < num_magic_classes; ++class)
                  sprintf(mybuf, "%s %2.2s", mybuf,
                              strip_ansi(classes[magic_class_index[class]].abbrev));
               strcat(mybuf, "&0\r\n");
            }
            ++j;
            ...
            for (class = 0; class < num_magic_classes; ++class)
               if (skills[i].min_level[magic_class_index[class]] < LVL_IMMORT) {
                  if (circle)
                     sprintf(mybuf, "%s&3%2d ", mybuf, (skills[i].min_level[magic_class_index[class]] - 1) / 8 + 1);
                  else
...
ACMD(do_skills) {
   ...
   for (x = 0; x <= TOP_SKILL; ++x) {
      ...
      if (godpeek)
         sprintf(points, " (%4d/%4d/%4d)", GET_ISKILL(tch, i),
                return_max_skill(tch, i),
...
static int scan_chars(struct char_data *ch, int room, int dis, int dir, int seen_any)
{
   ...
   for (i = world[room].people; i; i = i->next_in_room) {
       ...
       */
      if (!(((GET_LEVEL(REAL_CHAR(ch)) >= GET_INVIS_LEV(i) &&
            (INVIS_OK(ch, i) || PRF_FLAGGED(ch, PRF_HOLYLIGHT)))) ||
            IMM_VIS_OK(ch, i)))
         continue;
...
bool exit_is_scannable(struct char_data *ch, struct exit *exit) {
   ...
   if (!EXIT_IS_OPEN(exit)) return FALSE;

   if (EXIT_IS_HIDDEN(exit) && GET_LEVEL(ch) < LVL_IMMORT) return FALSE;

   if (EXIT_NDEST(exit) == NOWHERE) return FALSE;

   if (EXIT_DEST(exit) == NULL) return FALSE;

   if (ROOM_FLAGGED(EXIT_NDEST(exit), ROOM_NOSCAN)) return FALSE;
   ...
   for (dir = 0; dir < NUM_OF_DIRS; ++dir) {
      if (only_dir == -1 || only_dir == dir) {
         ...
         for (dis = 1; dis <= maxdis; ++dis) {
            if ((exit = world[from_room].exits[dir]) &&
                  exit_is_scannable(ch, exit)) {
...
ACMD(do_innate)
{
   ...
   *buf = '\0';
   ...
   if (!*arg) {
      ...
      if(GET_CLASS(ch) == CLASS_PRIEST || GET_CLASS(ch) == CLASS_DIABOLIST ||
            GET_CLASS(ch) == CLASS_PALADIN || GET_CLASS(ch) == CLASS_ANTI_PALADIN)
         send_to_char(" detect alignment*\r\n", ch);
      if(GET_SKILL(ch, SKILL_DOORBASH))
         send_to_char(" doorbash\r\n", ch);
      if(GET_RACE(ch) == RACE_ELF || GET_RACE(ch) == RACE_DWARF ||
            GET_RACE(ch) == RACE_HALFLING || GET_RACE(ch) == RACE_HALF_ELF ||
      ...
      if (is_abbrev(arg, "detect") || is_abbrev(arg, "infravision") ||
            is_abbrev(arg, "ultravision")) {
 ...
 * Add macros POSSESSED and POSSESSOR. Fix crash bug when getting skills of linkless person.
 *
 ...
 * Set room desc of players to "<name> <title> <race>..." moving title next to name.
 *
 ...
 * Who is all messed up.... fixed it so that when you who when you're anonymous your name is shown... apparently throught the who, someone is using the buf buffer sometimes and the Mort_buff other times... need to go through this thoroughly and verify that everything is kosher.
 *
 * Revision 1.75  2005/02/14 04:11:51  djb
 * Fixing mistake made when checking in the files.
 *
 * Revision 1.74  2005/02/14 02:17:57  djb
 * Removed the align checks done when looking to allow evil/good races to see/group with each other.
 *
 ...
 * fix for farsee which allowed a player to escape battle (involved breaking look_at_room
 * into two parts to keep interface for other files whicle allowing a specfied room
 ...
 * do_attributes had a send_to_char misplaced so that it only executed for chars over lvl75. moved it.
 * /
 *
 ...
 * fixed problem in do_skills where unswitched mobs executing 'skills' would crash the mud
 *
 ...
 * Fixed attribute, aligned new score and added old score back, under the command
 * old.
 *
