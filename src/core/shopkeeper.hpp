...
struct ShopItem {
    ...
    ShopItem() = default;
    ShopItem(EntityId id, std::string_view item_name, std::string_view desc, int price, int item_level = 1,
             int quantity = -1)
        : prototype_id(id), name(item_name), description(desc), cost(price), level(item_level), stock(quantity),
          max_stock(quantity) {}
};
...
struct ShopMob {
    ...
    ShopMob() = default;
    ShopMob(EntityId id, std::string_view mob_name, std::string_view desc, int price, int mob_level = 1,
            int quantity = -1)
        : prototype_id(id), name(mob_name), description(desc), cost(price), level(mob_level), stock(quantity),
          max_stock(quantity) {}
};
...
class ShopManager {
  public:
    ...
    template <typename Func> void for_each_shop(Func &&func) const {
