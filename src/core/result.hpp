...
namespace Errors {
inline Error NotFound(std::string_view what) { return Error{ErrorCode::NotFound, fmt::format("{} not found", what)}; }
...
#include <ostream>
inline std::ostream &operator<<(std::ostream &os, const Error &error) { return os << error.to_string(); }
