...
Result<void> Stats::validate() const {
    if (strength < MIN_ATTRIBUTE || strength > MAX_ATTRIBUTE) {
        return std::unexpected(
            Errors::InvalidState(fmt::format("Strength must be between {} and {}", MIN_ATTRIBUTE, MAX_ATTRIBUTE)));
    }
    if (dexterity < MIN_ATTRIBUTE || dexterity > MAX_ATTRIBUTE) {
        return std::unexpected(
            Errors::InvalidState(fmt::format("Dexterity must be between {} and {}", MIN_ATTRIBUTE, MAX_ATTRIBUTE)));
    }
    if (intelligence < MIN_ATTRIBUTE || intelligence > MAX_ATTRIBUTE) {
        return std::unexpected(
            Errors::InvalidState(fmt::format("Intelligence must be between {} and {}", MIN_ATTRIBUTE, MAX_ATTRIBUTE)));
    }
    if (wisdom < MIN_ATTRIBUTE || wisdom > MAX_ATTRIBUTE) {
        return std::unexpected(
            Errors::InvalidState(fmt::format("Wisdom must be between {} and {}", MIN_ATTRIBUTE, MAX_ATTRIBUTE)));
    }
    if (constitution < MIN_ATTRIBUTE || constitution > MAX_ATTRIBUTE) {
        return std::unexpected(
            Errors::InvalidState(fmt::format("Constitution must be between {} and {}", MIN_ATTRIBUTE, MAX_ATTRIBUTE)));
    }
    if (charisma < MIN_ATTRIBUTE || charisma > MAX_ATTRIBUTE) {
        return std::unexpected(
            Errors::InvalidState(fmt::format("Charisma must be between {} and {}", MIN_ATTRIBUTE, MAX_ATTRIBUTE)));
    }
    ...
    if (level < MIN_LEVEL || level > MAX_LEVEL) {
        return std::unexpected(
            Errors::InvalidState(fmt::format("Level must be between {} and {}", MIN_LEVEL, MAX_LEVEL)));
    }
    if (experience < 0) {
...
long Stats::experience_to_next_level() const { return ActorUtils::experience_for_level(level + 1) - experience; }

bool Stats::can_level_up() const { return experience >= ActorUtils::experience_for_level(level + 1); }
...
Result<Stats> Stats::from_json(const nlohmann::json &json) {
    try {
        ...
        if (json.contains("dexterity"))
            stats.dexterity = json["dexterity"].get<int>();
        if (json.contains("intelligence"))
            stats.intelligence = json["intelligence"].get<int>();
        if (json.contains("wisdom"))
            stats.wisdom = json["wisdom"].get<int>();
        if (json.contains("constitution"))
            stats.constitution = json["constitution"].get<int>();
        if (json.contains("charisma"))
            stats.charisma = json["charisma"].get<int>();

        if (json.contains("hit_points"))
            stats.hit_points = json["hit_points"].get<int>();
        if (json.contains("max_hit_points"))
            stats.max_hit_points = json["max_hit_points"].get<int>();
        if (json.contains("mana"))
            stats.mana = json["mana"].get<int>();
        if (json.contains("max_mana"))
            stats.max_mana = json["max_mana"].get<int>();
        if (json.contains("stamina"))
            stats.stamina = json["stamina"].get<int>();
        if (json.contains("max_stamina"))
            stats.max_stamina = json["max_stamina"].get<int>();

        // New combat stats
        if (json.contains("accuracy"))
            stats.accuracy = json["accuracy"].get<int>();
        if (json.contains("attack_power"))
            stats.attack_power = json["attack_power"].get<int>();
        if (json.contains("spell_power"))
            stats.spell_power = json["spell_power"].get<int>();
        if (json.contains("penetration_flat"))
            stats.penetration_flat = json["penetration_flat"].get<int>();
        if (json.contains("penetration_percent"))
            stats.penetration_percent = json["penetration_percent"].get<int>();
        if (json.contains("evasion"))
            stats.evasion = json["evasion"].get<int>();
        if (json.contains("armor_rating"))
            stats.armor_rating = json["armor_rating"].get<int>();
        if (json.contains("damage_reduction_percent"))
            stats.damage_reduction_percent = json["damage_reduction_percent"].get<int>();
        if (json.contains("soak"))
            stats.soak = json["soak"].get<int>();
        if (json.contains("hardness"))
            stats.hardness = json["hardness"].get<int>();
        if (json.contains("ward_percent"))
            stats.ward_percent = json["ward_percent"].get<int>();

        // Elemental resistances
        if (json.contains("resistance_fire"))
            stats.resistance_fire = json["resistance_fire"].get<int>();
        if (json.contains("resistance_cold"))
            stats.resistance_cold = json["resistance_cold"].get<int>();
        if (json.contains("resistance_lightning"))
            stats.resistance_lightning = json["resistance_lightning"].get<int>();
        if (json.contains("resistance_acid"))
            stats.resistance_acid = json["resistance_acid"].get<int>();
        if (json.contains("resistance_poison"))
            stats.resistance_poison = json["resistance_poison"].get<int>();
...
std::shared_ptr<Object> Inventory::remove_item(EntityId item_id) {
    auto it = std::find_if(items_.begin(), items_.end(),
                           [item_id](const std::shared_ptr<Object> &item) { return item && item->id() == item_id; });
...
std::span<const std::shared_ptr<Object>> Inventory::get_all_items() const { return items_; }
...
std::string Equipment::get_slot_conflict_message(EquipSlot slot, const Object &item) const {
    ...
    if (is_equipped(slot)) {
        ...
        case EquipSlot::Light:
            return "You are already using something as light.";
        case EquipSlot::Finger_R:
            return "You are already wearing something on your right finger.";
        case EquipSlot::Finger_L:
            return "You are already wearing something on your left finger.";
        case EquipSlot::Neck1:
        case EquipSlot::Neck2:
            return "You are already wearing something around your neck.";
        case EquipSlot::Body:
            return "You are already wearing something on your body.";
        case EquipSlot::Head:
            return "You are already wearing something on your head.";
        case EquipSlot::Legs:
            return "You are already wearing something on your legs.";
        case EquipSlot::Feet:
            return "You are already wearing something on your feet.";
        case EquipSlot::Hands:
            return "You are already wearing something on your hands.";
        case EquipSlot::Arms:
            return "You are already wearing something on your arms.";
        case EquipSlot::Shield:
            return "You are already using something as a shield.";
        case EquipSlot::About:
            return "You are already wearing something about your body.";
        case EquipSlot::Waist:
            return "You are already wearing something around your waist.";
        case EquipSlot::Wrist_R:
            return "You are already wearing something on your right wrist.";
        case EquipSlot::Wrist_L:
            return "You are already wearing something on your left wrist.";
        ...
        case EquipSlot::Float:
            return "You already have something floating nearby.";
        default:
...
bool Equipment::is_equipped(EquipSlot slot) const { return equipped_.contains(slot); }
...
std::shared_ptr<Object> Equipment::get_main_weapon() const { return get_equipped(EquipSlot::Wield); }

std::shared_ptr<Object> Equipment::get_off_weapon() const { return get_equipped(EquipSlot::Wield2); }
...
bool Equipment::is_wielding_two_handed() const {
    ...
    if (weapon->has_flag(ObjectFlag::TwoHanded))
        return true;

    // Versatile weapon being used with two-handed grip
    if (weapon->has_flag(ObjectFlag::Versatile) && using_two_handed_grip_)
        return true;
        ...
        for (const auto &[slot_name, item_json] : json.items()) {
            if (auto slot = magic_enum::enum_cast<EquipSlot>(slot_name)) {
                ...
                if (item_result) {
                    ...
                    Log::error("Failed to load equipped item in slot {}: {}", slot_name, item_result.error().message);
                }
            }
        }
...
Actor::Actor(EntityId id, std::string_view name) : Entity(id, name), spell_slots_(std::make_unique<SpellSlots>()) {}

Actor::Actor(EntityId id, std::string_view name, const Stats &stats, const Inventory &inventory,
             const Equipment &equipment)
    : Entity(id, name), stats_(stats), inventory_(inventory), equipment_(equipment),
      spell_slots_(std::make_unique<SpellSlots>()) {}

bool Actor::has_flag(ActorFlag flag) const { return flags_.contains(flag); }
...
std::shared_ptr<Room> Actor::current_room() const { return current_room_.lock(); }
...
std::string Actor::room_presence(std::shared_ptr<Actor> viewer) const {
    ...
    bool use_ground_desc = !ground_desc.empty() && position() == Position::Standing && !is_fighting() &&
                           !has_flag(ActorFlag::Paralyzed) && !has_flag(ActorFlag::Immobilized);
...
Result<void> Actor::give_item(std::shared_ptr<Object> item) {
    ...
    if (!inventory_.can_carry(item->weight(), max_carry_weight())) {
        return std::unexpected(Errors::InvalidState(
            fmt::format("Cannot carry that much weight: item {} weighs {}, current weight {}/{}", item->name(),
                        item->weight(), current_carry_weight(), max_carry_weight())));
    }

    return inventory_.add_item(std::move(item));
}

std::shared_ptr<Object> Actor::take_item(EntityId item_id) { return inventory_.remove_item(item_id); }
...
bool Actor::is_overloaded() const { return current_carry_weight() > max_carry_weight(); }
...
bool Actor::has_spell_slots(int circle) const { return spell_slots_ && spell_slots_->has_slots(circle); }

bool Actor::consume_spell_slot(int circle) { return spell_slots_ && spell_slots_->consume_slot(circle); }
...
const SpellSlots &Actor::spell_slots() const { return *spell_slots_; }

SpellSlots &Actor::spell_slots() { return *spell_slots_; }
...
void Actor::add_effect(const ActiveEffect &effect) {
    ...
    Log::game()->debug("Effect '{}' applied to {} (duration: {} hours)", effect.name, name(), effect.duration_hours);
}

void Actor::remove_effect(const std::string &effect_name) {
    auto it = std::find_if(active_effects_.begin(), active_effects_.end(),
                           [&effect_name](const ActiveEffect &e) { return e.name == effect_name; });
...
int Actor::calculate_hit_gain() const {
    ...
    if (has_effect("Song of Rest") && (position_ == Position::Sleeping || position_ == Position::Resting)) {
        gain *= 3;
    }
...
Actor::TickResult Actor::perform_regen_tick() {
    ...
    if (position_ == Position::Stunned || position_ == Position::Incapacitated ||
...
Actor::TickResult Actor::perform_hour_tick() {
    ...
    if (position_ == Position::Dead || position_ == Position::Ghost || position_ == Position::Stunned ||
        position_ == Position::Incapacitated || position_ == Position::Mortally_Wounded) {
        return result;
    }
    ...
    if (dynamic_cast<Player *>(this) != nullptr) {
        ...
        if (stats_.drunk > Stats::CONDITION_MIN) {
            ...
            if (old_drunk > Stats::DRUNK_TOO_DRUNK_THRESHOLD && stats_.drunk <= Stats::DRUNK_TOO_DRUNK_THRESHOLD) {
                send_message("You feel less intoxicated.\r\n");
            }
            if (old_drunk > Stats::DRUNK_SLURRED_THRESHOLD && stats_.drunk <= Stats::DRUNK_SLURRED_THRESHOLD) {
...
bool Actor::interrupt_concentration() {
    ...
    static const std::vector<std::string> concentration_effects = {"Meditate"};
...
void Actor::clear_casting_blackout() { casting_blackout_end_ = std::chrono::steady_clock::time_point{}; }

void Actor::queue_spell(std::string_view spell_name, std::string_view target_name) {
    queued_spell_ = QueuedSpell{.spell_name = std::string{spell_name},
...
void Actor::start_casting(const CastingState &state) { casting_state_ = state; }
...
void Actor::stop_casting(bool interrupted) {
    ...
    if (interrupted) {
        Log::game()->debug("{}'s casting of {} was interrupted", name(), casting_state_->ability_name);
        send_message("Your concentration is broken!\n");
    }
...
void Actor::add_enemy(std::shared_ptr<Actor> enemy) {
    ...
    fighting_list_.erase(std::remove_if(fighting_list_.begin(), fighting_list_.end(),
    ...
    for (const auto &wp : fighting_list_) {
        if (auto existing = wp.lock()) {
            if (existing == enemy)
                return; // Already fighting this enemy
        }
    }
...
void Actor::clear_enemies() { fighting_list_.clear(); }
...
void Actor::add_dot_effect(const fiery::DotEffect &effect) {
    ...
    if (effect.stackable) {
        ...
        for (auto &existing : dot_effects_) {
            if (existing.ability_id == effect.ability_id && existing.effect_id == effect.effect_id) {
                if (existing.stack_count < existing.max_stacks) {
                    existing.stack_count++;
                    // Refresh duration
                    existing.remaining_ticks = effect.remaining_ticks;
                    Log::game()->debug("DoT effect {} stacked to {} on {}", effect.damage_type, existing.stack_count,
                                       name());
                }
                return;
            }
    ...
    update_dot_flags();
    Log::game()->debug("DoT effect {} (potency {}) applied to {}", effect.damage_type, effect.potency, name());
}

void Actor::remove_dot_effects_by_category(const std::string &cure_category) {
    auto it = std::remove_if(dot_effects_.begin(), dot_effects_.end(), [&cure_category](const fiery::DotEffect &e) {
...
bool Actor::has_dot_effect(const std::string &cure_category) const {
    return std::any_of(dot_effects_.begin(), dot_effects_.end(),
                       [&cure_category](const fiery::DotEffect &e) { return e.matches_cure_category(cure_category); });
}

bool Actor::is_regen_blocked_by_dot() const {
    return std::any_of(dot_effects_.begin(), dot_effects_.end(),
                       [](const fiery::DotEffect &e) { return e.blocks_regen; });
}
...
fiery::CureAttemptResult Actor::attempt_cure(const std::string &cure_category, int cure_power, bool cure_all,
    ...
    std::sort(matching_indices.begin(), matching_indices.end(),
              [this](size_t a, size_t b) { return dot_effects_[a].potency > dot_effects_[b].potency; });
    ...
    for (size_t idx : matching_indices) {
        ...
        if (cure_power >= effect.potency) {
            ...
            if (result.result == fiery::CureResult::NoEffect) {
                ...
                result.message = fmt::format("The {} is too powerful for your cure!", effect.damage_type);
            }
        }
    ...
    if (result.effects_removed > 0 && result.effects_reduced > 0) {
        result.message = fmt::format("You cure {} effect{} and weaken {} more.", result.effects_removed,
                                     result.effects_removed == 1 ? "" : "s", result.effects_reduced);
    } else if (result.effects_removed > 0) {
        result.message = fmt::format("You cure {} {} effect{}.", result.effects_removed, cure_category,
                                     result.effects_removed == 1 ? "" : "s");
    } else if (result.effects_reduced > 0) {
        result.message = fmt::format("You weaken {} {} effect{}, but cannot fully cure them.", result.effects_reduced,
                                     cure_category, result.effects_reduced == 1 ? "" : "s");
    }
...
void Actor::update_dot_flags() {
    ...
    bool has_poison = std::any_of(dot_effects_.begin(), dot_effects_.end(),
                                  [](const fiery::DotEffect &e) { return e.cure_category == "poison"; });
    ...
    bool has_fire = std::any_of(dot_effects_.begin(), dot_effects_.end(),
                                [](const fiery::DotEffect &e) { return e.cure_category == "fire"; });
...
void Actor::add_hot_effect(const fiery::HotEffect &effect) {
        ...
        for (auto &existing : hot_effects_) {
            if (existing.ability_id == effect.ability_id && existing.effect_id == effect.effect_id) {
                if (existing.stack_count < existing.max_stacks) {
                    ...
                    // Refresh duration
                    existing.remaining_ticks = effect.remaining_ticks;
                    Log::game()->debug("HoT effect {} stacked to {} on {}", effect.heal_type, existing.stack_count,
                                       name());
                }
                return;
            }
    ...
    Log::game()->debug("HoT effect {} applied to {}", effect.heal_type, name());
}

void Actor::remove_hot_effects_by_category(const std::string &dispel_category) {
    auto it = std::remove_if(hot_effects_.begin(), hot_effects_.end(), [&dispel_category](const fiery::HotEffect &e) {
...
bool Actor::has_hot_effect(const std::string &hot_category) const {
    return std::any_of(hot_effects_.begin(), hot_effects_.end(),
                       [&hot_category](const fiery::HotEffect &e) { return e.matches_dispel_category(hot_category); });
}

bool Actor::is_regen_boosted_by_hot() const {
    return std::any_of(hot_effects_.begin(), hot_effects_.end(),
                       [](const fiery::HotEffect &e) { return e.boosts_regen; });
}
...
Mobile::Mobile(EntityId id, std::string_view name, int level) : Actor(id, name) { initialize_for_level(level); }
...
Result<std::unique_ptr<Mobile>> Mobile::create(EntityId id, std::string_view name, int level) {
    ...
    if (level < MIN_LEVEL || level > MAX_LEVEL) {
        return std::unexpected(
            Errors::InvalidArgument("level", fmt::format("must be between {} and {}", MIN_LEVEL, MAX_LEVEL)));
    }
...
std::shared_ptr<Container> Mobile::die() {
    ...
    Log::info("Mobile::die() - {} has {} equipped items and {} inventory items", name(), equipped_items.size(),
              inv_items.size());
    ...
    if (corpse_result) {
        ...
        for (auto &item : equipped_items) {
            if (item) {
                ...
                if (!result.has_value()) {
                    Log::warn("Failed to add equipment '{}' to corpse: {}", item->display_name(),
        ...
        if (total_coins > 0) {
            ...
            if (coins_result) {
                ...
                if (plat > 0)
                    coin_desc += fmt::format("{} platinum, ", plat);
                if (gold_coins > 0)
                    coin_desc += fmt::format("{} gold, ", gold_coins);
                if (silver_coins > 0)
                    coin_desc += fmt::format("{} silver, ", silver_coins);
                if (copper_coins > 0)
                    coin_desc += fmt::format("{} copper, ", copper_coins);
                // Remove trailing ", "
                if (!coin_desc.empty()) {
        ...
        Log::info("Created corpse for mobile {} with {} items in room {}", name(), corpse->current_capacity(),
                  room->id());
    } else {
...
Player::Player(EntityId id, std::string_view name) : Actor(id, name) {
    ...
    try {
        ...
        if (json.contains("stats")) {
            ...
            if (stats_result) {
                ...
                Log::warn("Failed to load stats for player '{}': {}", player->name(), stats_result.error().message);
            }
        }
        ...
        if (json.contains("spell_slots")) {
            ...
            if (spell_slots_result) {
                ...
                Log::warn("Failed to load spell slots for player '{}': {}", player->name(),
                          spell_slots_result.error().message);
            }
        }
...
void Player::stop_composing() { active_composer_.reset(); }
...
void Player::interrupt_composing(std::string_view reason) {
    ...
    if (!lines.empty()) {
        ...
        send_message(fmt::format("({} line{} of text was saved. Use 'redo' to recover.)", redo_buffer().size(),
                                 redo_buffer().size() == 1 ? "" : "s"));
    }
...
nlohmann::json Player::get_vitals_gmcp() const {
    ...
    for (int circle = 1; circle <= 9; ++circle) {
        ...
        if (max_slots > 0) {
            spell_slots_json[std::to_string(circle)] = nlohmann::json{{"available", current}, {"max", max_slots}};
        }
    }
...
void Player::initialize_spell_slots() {
    ...
    Log::debug("Initialized spell slots for player '{}' (class: {}, level: {})", name(), player_class_, stats().level);
}
...
const std::unordered_map<PlayerFlag, std::string> player_flag_names = {
    {PlayerFlag::Brief, "BRIEF"},       {PlayerFlag::Compact, "COMPACT"},
    {PlayerFlag::NoRepeat, "NOREPEAT"}, {PlayerFlag::AutoLoot, "AUTOLOOT"},
    {PlayerFlag::AutoGold, "AUTOGOLD"}, {PlayerFlag::AutoSplit, "AUTOSPLIT"},
    {PlayerFlag::AutoExit, "AUTOEXIT"}, {PlayerFlag::AutoAssist, "AUTOASSIST"},
    {PlayerFlag::Wimpy, "WIMPY"},       {PlayerFlag::ShowDiceRolls, "SHOWDICEROLLS"},
    {PlayerFlag::Afk, "AFK"},           {PlayerFlag::Deaf, "DEAF"},
    {PlayerFlag::NoTell, "NOTELL"},     {PlayerFlag::NoSummon, "NOSUMMON"},
    {PlayerFlag::Quest, "QUEST"},       {PlayerFlag::PkEnabled, "PKENABLED"},
    {PlayerFlag::Consent, "CONSENT"},   {PlayerFlag::ColorBlind, "COLORBLIND"},
    {PlayerFlag::Msp, "MSP"},           {PlayerFlag::MxpEnabled, "MXP"},
};

const std::unordered_map<std::string, PlayerFlag> player_flag_from_string = {
    {"BRIEF", PlayerFlag::Brief},       {"COMPACT", PlayerFlag::Compact},
    {"NOREPEAT", PlayerFlag::NoRepeat}, {"AUTOLOOT", PlayerFlag::AutoLoot},
    {"AUTOGOLD", PlayerFlag::AutoGold}, {"AUTOSPLIT", PlayerFlag::AutoSplit},
    {"AUTOEXIT", PlayerFlag::AutoExit}, {"AUTOASSIST", PlayerFlag::AutoAssist},
    {"WIMPY", PlayerFlag::Wimpy},       {"SHOWDICEROLLS", PlayerFlag::ShowDiceRolls},
    {"AFK", PlayerFlag::Afk},           {"DEAF", PlayerFlag::Deaf},
    {"NOTELL", PlayerFlag::NoTell},     {"NOSUMMON", PlayerFlag::NoSummon},
    {"QUEST", PlayerFlag::Quest},       {"PKENABLED", PlayerFlag::PkEnabled},
    {"CONSENT", PlayerFlag::Consent},   {"COLORBLIND", PlayerFlag::ColorBlind},
    {"MSP", PlayerFlag::Msp},           {"MXP", PlayerFlag::MxpEnabled},
};
} // namespace
...
std::string format_stats(const Stats &stats) {
    return fmt::format("HP: {}/{}, Mana: {}/{}, Stamina: {}/{}, ACC: {}, EVA: {}, AR: {}, Level: {}", stats.hit_points,
                       stats.max_hit_points, stats.mana, stats.max_mana, stats.stamina, stats.max_stamina,
...
std::optional<ActorFlag> parse_flag(std::string_view flag_name) { return magic_enum::enum_cast<ActorFlag>(flag_name); }
} // namespace ActorUtils

// JSON Support Functions

void to_json(nlohmann::json &json, const Stats &stats) { json = stats.to_json(); }
...
void to_json(nlohmann::json &json, const Actor &actor) { json = actor.to_json(); }
...
std::string Actor::get_stat_info() const {
    ...
    output << fmt::format("{} '{}'  ID: [{}], In room [{}]\n", player ? "PC" : (mobile ? "MOB" : "NPC"), name(),
                          format_id(id()), room_str);
    ...
    output << fmt::format("Class: {}, Lev: [{:2d}], XP: [{:7}], Align: [{:4d}]\n",
                          player ? player->player_class() : "NPC", stats_.level, stats_.experience, stats_.alignment);
    ...
    if (player) {
        ...
        auto format_time = [](std::time_t time) -> std::string {
            ...
            return fmt::format("{:02d}/{:02d}/{:4d} {:02d}:{:02d}", tm.tm_mon + 1, tm.tm_mday, tm.tm_year + 1900,
        ...
        output << fmt::format("Age: [{}], Homeroom: [{}]", PLAYER_BASE_AGE + stats_.level,
                              format_id(player->start_room()));

        // Clan/Guild information
        if (player->in_clan()) {
            output << fmt::format(", Clan: [{} ({})] Rank: [{}]", player->clan_name(), player->clan_abbreviation(),
    ...
    total_copper %= GOLD_TO_COPPER;
    long silver = total_copper / SILVER_TO_COPPER;
    total_copper %= SILVER_TO_COPPER;
    ...
    if (mobile) {
        ...
        std::string hp_dice = fmt::format("{}d{}", mobile->hp_dice_num(), mobile->hp_dice_size());
        if (hp_bonus != 0) {
        ...
        std::string damage_dice =
            fmt::format("{}d{}", mobile->bare_hand_damage_dice_num(), mobile->bare_hand_damage_dice_size());
        if (dam_bonus != 0) {
    ...
    output << fmt::format(
        ...
        "Carried: {} ({} weight); "
        "Worn: {} ({} weight)\n",
        20, max_carry_weight(), inventory().item_count(), current_carry_weight(), worn_items, worn_weight);
    ...
    if (!inventory_items.empty()) {
        ...
        for (const auto &item : inventory_items) {
            if (item) {
                output << fmt::format("  - {} [{}] (Weight: {})\n", item->short_description(), format_id(item->id()),
                                      item->weight());
            }
        }
    } else {
    ...
    if (stats_.drunk > 0) {
        ...
        if (stats_.is_too_drunk())
            drunk_str = "Very Drunk";
        else if (stats_.is_slurring())
            drunk_str = "Tipsy";
        else
            drunk_str = "Slightly Buzzed";
