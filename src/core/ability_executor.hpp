...
public:
     ...
     */
    static std::expected<AbilityExecutionResult, Error> execute(
     ...
     */
    static std::expected<AbilityExecutionResult, Error> execute_by_id(
        const CommandContext& ctx,
        int ability_id,
        std::shared_ptr<Actor> target,
        int skill_level = 50);
     ...
     */
    static std::expected<void, Error> check_prerequisites(
        const CommandContext& ctx,
...
private:
     ...
     */
    static std::expected<std::vector<EffectResult>, Error> execute_effects(
        const WorldQueries::AbilityData& ability,
