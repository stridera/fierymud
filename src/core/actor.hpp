...
class Actor : public Entity {
  public:
    ...
    bool can_act() const {
        return is_alive() && position_ != Position::Incapacitated && position_ != Position::Stunned &&
               position_ != Position::Sleeping;
    }
     ...
     */
    fiery::CureAttemptResult attempt_cure(const std::string &cure_category, int cure_power, bool cure_all = false,
  ...
  protected:
    ...
    Actor(EntityId id, std::string_view name, const Stats &stats, const Inventory &inventory,
    ...
    virtual void on_position_change([[maybe_unused]] Position old_pos, [[maybe_unused]] Position new_pos) {}
  ...
  public:
    ...
    void add_follower(std::shared_ptr<Actor> follower) {
        ...
        followers_.erase(std::remove_if(followers_.begin(), followers_.end(),
...
class Mobile : public Actor {
  public:
    ...
    bool has_trait(MobTrait trait) const { return traits_.test(static_cast<size_t>(trait)); }
    void set_trait(MobTrait trait, bool value = true) { traits_.set(static_cast<size_t>(trait), value); }
    void clear_trait(MobTrait trait) { set_trait(trait, false); }

    /** Mobile behaviors (how the mob ACTS) */
    bool has_behavior(MobBehavior behavior) const { return behaviors_.test(static_cast<size_t>(behavior)); }
    void set_behavior(MobBehavior behavior, bool value = true) {
        ...
        if (behavior == MobBehavior::Teacher)
            is_teacher_ = value;
    }
    void clear_behavior(MobBehavior behavior) { set_behavior(behavior, false); }

    /** Mobile professions (services the mob provides) */
    bool has_profession(MobProfession profession) const { return professions_.test(static_cast<size_t>(profession)); }
    void set_profession(MobProfession profession, bool value = true) {
        ...
        if (profession == MobProfession::Banker)
            is_banker_ = value;
        if (profession == MobProfession::Receptionist)
            is_receptionist_ = value;
        if (profession == MobProfession::Postmaster)
            is_postmaster_ = value;
        if (profession == MobProfession::Shopkeeper)
            is_shopkeeper_ = value;
    }
    ...
    bool has_effect(EffectFlag effect) const { return effect_flags_.contains(effect); }
    ...
    bool take_wealth(long copper_amount) override {
        if (copper_amount <= 0 || money_.value() < copper_amount)
            return false;
        money_ -= fiery::Money(copper_amount);
        return true;
    }
    bool can_afford(long copper_amount) const override { return money_.value() >= copper_amount; }
...
class Player : public Actor {
  public:
    ...
    /** Player preference flags */
    bool has_player_flag(PlayerFlag flag) const { return player_flags_.test(static_cast<size_t>(flag)); }
    void set_player_flag(PlayerFlag flag, bool value = true) { player_flags_.set(static_cast<size_t>(flag), value); }
    void toggle_player_flag(PlayerFlag flag) { player_flags_.flip(static_cast<size_t>(flag)); }
    ...
    void give_wealth(long copper_amount) override {
        if (copper_amount > 0)
            wallet_.receive(copper_amount);
    }
    bool take_wealth(long copper_amount) override { return spend(copper_amount); }
    ...
    bool account_withdraw(const fiery::Money &amount) {
        if (!account_bank_.charge(amount))
            return false;
        wallet_ += amount;
        return true;
    }
    ...
    void set_clan(unsigned int clan_id, std::string_view name, std::string_view abbreviation, int rank_index,
  ...
  public:
    ...
    bool has_group() const { return !leader_.expired() || !followers_.empty(); }

    bool is_group_leader() const { return leader_.expired() && !followers_.empty(); }

    bool is_following() const { return !leader_.expired(); }

    std::shared_ptr<Actor> get_leader() const { return leader_.lock(); }

    const std::vector<std::weak_ptr<Actor>> &get_followers() const { return followers_; }

    void set_leader(std::shared_ptr<Actor> leader) { leader_ = leader; }

    void clear_leader() { leader_.reset(); }
    ...
    const std::unordered_map<int, LearnedAbility> &get_abilities() const { return abilities_; }

    /** Add or update a learned ability */
    void set_ability(const LearnedAbility &ability) { abilities_[ability.ability_id] = ability; }
    ...
    void clear_abilities() { abilities_.clear(); }
    ...
    /** Remove an alias */
    bool remove_alias(std::string_view alias) { return aliases_.erase(std::string(alias)) > 0; }

    /** Check if an alias exists */
    bool has_alias(std::string_view alias) const { return aliases_.find(std::string(alias)) != aliases_.end(); }
    ...
    /** Clear all aliases */
    void clear_aliases() { aliases_.clear(); }
    ...
    const std::unordered_set<std::string> &get_ignored_players() const { return ignored_players_; }
...
template <> struct fmt::formatter<Position> {
    ...
    template <typename FormatContext> auto format(Position pos, FormatContext &ctx) const {
        return fmt::format_to(ctx.out(), "{}", ActorUtils::get_position_name(pos));
    }
};

template <> struct fmt::formatter<ActorFlag> {
    constexpr auto parse(format_parse_context &ctx) { return ctx.begin(); }

    template <typename FormatContext> auto format(ActorFlag flag, FormatContext &ctx) const {
