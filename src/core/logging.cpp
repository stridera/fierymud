...
void Logger::initialize(const std::string &log_file, LogLevel level, bool console_output) {
    try {
        ...
        auto file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(log_file, 10 * 1024 * 1024, 5);
...
std::shared_ptr<Logger> Logger::get(std::string_view component) {
    ...
    try {
        ...
        if (!spdlog_logger) {
            ...
            if (default_logger) {
                spdlog_logger = std::make_shared<spdlog::logger>(comp_str, default_logger->sinks().begin(),
...
std::shared_ptr<Logger> game() { return Logger::get("game"); }

std::shared_ptr<Logger> combat() { return Logger::get("combat"); }

std::shared_ptr<Logger> movement() { return Logger::get("movement"); }

std::shared_ptr<Logger> commands() { return Logger::get("commands"); }

std::shared_ptr<Logger> network() { return Logger::get("network"); }

std::shared_ptr<Logger> persistence() { return Logger::get("persistence"); }

std::shared_ptr<Logger> scripting() { return Logger::get("scripting"); }

std::shared_ptr<Logger> database() { return Logger::get("database"); }
} // namespace Log
