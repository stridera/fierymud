...
class EntityId {
  public:
    ...
    constexpr explicit EntityId(std::uint64_t legacy_id)
        : zone_id_(static_cast<std::uint32_t>(legacy_id / 100)), local_id_(static_cast<std::uint32_t>(legacy_id % 100)),
          valid_(true) {}
    ...
    constexpr std::uint64_t value() const { return static_cast<std::uint64_t>(zone_id_) * 100 + local_id_; }
    ...
    constexpr auto operator<=>(const EntityId &other) const {
        ...
        if (auto cmp = zone_id_ <=> other.zone_id_; cmp != 0)
            return cmp;
...
constexpr uint32_t unpack_zone_id(uint32_t packed) { return packed >> PACKED_ID_ZONE_SHIFT; }

/** Helper to extract local_id from a packed uint32_t */
constexpr uint32_t unpack_local_id(uint32_t packed) { return packed & PACKED_ID_LOCAL_MASK; }

/** Create an EntityId from a packed uint32_t */
inline EntityId unpack_entity_id(uint32_t packed) { return EntityId(unpack_zone_id(packed), unpack_local_id(packed)); }

/** Type-safe entity registry template for managing entities by ID */
template <typename T> class EntityRegistry {
  public:
    ...
    bool unregister_entity(EntityId id) { return entities_.erase(id) > 0; }
  ...
  private:
    ...
    EntityId generate_id() { return EntityId{next_id_++}; }
};

/** Hash specialization for EntityId */
template <> struct std::hash<EntityId> {
    std::size_t operator()(const EntityId &id) const noexcept { return EntityId::Hash{}(id); }
};

/** Formatting support for EntityId */
#include <fmt/format.h>

template <> struct fmt::formatter<EntityId> {
    constexpr auto parse(format_parse_context &ctx) { return ctx.begin(); }

    template <typename FormatContext> auto format(const EntityId &id, FormatContext &ctx) const {
