...
inline int rounds_to_pulses(int rounds) { return rounds * CAST_SPEED_PER_ROUND; }
 ...
 */
std::pair<int, bool> calculate_quickcast_reduction(int base_ticks, const std::shared_ptr<Actor> &caster,
                                                   int spell_circle) {
    ...
    Log::game()->info("Quick Chant: {} proficiency, roll {} vs {}, reduced {} -> {} ticks", quickcast_proficiency, roll,
                      skill_check_target, base_ticks, reduced_ticks);
 ...
 */
static std::string format_ability_message(std::string_view template_str, const std::shared_ptr<Actor> &actor,
                                          const std::shared_ptr<Actor> &target, int damage = 0) {
...
Result<void> AbilityCache::reload() {
    ...
    auto result = ConnectionPool::instance().execute([this](pqxx::work &txn) -> Result<void> {
        ...
        if (!restrictions_result) {
            ...
            for (const auto &restr_data : *restrictions_result) {
                ...
                restr.requirements =
                    WorldQueries::parse_ability_requirements(restr_data.requirements_json, restr_data.ability_id);
    ...
    initialized_ = true;
    logger->info(
        "Ability cache loaded: {} abilities, {} effect types, {} ability-effects, {} messages, {} restrictions",
        abilities_.size(), effects_.size(), ability_effects_.size(), ability_messages_.size(),
        ability_restrictions_.size());
    return {};
}
...
std::expected<AbilityExecutionResult, Error> AbilityExecutor::execute(const CommandContext &ctx,
                                                                      std::string_view ability_name,
                                                                      std::shared_ptr<Actor> target, int skill_level) {
...
std::expected<AbilityExecutionResult, Error> AbilityExecutor::execute_by_id(const CommandContext &ctx, int ability_id,
    ...
    if (ability->is_toggle) {
        ...
        if (ctx.actor->has_effect(ability->plain_name)) {
            ...
            if (custom_msgs && !custom_msgs->wearoff_to_target.empty()) {
                result.attacker_message = format_ability_message(custom_msgs->wearoff_to_target, ctx.actor, nullptr, 0);
                result.room_message = format_ability_message(custom_msgs->wearoff_to_room, ctx.actor, nullptr, 0);
            } else {
                result.attacker_message = fmt::format("You stop {}.", ability->name);
                result.room_message = fmt::format("{} stops {}.", ctx.actor->display_name(), ability->name);
            }
    ...
    for (const auto &current_target : targets) {
        ...
        if (trigger_mgr.is_initialized() && current_target != ctx.actor) {
            auto trigger_result = trigger_mgr.dispatch_cast(current_target, ctx.actor, ability->name);
            if (trigger_result == TriggerResult::Halt) {
                // Mob blocked this spell on itself
                Log::game()->debug("Spell {} on {} blocked by CAST trigger", ability->name,
        ...
        if (!effects_result) {
            ...
            Log::game()->warn("Effect execution failed for target {}: {}", current_target->display_name(),
                              effects_result.error().message);
            continue;
        }
        ...
        if (ability->is_area && current_target != ctx.actor && target_damage > 0) {
            std::string target_dmg_msg =
                fmt::format("{}'s {} hits you for {} damage!", ctx.actor->display_name(), ability->name, target_damage);
    ...
    if (custom_msgs) {
        ...
        if (result.success) {
            if (ability->is_area) {
                ...
                if (!custom_msgs->success_to_self.empty()) {
                    result.attacker_message =
                        format_ability_message(custom_msgs->success_to_self, ctx.actor, ctx.actor, result.total_damage);
                } else {
                ...
                for (const auto &tr : target_results) {
                    std::string target_hit_msg =
                        format_ability_message(custom_msgs->success_to_caster, ctx.actor, tr.target, tr.damage);
                ...
                if (!custom_msgs->success_to_room.empty() && !damaged_targets.empty()) {
                    result.room_message = format_ability_message(custom_msgs->success_to_room, ctx.actor,
                                                                 damaged_targets[0], result.total_damage);
                    if (damaged_targets.size() > 1) {
                        result.room_message += fmt::format(" and {} other{}!", damaged_targets.size() - 1,
                                                           damaged_targets.size() > 2 ? "s" : "");
                    }
                }
            } else {
                // Check if this is a self-cast (target is the caster, or null for non-violent self-target)
                bool self_cast = (target == ctx.actor) || (target == nullptr && !ability->violent);
                ...
                if (self_cast && !custom_msgs->success_to_self.empty()) {
                    ...
                    result.attacker_message = format_ability_message(custom_msgs->success_to_self, ctx.actor,
                                                                     msg_target, result.total_damage);
                    // No target message for self-cast (caster IS the target)
                    result.room_message =
                        format_ability_message(!custom_msgs->success_self_room.empty() ? custom_msgs->success_self_room
                                                                                       : custom_msgs->success_to_room,
                                               ctx.actor, msg_target, result.total_damage);
                } else {
                    // Normal caster/target messages
                    result.attacker_message = format_ability_message(custom_msgs->success_to_caster, ctx.actor,
                                                                     msg_target, result.total_damage);
                    ...
                    result.target_message = format_ability_message(custom_msgs->success_to_victim, ctx.actor,
                                                                   msg_target, result.total_damage);
                    result.room_message = format_ability_message(custom_msgs->success_to_room, ctx.actor, msg_target,
                                                                 result.total_damage);
            ...
            result.attacker_message = format_ability_message(custom_msgs->fail_to_caster, ctx.actor, target, 0);
            result.target_message = format_ability_message(custom_msgs->fail_to_victim, ctx.actor, target, 0);
            result.room_message = format_ability_message(custom_msgs->fail_to_room, ctx.actor, target, 0);
        }
    } else {
...
std::expected<void, Error> AbilityExecutor::check_prerequisites(const CommandContext &ctx,
    ...
    if (actor_position < ability.min_position) {
        return std::unexpected(Errors::InvalidState(fmt::format(
            "You must be {} to use {}", ActorUtils::get_position_name(static_cast<Position>(ability.min_position)),
            ability.plain_name)));
    }
    ...
    if (ability.violent && !target && !ability.is_area) {
        return std::unexpected(
            Errors::InvalidArgument("target", fmt::format("{} requires a target", ability.plain_name)));
    }
    ...
    if (ability.in_combat_only && !is_fighting) {
        return std::unexpected(
            Errors::InvalidState(fmt::format("You can only use {} while fighting!", ability.plain_name)));
    }
    if (!ability.combat_ok && is_fighting) {
        return std::unexpected(
            Errors::InvalidState(fmt::format("You can't use {} while fighting!", ability.plain_name)));
    }
    ...
    if (ability.cooldown_ms > 0) {
        ...
        if (player) {
            ...
            if (learned) {
                ...
                auto elapsed_ms =
                    std::chrono::duration_cast<std::chrono::milliseconds>(now - learned->last_used).count();
                if (elapsed_ms < ability.cooldown_ms) {
                    int remaining_sec = static_cast<int>((ability.cooldown_ms - elapsed_ms) / 1000) + 1;
                    return std::unexpected(
                        Errors::InvalidState(fmt::format("You must wait {} more second{} to use {}.", remaining_sec,
                                                         remaining_sec == 1 ? "" : "s", ability.plain_name)));
                }
            }
        }
    }
        ...
        if (player && !player->is_god()) {
            ...
            if (spell_circle > 0) {
                ...
                if (!has_available_slot) {
                    return std::unexpected(
                        Errors::InvalidState(fmt::format("You don't have any available spell slots for {} (circle {}).",
                ...
                if (!player->consume_spell_slot(spell_circle)) {
                    return std::unexpected(Errors::InvalidState(
                        fmt::format("Failed to use spell slot for {} (circle {}).", ability.plain_name, spell_circle)));
                }
            }
        }
...
AbilityExecutor::execute_effects(const WorldQueries::AbilityData &ability, EffectContext &context,
    ...
    Log::game()->info("Ability {} (id={}) has {} effects, looking for trigger={}", ability.plain_name, ability.id,
                      ability_effects.size(), trigger == EffectTrigger::OnCast ? "on_cast" : "on_hit");
    ...
    for (const auto &ability_effect : ability_effects) {
        ...
        if (def) {
            ...
            Log::game()->info("  Effect {} (id={}) type={} trigger={} chance={}%", def->name, def->id,
                              static_cast<int>(def->type), static_cast<int>(ability_effect.trigger),
    ...
    // Execute all effects
    return EffectExecutor::execute_ability_effects(ability_effects, effect_defs, context, trigger);
}
...
std::expected<void, Error> AbilityExecutor::begin_casting(std::shared_ptr<Actor> caster, int ability_id,
                                                          std::shared_ptr<Actor> target, int spell_circle) {

    auto &cache = AbilityCache::instance();
    const auto *ability = cache.get_ability(ability_id);
    if (!ability) {
        return std::unexpected(Errors::NotFound(fmt::format("Ability ID {} not found", ability_id)));
    }
    ...
    auto [reduced_ticks, quickcast_applied] = calculate_quickcast_reduction(base_ticks, caster, spell_circle);
...
std::expected<AbilityExecutionResult, Error>
AbilityExecutor::execute_completed_cast(std::shared_ptr<Actor> caster, int ability_id, std::shared_ptr<Actor> target) {
...
Result<CommandResult> execute_skill_command(const CommandContext &ctx, std::string_view skill_name,
                                            bool requires_target, bool can_initiate_combat) {

    // Can't use skills while casting a spell
    if (ctx.actor->is_casting()) {
        auto state_opt = ctx.actor->casting_state();
        if (state_opt.has_value()) {
            ctx.send_error(fmt::format("You can't {} while casting {}!", skill_name, state_opt->ability_name));
        } else {
    ...
    if (ctx.actor->position() == Position::Fighting) {
        ...
        if (requires_target) {
            if (ctx.arg_count() == 0) {
                ctx.send_error(fmt::format("{} who?", std::string(1, std::toupper(skill_name[0])) +
                                                          std::string(skill_name.substr(1))));
                return CommandResult::InvalidTarget;
            }
    ...
    if (target && target->stats().hit_points <= 0) {
        ...
        ctx.send_to_room(fmt::format("{} has been slain by {}!", target->display_name(), ctx.actor->display_name()),
                         true, std::array{target});
    }
