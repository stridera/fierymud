    ...
    try {
        ...
        if (!file.is_open()) {
            return std::unexpected(
                Error{ErrorCode::FileAccessError, fmt::format("Failed to open config file: {}", config_file)});
        }
        ...
        return std::unexpected(
            Error{ErrorCode::FileAccessError, fmt::format("Failed to load config from {}: {}", config_file, e.what())});
    }
}
...
Result<Config> Config::from_json(const nlohmann::json &data) {
    ...
    try {
        ...
        return std::unexpected(Error{ErrorCode::ParseError, fmt::format("Failed to parse Config: {}", e.what())});
    }
}
