...
void Entity::remove_keyword(std::string_view keyword) {
    ...
    keywords_.erase(std::remove(keywords_.begin(), keywords_.end(), normalized), keywords_.end());
}
    ...
    try {
        ...
        if (json.contains("name")) {
            ...
            return std::unexpected(
                Errors::ParseError("Entity JSON missing 'name', 'short', 'keywords', or 'name_list' field"));
        }
        ...
        if (json["id"].is_string()) {
            ...
            if (id_value == 0)
                id_value = 1000; // Convert zone 0 to ID 1000
...
bool is_valid_keyword(std::string_view keyword) {
    ...
    if (!std::all_of(keyword.begin(), keyword.end(), [](char c) { return std::isprint(c) && c != '"' && c != '\''; })) {
        return false;
    }
...
std::string format_entity_name(std::string_view name, std::string_view short_desc, bool with_article,
                               bool definite_article) {
    ...
    if (with_article) {
        ...
        if (lower_display.starts_with("a ") || lower_display.starts_with("an ") || lower_display.starts_with("the ") ||
            lower_display.starts_with("some ") || lower_display.starts_with("this ") ||
...
void to_json(nlohmann::json &json, const Entity &entity) { json = entity.to_json(); }
