...
struct DotEffect {
     ...
     */
    [[nodiscard]] bool should_tick() const { return ticks_since_last >= tick_interval; }

    /**
     * @brief Advance the tick counter
     */
    void advance_tick() { ticks_since_last++; }

    /**
     * @brief Reset tick counter after applying damage
     */
    void reset_tick_counter() { ticks_since_last = 0; }

    /**
     * @brief Check if effect has expired
     * @return true if remaining_ticks has reached 0
     */
    [[nodiscard]] bool is_expired() const { return remaining_ticks == 0; }
