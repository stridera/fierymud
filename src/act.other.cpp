...
ACMD(do_bind) {
    ...
    if (!*arg)
        ...
        if (GET_SKILL(ch, SKILL_BIND) == 0) {
            ...
            percent += fight_bonus[GET_DEX(vict)].armor;
...
ACMD(do_hide) {
    ...
    if (group_size(ch, true) > 1 && GET_RACE(ch) == RACE_HALFLING)
        GET_HIDDENNESS(ch) =
            random_number(lower_bound, upper_bound) + (skill_stat_bonus[GET_DEX(ch)].rogue_skills * ((GET_LEVEL(ch) / 30) + 1));
    else
        GET_HIDDENNESS(ch) = random_number(lower_bound, upper_bound) + skill_stat_bonus[GET_DEX(ch)].rogue_skills;
...
ACMD(do_steal) {
    ...
    percent = random_number(1, 101) - skill_stat_bonus[GET_DEX(ch)].rogue_skills;
    if (!CAN_SEE(vict, ch))
        percent -= skill_stat_bonus[GET_DEX(ch)].rogue_skills;
