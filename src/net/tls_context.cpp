...
Result<void> TLSContextManager::initialize() {
    ...
    try {
        ...
        return std::unexpected(
            Error{ErrorCode::NetworkError, fmt::format("TLS context initialization failed: {}", e.what())});
    }
}
...
Result<void> TLSContextManager::load_certificates() {
    try {
        ...
        return std::unexpected(
            Error{ErrorCode::FileAccessError, fmt::format("Failed to load certificates: {}", e.what())});
    }
}
...
Result<void> TLSContextManager::load_dh_params() {
    try {
        ...
        return std::unexpected(
            Error{ErrorCode::FileAccessError, fmt::format("Failed to load DH parameters: {}", e.what())});
    }
}
...
Result<void> TLSContextManager::configure_security_options() {
    try {
        ...
        return std::unexpected(
            Error{ErrorCode::NetworkError, fmt::format("Failed to configure TLS security options: {}", e.what())});
    }
}
...
Result<void> TLSContextManager::setup_cipher_list() {
    try {
        ...
        return std::unexpected(
            Error{ErrorCode::NetworkError, fmt::format("Failed to setup cipher list: {}", e.what())});
    }
}
...
Result<void> TLSContextManager::create_self_signed_certificate() {
    ...
    try {
        ...
        return std::unexpected(
            Error{ErrorCode::FileAccessError, fmt::format("Failed to create certificate directory: {}", e.what())});
    }
...
bool TLSContextManager::file_exists(const std::string &path) const { return std::filesystem::exists(path); }

// TLSSocket implementation

TLSSocket::TLSSocket(asio::ip::tcp::socket socket) : is_tls_(false), tcp_socket_(std::move(socket)) {
...
void TLSSocket::async_handshake(std::function<void(const asio::error_code &)> handler) {
    if (!is_tls_) {
        ...
        asio::post(tcp_socket_.get_executor(), [handler]() { handler(asio::error_code{}); });
        return;
    }

    if (!tls_stream_) {
        asio::post(tcp_socket_.get_executor(), [handler]() { handler(asio::error::invalid_argument); });
        return;
    }
...
void TLSSocket::async_read_some(asio::mutable_buffer buffer,
    ...
    if (is_tls_) {
        if (!tls_stream_ || !handshake_complete_) {
            asio::post(tcp_socket_.get_executor(), [handler]() { handler(asio::error::not_connected, 0); });
            return;
        }
...
bool TLSSocket::is_open() const { return tcp_socket_.is_open(); }
...
asio::ip::tcp::socket &TLSSocket::tcp_socket() { return tcp_socket_; }

const asio::ip::tcp::socket &TLSSocket::tcp_socket() const { return tcp_socket_; }
