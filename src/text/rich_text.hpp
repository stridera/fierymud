...
class RichText {
  public:
    ...
    RichText &progress_bar(float percentage, int width, std::string_view filled, std::string_view empty);

    /** Add table row with columns */
    RichText &table_row(const std::vector<std::string> &columns, const std::vector<int> &widths = {});
