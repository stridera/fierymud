...
Capabilities detect_capabilities_from_new_environ(const std::unordered_map<std::string, std::string> &env_vars) {
    ...
    if (!caps.terminal_name.empty()) {
        ...
        if (!caps.supports_color)
            caps.supports_color = term_caps.supports_color;
        if (!caps.supports_256_color)
            caps.supports_256_color = term_caps.supports_256_color;
        if (!caps.supports_true_color)
            caps.supports_true_color = term_caps.supports_true_color;
        if (!caps.supports_unicode)
            caps.supports_unicode = term_caps.supports_unicode;
...
Capabilities get_capabilities_for_terminal(std::string_view terminal_name) {
    ...
    if (term_lower.find("alacritty") != std::string::npos || term_lower.find("kitty") != std::string::npos ||
        term_lower.find("wezterm") != std::string::npos || term_lower.find("iterm2") != std::string::npos) {
        caps.supports_color = true;
        caps.supports_256_color = true;
        caps.supports_true_color = true;
        caps.supports_unicode = true;
    ...
    else if (term_lower.find("xterm-256") != std::string::npos || term_lower.find("screen-256") != std::string::npos ||
    ...
    else if (term_lower.find("xterm") != std::string::npos || term_lower.find("rxvt") != std::string::npos ||
             term_lower.find("screen") != std::string::npos || term_lower.find("tmux") != std::string::npos) {
        caps.supports_color = true;
        caps.supports_bold = true;
        caps.supports_underline = true;
        caps.overall_level = SupportLevel::Standard;
