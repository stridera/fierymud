...
bool RichTextFormat::has_formatting() const {
    return foreground != Color::Default || background != BackgroundColor::Default || !styles.empty();
}
...
void RichText::set_capabilities(const TerminalCapabilities::Capabilities &caps) { capabilities_ = &caps; }
...
RichText &RichText::progress_bar(float percentage, int width, std::string_view filled, std::string_view empty) {
    ...
    if (percentage < 0.3f)
        bar_color = Color::BrightRed;
    else if (percentage < 0.6f)
        bar_color = Color::BrightYellow;
...
std::string RichText::to_plain() const { return content_; }

size_t RichText::plain_length() const { return content_.size(); }

bool RichText::empty() const { return content_.empty(); }
...
void RichText::insert_reset() { format_inserts_.emplace_back(content_.size(), "\033[0m"); }
...
RichText table(const std::vector<std::string> &headers, const std::vector<std::vector<std::string>> &rows) {
