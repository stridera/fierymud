/***************************************************************************
 *   File: src/server/modern_mud_server.cpp               Part of FieryMUD *
...
Result<ServerConfig> ServerConfig::load_from_file(const std::string& filename) {
    try {
        ...
        if (j.contains("performance")) {
            ...
            config.enable_performance_monitoring = perf.value("enable_monitoring", config.enable_performance_monitoring);
        }
...
Result<void> ServerConfig::save_to_file(const std::string& filename) const {
    try {
        ...
        j["persistence"] = {
            {"auto_save_interval_seconds", auto_save_interval.count()},
...
std::string ServerStats::summary() const {
    ...
    oss << fmt::format("Uptime: {}s, Connections: {} current/{} total, ", 
                       uptime().count(), current_connections.load(), total_connections.load());
    oss << fmt::format("Commands: {} total ({:.2f}/s), Logins: {} total/{} failed",
                       total_commands.load(), commands_per_second(), 
                       total_logins.load(), failed_logins.load());
    return oss.str();
}

// ModernMUDServer Implementation

ModernMUDServer::ModernMUDServer(ServerConfig config) 
...
std::string ModernMUDServer::get_performance_report() const {
    ...
    report << fmt::format("Resources: CPU {:.1f}%, Memory {}MB, Disk Free {}GB\n",
                         resource_usage.cpu_percentage, resource_usage.memory_mb, 
                         resource_usage.disk_free_mb / 1024);
...
Result<void> ModernMUDServer::initialize_logging() {
    return ServerUtils::setup_logging(config_);
}
...
Result<void> ModernMUDServer::initialize_directories() {
    ...
    std::vector<std::string> required_dirs = {
        config_.world_directory,
        config_.player_directory,
        ...
        if (!fs::exists(dir)) {
            ...
            if (!fs::create_directories(dir, ec)) {
                return std::unexpected(Errors::FileSystem(
                    fmt::format("Failed to create directory {}: {}", dir, ec.message())));
            }
...
Result<void> ModernMUDServer::initialize_persistence() {
    return persistence_manager_->initialize(config_);
}
...
void ModernMUDServer::log_state_change(ServerState old_state, ServerState new_state) {
    Log::info("Server state changed: {} -> {}", 
             magic_enum::enum_name(old_state), magic_enum::enum_name(new_state));
}
