...
WorldServer::WorldServer(asio::io_context& io_context, const ServerConfig& config)
    ...
    , world_strand_(asio::make_strand(io_context))
    , config_(config)
...
void WorldServer::stop() {
    ...
    // Cancel all timers
    if (save_timer_) save_timer_->cancel();
    if (cleanup_timer_) cleanup_timer_->cancel();
    if (heartbeat_timer_) heartbeat_timer_->cancel();
    
    // Disconnect all players
    asio::post(world_strand_, [this]() {
        for (auto& connection : active_connections_) {
            connection->send("Server is shutting down. Goodbye!\r\n");
            connection->close();
        }
...
std::future<void> WorldServer::get_world_loaded_future() {
    return world_loaded_promise_.get_future();
}
...
void WorldServer::process_command(CommandRequest command) {
    ...
    asio::post(world_strand_, [this, cmd = std::move(command)]() {
        handle_command(std::move(cmd));
    });
}

void WorldServer::process_command(std::shared_ptr<PlayerConnection> connection, 
...
void WorldServer::process_command(std::shared_ptr<Actor> actor, 
                                 std::string_view command_text) {
    if (!running_.load()) {
        return;
    }
    
    Log::debug("WorldServer::process_command (Actor): Received command '{}' for actor '{}'", command_text, actor->name());
    ...
    asio::post(world_strand_, [this, actor, cmd = std::move(cmd)]() {
        Log::debug("WorldServer::process_command (Actor) - posted lambda: Processing command '{}' for actor '{}'", cmd.command, actor->name());
        // Process the command and send response
        if (!cmd.command.empty()) {
            Log::debug("Processing command '{}' on world strand", cmd.command);
...
void WorldServer::add_player_connection(std::shared_ptr<PlayerConnection> connection) {
    asio::post(world_strand_, [this, connection]() {
...
void WorldServer::remove_player_connection(std::shared_ptr<PlayerConnection> connection) {
    asio::post(world_strand_, [this, connection]() {
        handle_player_disconnection(connection);
    });
}
...
void WorldServer::schedule_periodic_save() {
    ...
    schedule_timer(config_.auto_save_interval, 
                   [this]() { perform_world_save(); }, save_timer_);
}

void WorldServer::schedule_periodic_cleanup() {
    cleanup_timer_ = std::make_shared<asio::steady_timer>(world_strand_);
    schedule_timer(std::chrono::minutes(5), 
                   [this]() { perform_cleanup(); }, cleanup_timer_);
}

void WorldServer::schedule_heartbeat() {
    heartbeat_timer_ = std::make_shared<asio::steady_timer>(world_strand_);
    schedule_timer(std::chrono::seconds(30), 
                   [this]() { perform_heartbeat(); }, heartbeat_timer_);
}

void WorldServer::schedule_timer(std::chrono::milliseconds interval, 
...
void WorldServer::perform_heartbeat() {
    ...
    Log::debug("WorldServer heartbeat - Uptime: {}s, Active connections: {}, Commands processed: {}", 
               uptime, active_connections_.size(), commands_processed_.load());
}

// Statistics

size_t WorldServer::active_player_count() const {
...
Result<void> WorldServer::create_default_world() {
    ...
    asio::post(world_strand_, [this]() {
        ...
        start_room->set_description("This is a simple starting room for the modern MUD server. "
...
void WorldServer::send_room_info_to_player(std::shared_ptr<PlayerConnection> connection) {
    // For now, send info about the test room we loaded
    // TODO: Track player's actual room location when we have a player system
    
    if (!world_manager_) {
        return;
    }
    
    // Get the test room (ID 1000) that we loaded from the zone
    auto room = world_manager_->get_room(EntityId{1000});
    if (room) {
        connection->gmcp_handler().send_room_info(*room);
