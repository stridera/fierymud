...
Result<void> CommandSystem::initialize() {
    ...
    options.allow_abbreviations = true;
    options.fuzzy_matching = true;
    options.min_abbreviation_length = 2;
    parser_.set_options(options);
...
Result<CommandResult> CommandSystem::execute_parsed_command(std::shared_ptr<Actor> actor,
    ...
    // Find command
    const CommandInfo *cmd_info = find_command(command.command);
    if (!cmd_info) {
    ...
    if (result == CommandResult::Success && cmd_info->cooldown.count() > 0) {
        cooldowns_[actor->id()][cmd_info->name] = CommandCooldown{cmd_info->cooldown, std::chrono::steady_clock::now()};
    }
...
const CommandInfo *CommandSystem::find_command(std::string_view name) const {
    ...
    auto match = parser_.find_command_match(normalized_name, command_names);
    if (match) {
...
std::vector<std::string> CommandSystem::get_command_suggestions(std::string_view partial,
    ...
    return parser_.get_suggestions(partial, available, 5);
}
