...
bool ParsedCommand::is_command(std::string_view name) const {
    return CommandParserUtils::starts_with_ci(command, name) && 
           (command.length() == name.length() || is_abbreviated);
}

Result<void> ParsedCommand::require_args(size_t min_count, std::string_view usage) const {
    if (arguments.size() < min_count) {
        std::string error = fmt::format("Command '{}' requires at least {} argument{}",
                                       command, min_count, min_count == 1 ? "" : "s");
        if (!usage.empty()) {
...
void ParsedCommand::rebuild_argument_string() {
    full_argument_string = join_args(" ");
}
...
ParseResult CommandParser::parse(std::string_view input) const {
    ...
    cmd.has_quoted_args = std::any_of(tokens.quoted_flags.begin() + 1, tokens.quoted_flags.end(),
                                     [](bool quoted) { return quoted; });
...
ParseResult CommandParser::parse(std::string_view input, std::span<const std::string> available_commands) const {
    ...
    if (!result.suggestions.empty()) {
        result.error_message = fmt::format("Unknown command '{}'. Did you mean: {}?",
...
std::vector<std::string> CommandParser::get_suggestions(std::string_view partial,
...
bool CommandParser::is_comment_line(std::string_view input) const {
    ...
    return options_.comment_prefixes.find(trimmed[0]) != std::string::npos;
}
...
namespace CommandParserUtils {
    ...
    std::string expand_abbreviations(std::string_view input) {
        ...
        static const std::unordered_map<std::string, std::string> abbreviations = {
            {"l", "look"}, {"i", "inventory"}, {"eq", "equipment"},
            {"who", "who"}, {"n", "north"}, {"s", "south"},
...
std::vector<std::string> CommandCompletion::complete_with_context(std::string_view partial,
    ...
    basic_completions.erase(std::unique(basic_completions.begin(), basic_completions.end()),
                           basic_completions.end());
...
void CommandSuggestionEngine::record_usage(std::string_view command) {
    usage_counts_[std::string{command}]++;
}
...
int CommandSuggestionEngine::calculate_suggestion_score(std::string_view invalid,
