...
bool RoomContents::remove_object(EntityId obj_id) {
    auto it = std::find_if(objects.begin(), objects.end(),
        [obj_id](const auto& obj) { return obj && obj->id() == obj_id; });
...
std::shared_ptr<Object> RoomContents::find_object(EntityId id) const {
    auto it = std::find_if(objects.begin(), objects.end(),
        [id](const auto& obj) { return obj && obj->id() == id; });
...
std::shared_ptr<Actor> RoomContents::find_actor(EntityId id) const {
    auto it = std::find_if(actors.begin(), actors.end(),
        [id](const auto& actor) { return actor && actor->id() == id; });
...
bool Room::has_flag(RoomFlag flag) const {
...
void Room::remove_exit(Direction dir) {
    exits_.erase(dir);
}
...
void Room::add_object(std::shared_ptr<Object> obj) {
    contents_.add_object(std::move(obj));
}

bool Room::remove_object(EntityId obj_id) {
    return contents_.remove_object(obj_id);
}

void Room::add_actor(std::shared_ptr<Actor> actor) {
    contents_.add_actor(std::move(actor));
}

bool Room::remove_actor(EntityId actor_id) {
    return contents_.remove_actor(actor_id);
}

std::shared_ptr<Object> Room::find_object(EntityId id) const {
    return contents_.find_object(id);
}

std::shared_ptr<Actor> Room::find_actor(EntityId id) const {
    return contents_.find_actor(id);
}
...
bool Room::is_dark() const {
    return has_flag(RoomFlag::Dark) || calculate_effective_light() <= 0;
}

bool Room::is_naturally_lit() const {
    return sector_provides_light() && !has_flag(RoomFlag::Dark);
}
...
bool Room::is_full() const {
    return static_cast<int>(contents_.actors.size()) >= max_occupants();
}
...
bool Room::sector_provides_light() const {
    return RoomUtils::get_sector_light_level(sector_type_) > 0;
}
...
namespace RoomUtils {
    ...
    Direction get_opposite_direction(Direction dir) {
        static const std::unordered_map<Direction, Direction> opposites = {
    ...
    int get_movement_cost(SectorType sector) {
        static const std::unordered_map<SectorType, int> costs = {
            ...
            {SectorType::Forest, 3},
            {SectorType::Hills, 3},
            {SectorType::Mountains, 4},
            {SectorType::Water_Swim, 3},
            {SectorType::Water_Noswim, 1}, // Can't move without boat
            {SectorType::Underwater, 4},
            {SectorType::Flying, 1},
            {SectorType::Desert, 3},
            {SectorType::Swamp, 4},
            {SectorType::Beach, 2},
            {SectorType::Road, 1},
            {SectorType::Underground, 2},
            {SectorType::Lava, 5},
            {SectorType::Ice, 2}
        };
    ...
    bool sector_allows_flying(SectorType sector) {
        return sector != SectorType::Underwater && 
               sector != SectorType::Underground;
    }
    
    bool is_water_sector(SectorType sector) {
        return sector == SectorType::Water_Swim ||
               sector == SectorType::Water_Noswim ||
               sector == SectorType::Underwater;
    }
    ...
    int get_sector_light_level(SectorType sector) {
        static const std::unordered_map<SectorType, int> light_levels = {
            ...
            {SectorType::Forest, 1},
            {SectorType::Hills, 3},
            {SectorType::Mountains, 3},
            {SectorType::Water_Swim, 2},
            {SectorType::Water_Noswim, 2},
            {SectorType::Underwater, 0},
            {SectorType::Flying, 4},
            {SectorType::Desert, 4},
            {SectorType::Swamp, 1},
            {SectorType::Beach, 3},
            {SectorType::Road, 3},
            {SectorType::Underground, 0},
            {SectorType::Lava, 5},
            {SectorType::Ice, 2},
            {SectorType::Fire, 6}
        };
