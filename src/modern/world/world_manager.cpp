...
Result<void> WorldManager::load_world() {
    ...
    // Load zones with embedded rooms, objects, and mobs
    std::string zone_dir = world_path_ + "/zon";
    TRY(load_zones_from_directory(zone_dir));
...
Result<void> WorldManager::reload_zone(EntityId zone_id) {
    ...
    std::string filename = fmt::format("{}/zon/{}.zon", world_path_, zone_id.value());
...
Result<void> WorldManager::save_world_state() {
    ...
    for (const auto& [id, zone] : zones_) {
        if (zone) {
            std::string filename = fmt::format("{}/zon/{}.zon", world_path_, id.value());
...
Result<void> WorldManager::load_zones_from_directory(const std::string& zone_dir) {
    if (!std::filesystem::exists(zone_dir)) {
        return std::unexpected(Errors::FileNotFound(zone_dir));
    }
    
    auto zone_files = WorldUtils::get_zone_files(zone_dir);
    
    for (const auto& filename : zone_files) {
        std::string full_path = zone_dir + "/" + filename;
...
void WorldManager::update_file_timestamps() {
    ...
    std::string zone_dir = world_path_ + "/zon";
    if (std::filesystem::exists(zone_dir)) {
        for (const auto& entry : std::filesystem::recursive_directory_iterator(zone_dir)) {
...
namespace WorldUtils {
    bool validate_world_directory(const std::string& world_path) {
        return std::filesystem::exists(world_path) &&
               std::filesystem::exists(world_path + "/zon") &&
               std::filesystem::exists(world_path + "/wld");
    }
    
    Result<void> create_world_directory(const std::string& world_path) {
        try {
            std::filesystem::create_directories(world_path);
            std::filesystem::create_directories(world_path + "/zon");
            std::filesystem::create_directories(world_path + "/wld");
            std::filesystem::create_directories(world_path + "/obj");
            std::filesystem::create_directories(world_path + "/mob");
