/***************************************************************************
 *   File: src/modern/game/player_output.hpp          Part of FieryMUD *
...
public:
    void send_message(std::string_view message) override {
        captured_output += std::string(message) + "\n";
    }
    
    void send_prompt(std::string_view prompt) override {
        captured_output += "[PROMPT: " + std::string(prompt) + "]\n";
    }
    
    void send_line(std::string_view line) override {
        captured_output += std::string(line) + "\n";
    }
