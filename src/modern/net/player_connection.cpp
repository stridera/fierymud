/***************************************************************************
 *   File: src/modern/net/player_connection.cpp     Part of FieryMUD *
    ...
    switch (command) {
        ...
        default:
            return std::unexpected(Error{ErrorCode::InvalidArgument, 
                fmt::format("Unknown telnet command: {}", command)});
    }
}
...
Result<void> GMCPHandler::process_gmcp_message(std::string_view message) {
    try {
        ...
        return std::unexpected(Error{ErrorCode::ParseError, 
            fmt::format("Failed to parse GMCP message: {}", e.what())});
    }
}
...
Result<void> GMCPHandler::send_gmcp(std::string_view module, const nlohmann::json& data) {
    ...
    try {
        ...
        return std::unexpected(Error{ErrorCode::SerializationError,
            fmt::format("Failed to send GMCP: {}", e.what())});
    }
}
...
void GMCPHandler::send_room_info(const Room& room) {
    if (!client_supports("Room")) return;
    
    nlohmann::json room_data = {
        {"num", room.id().value()},
        {"name", room.name()},
        {"zone", room.zone_name()},
        {"environment", room.sector_type()},
        {"coord", {
    ...
    // Add exits
    for (const auto& [direction, exit] : room.exits()) {
        room_data["exits"][direction] = exit.destination().value();
    }
...
std::shared_ptr<PlayerConnection> PlayerConnection::create(
    ...
    return std::shared_ptr<PlayerConnection>(
        new PlayerConnection(io_context, std::move(world_server))
    );
}
...
PlayerConnection::PlayerConnection(
    ...
    : io_context_(io_context),
      ...
      socket_(io_context),
      world_server_(std::move(world_server)),
      gmcp_handler_(*this),
...
void PlayerConnection::start() {
    ...
    login_system_->set_player_loaded_callback(
        [this](std::shared_ptr<Player> player) {
            on_login_completed(player);
        }
    );
...
void PlayerConnection::send_telnet_negotiation() {
    ...
    std::vector<uint8_t> gmcp_offer = {
        GMCPHandler::TELNET_IAC, 
        GMCPHandler::TELNET_WILL, 
        GMCPHandler::GMCP_OPTION
    };
    ...
    for (size_t i = 0; i < data.size(); ++i) {
        ...
        if (byte == GMCPHandler::TELNET_IAC && !in_telnet_negotiation_) {
            ...
            if (telnet_buffer_.size() >= 3) {
                ...
                if (command == GMCPHandler::TELNET_WILL || 
                    command == GMCPHandler::TELNET_WONT ||
                    command == GMCPHandler::TELNET_DO || 
                    command == GMCPHandler::TELNET_DONT) {
                    ...
                    if (telnet_buffer_.size() >= 5 && 
                        ...
                        std::string sub_data(
                            telnet_buffer_.begin() + 3, 
                            telnet_buffer_.end() - 2
                        );
...
void PlayerConnection::process_input(std::string_view input) {
    Log::debug("PlayerConnection::process_input: state={}, input='{}'", 
               static_cast<int>(state_), input);
    ...
    if (state_ == ConnectionState::Login && login_system_) {
        ...
        if (auto cmd_system = world_server_->get_command_system()) {
            Log::debug("Command system available, processing input");
            cmd_system->process_input(player_, input);
        } else {
            Log::error("Command system not available!");
            send_message("Command system not available.");
        }
        send_prompt();
    }
    else {
        Log::warn("Unexpected state in process_input: state={}, has_player={}", 
                 static_cast<int>(state_), player_ != nullptr);
    }
}
...
void PlayerConnection::transition_to(ConnectionState new_state) {
    ...
    Log::debug("Connection {} transitioning from {} to {}", 
               remote_address(), 
...
void PlayerConnection::send_room_info() {
    if (!player_ || !supports_gmcp()) return;
